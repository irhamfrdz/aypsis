...
class TandaTerimaLclBatam extends Model
{
    use AsuransiManageable, Auditable, HasFactory;

    protected $table = 'tanda_terima_lcl_batams';
    ...
    {
        return $this->hasMany(TandaTerimaLclBatamItem::class, 'tanda_terima_lcl_batam_id');
    }

    // Kontainer pivot relationship
    public function kontainerPivot()
    {
        return $this->hasMany(TtLclBatamKontainerPivot::class, 'tt_lcl_batam_id');
    }
    ...
    public function getTandaTerimaSeKontainerAttribute()
    {
        ...
        $tandaTerimaIds = TtLclBatamKontainerPivot::where('nomor_kontainer', $pivot->nomor_kontainer)
            ->where('tt_lcl_batam_id', '!=', $this->id)
            ->pluck('tt_lcl_batam_id');
