...
class UangJalanController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware('permission:uang-jalan-create')->only(['create', 'store', 'storePenyesuaian']);
    ...
    public function selectSuratJalan(Request $request)
    {
        ...
        return view('uang-jalan.select-surat-jalan-penyesuaian', compact('suratJalans', 'search', 'status', 'statusOptions'));
    }

    /**
     * Show the form for creating a new penyesuaian uang jalan.
     */
    public function createPenyesuaian(Request $request)
    {
        $suratJalanId = $request->get('surat_jalan_id');

        if (!$suratJalanId) {
            return redirect()->route('uang-jalan.select-surat-jalan-penyesuaian')
                           ->with('error', 'Surat jalan harus dipilih terlebih dahulu');
        }

        // Cari surat jalan dan uang jalan yang sudah ada
        $suratJalan = SuratJalan::with(['order.pengirim', 'order.jenisBarang', 'uangJalan'])->find($suratJalanId);
        ...
        if (!$suratJalan->uangJalan) {
            return redirect()->route('uang-jalan.select-surat-jalan-penyesuaian')
                           ->with('error', 'Surat jalan ini belum memiliki uang jalan');
        }

        return view('uang-jalan.create-penyesuaian', compact('suratJalan'));
    }

    /**
     * Store a newly created penyesuaian uang jalan in storage.
     */
    public function storePenyesuaian(Request $request)
    {
        $request->validate([
            ...
            'jenis_penyesuaian' => 'required|in:penambahan,pengurangan,penggantian',
            'nominal_penyesuaian' => 'required|numeric|min:0',
            'alasan_penyesuaian' => 'required|string|max:500',
        ]);

        try {
            // Cari surat jalan dan uang jalan yang sudah ada
            $suratJalan = SuratJalan::with('uangJalan')->findOrFail($request->surat_jalan_id);
            ...
            $nominalPenyesuaian = $request->nominal_penyesuaian;
            $jenisPenyesuaian = $request->jenis_penyesuaian;

            // Hitung total baru berdasarkan jenis penyesuaian
            $totalLama = $uangJalan->jumlah_total;
            $totalBaru = $totalLama;

            if ($jenisPenyesuaian === 'penambahan') {
                $totalBaru = $totalLama + $nominalPenyesuaian;
            } elseif ($jenisPenyesuaian === 'pengurangan') {
                $totalBaru = $totalLama - $nominalPenyesuaian;
                if ($totalBaru < 0) {
                    return back()->withInput()->with('error', 'Total uang jalan tidak boleh negatif');
                }
            } elseif ($jenisPenyesuaian === 'penggantian') {
                $totalBaru = $nominalPenyesuaian;
            }

            // Update uang jalan dengan data penyesuaian
            $uangJalan->update([
                'jumlah_total' => $totalBaru,
                'memo' => ($uangJalan->memo ? $uangJalan->memo . "\n\n" : '') .
                         "PENYESUAIAN " . strtoupper($jenisPenyesuaian) . " - " .
                         date('d/m/Y', strtotime($request->tanggal_penyesuaian)) . "\n" .
                         "Nominal: Rp " . number_format($nominalPenyesuaian, 0, ',', '.') . "\n" .
                         "Total Lama: Rp " . number_format($totalLama, 0, ',', '.') . "\n" .
                         "Total Baru: Rp " . number_format($totalBaru, 0, ',', '.') . "\n" .
                         "Alasan: " . $request->alasan_penyesuaian,
                'updated_by' => Auth::id(),
            ]);

            Log::info('Uang jalan penyesuaian created successfully', [
                'uang_jalan_id' => $uangJalan->id,
                'surat_jalan_id' => $suratJalan->id,
                'jenis_penyesuaian' => $jenisPenyesuaian,
                'nominal_penyesuaian' => $nominalPenyesuaian,
            ...
            return redirect()->route('uang-jalan.index')
                           ->with('success', 'Penyesuaian uang jalan berhasil disimpan');

        } catch (\Exception $e) {
            Log::error('Error creating uang jalan penyesuaian', [
    ...
    /**
     * Display a listing of surat jalan untuk dipilih sebagai basis penyesuaian uang jalan.
     */
    public function selectSuratJalanPenyesuaian(Request $request)
    {
        $search = $request->get('search');
        $status = $request->get('status', 'all'); // Default filter
