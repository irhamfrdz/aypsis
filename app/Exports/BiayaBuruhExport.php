...
class BiayaBuruhExport implements FromView, ShouldAutoSize, WithEvents
{
    protected $biayaKapal;

    public function __construct($biayaKapal)
    {
        $this->biayaKapal = $biayaKapal;
    }

    public function view(): View
    {
        // Get grouped details
        $groupedDetails = $this->biayaKapal->barangDetails->groupBy(function ($item) {
            return ($item->kapal ?? '-').'|'.($item->voyage ?? '-').'|'.($item->nomor_referensi ?? '-');
        });

        // Combined barang
        $combinedBarang = $this->biayaKapal->barangDetails
        ...
        $totalAdjustments = $this->biayaKapal->barangDetails
        ...
        // Grouped buruhs
        $tenagaKerjaGroups = $this->biayaKapal->tenagaKerjaDetails->groupBy(function ($item) {
            return ($item->kapal ?? '-').' - '.($item->voyage ?? '-');
        });

        return view('exports.biaya_buruh', [
            'biayaKapal' => $this->biayaKapal,
