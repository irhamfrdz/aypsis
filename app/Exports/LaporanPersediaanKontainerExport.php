    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('K4', 'BATAM');
                $sheet->mergeCells('K4:S4');
                $sheet->setCellValue('T4', 'PINANG');
                $sheet->mergeCells('T4:AB4');
                $sheet->setCellValue('AC4', 'KAPAL');
                $sheet->mergeCells('AC4:AK4');
                $sheet->setCellValue('AL4', 'GRAND TOTAL');
                $sheet->mergeCells('AL4:AV4');

                // Sizes Row 5
                $sheet->setCellValue('A5', 'BULAN');
                $sheet->mergeCells('A4:A6');
                $sheet->setCellValue('B5', '20 Feet');
                $sheet->mergeCells('B5:F5');
                $sheet->setCellValue('G5', '40 Feet');
                $sheet->mergeCells('G5:J5');
                $sheet->setCellValue('K5', '20 Feet');
                $sheet->mergeCells('K5:O5');
                $sheet->setCellValue('P5', '40 Feet');
                $sheet->mergeCells('P5:S5');
                $sheet->setCellValue('T5', '20 Feet');
                $sheet->mergeCells('T5:X5');
                $sheet->setCellValue('Y5', '40 Feet');
                $sheet->mergeCells('Y5:AB5');
                $sheet->setCellValue('AC5', '20 Feet');
                $sheet->mergeCells('AC5:AG5');
                $sheet->setCellValue('AH5', '40 Feet');
                $sheet->mergeCells('AH5:AK5');
                $sheet->setCellValue('AL5', '20 Feet');
                $sheet->mergeCells('AL5:AQ5');
                $sheet->setCellValue('AR5', '40 Feet');
                ...
                foreach ($months as $mIdx => $mName) {
                    ...
                    foreach ($stocks as $s) {
                        ...
                        $size = (int) $s->ukuran;
                        if ($size != 20 && $size != 40) {
                            if (str_contains($s->ukuran, '20')) {
                                $size = 20;
                            } elseif (str_contains($s->ukuran, '40')) {
                                $size = 40;
                            } else {
                                $size = 20;
                            }
                ...
                $sheet->setCellValue('G'.($startRow + 1), 'Jakarta');
                $sheet->mergeCells('G'.($startRow + 1).':I'.($startRow + 1));
                $sheet->setCellValue('J'.($startRow + 1), 'Batam');
                $sheet->mergeCells('J'.($startRow + 1).':L'.($startRow + 1));
                $sheet->setCellValue('M'.($startRow + 1), 'Pinang');
                $sheet->mergeCells('M'.($startRow + 1).':O'.($startRow + 1));
                $sheet->setCellValue('P'.($startRow + 1), 'Kapal');
                $sheet->mergeCells('P'.($startRow + 1).':R'.($startRow + 1));
                $sheet->setCellValue('S'.($startRow + 1), 'TOTAL');
                $sheet->mergeCells('S'.($startRow + 1).':U'.($startRow + 1));
                ...
                $sheet->setCellValue('X'.($startRow + 1), 'Jakarta');
                $sheet->mergeCells('X'.($startRow + 1).':Z'.($startRow + 1));
                $sheet->setCellValue('AA'.($startRow + 1), 'Batam');
                $sheet->mergeCells('AA'.($startRow + 1).':AC'.($startRow + 1));
                $sheet->setCellValue('AD'.($startRow + 1), 'Pinang');
                $sheet->mergeCells('AD'.($startRow + 1).':AF'.($startRow + 1));
                $sheet->setCellValue('AG'.($startRow + 1), 'Kapal');
                $sheet->mergeCells('AG'.($startRow + 1).':AI'.($startRow + 1));
                $sheet->setCellValue('AJ'.($startRow + 1), 'TOTAL');
                $sheet->mergeCells('AJ'.($startRow + 1).':AL'.($startRow + 1));
                ...
                foreach ($onBoardList as $no) {
                    ...
                    if ($latestNaik) {
                        ...
                        if ($size != 20 && $size != 40) {
                            if (str_contains($latestNaik->ukuran_kontainer, '20')) {
                                $size = 20;
                            } elseif (str_contains($latestNaik->ukuran_kontainer, '40')) {
                                $size = 40;
                            } else {
                                $size = 20;
                            }
