...
class AnalyzeCoa extends Command
{
    ...
    public function handle()
    {
        ...
        $coaTotalDebit = $coaTransactions->sum(function ($t) {
            return (float) $t->debit;
        });
        $coaTotalKredit = $coaTransactions->sum(function ($t) {
        ...
        $originalTagihans = DaftarTagihanKontainerSewa::whereNotNull('grand_total')
            ->where('grand_total', '>', 0)
            ->get();
        $originalTotalAmount = $originalTagihans->sum(function ($t) {
            return (float) $t->grand_total;
        });
        ...
        foreach ($coaAmountGroups as $group => $transactions) {
            $groupTotal = $transactions->sum(function ($t) {
                return (float) $t->debit;
            });
