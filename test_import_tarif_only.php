...
echo "=== Test Import TANPA tarif_nominal ===\n\n";
...
while (($row = fgetcsv($handle, 1000, $delimiter)) !== false) {
    ...
    try {
        ...
        // Calculate tarif untuk perhitungan (tidak disimpan di database)
        $tarifForCalc = ($vendor === 'DPE')
            ? (($size == '20') ? 25000 : 35000)
            : (($size == '20') ? 20000 : 30000);

        // Calculate financial data
        $dpp = $tarifForCalc * $jumlahHari;
        $ppn = $dpp * 0.11;
        $pph = $dpp * 0.02;
        ...
        if ($rowNumber <= 10) {
            ...
            echo "  Tarif (TEXT): \"$tarifText\"\n";
            echo "  Periode: $jumlahHari hari\n";
            echo "  DPP: Rp " . number_format($dpp, 0, ',', '.') . " (dihitung dari tarif default)\n\n";
        }
        ...
        // Create record dengan TARIF TEXT only (NO tarif_nominal)
        DaftarTagihanKontainerSewa::create([
            ...
            'periode' => $jumlahHari,
            'masa' => $masa,
            'tarif' => $tarifText, // ONLY TEXT: "Bulanan" or "Harian"
            'status' => $status,
            'status_pranota' => null,
            'pranota_id' => null,
            'dpp' => $dpp,
...
foreach ($samples as $sample) {
    ...
    echo "  Tarif: \"{$sample->tarif}\" (hanya TEXT)\n";
