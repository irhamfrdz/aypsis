    ...
    <div class="bg-white shadow-lg rounded-lg p-4">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
            <div class="mb-4 sm:mb-0">
                ...
                <p class="text-sm text-gray-600 mt-1">{{ $pranotaUangRitKenek->no_pranota }}</p>
            </div>
            <div class="flex flex-col sm:flex-row gap-2">
                @can('pranota-uang-rit-update')
                    @if(in_array($pranotaUangRitKenek->status, ['draft', 'submitted']))
                    <a href="{{ route('pranota-uang-rit-kenek.edit', $pranotaUangRitKenek) }}" 
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                        <div>
                            ...
                            <input type="text" class="{{ $inputClasses }} font-medium text-indigo-600" 
                                   value="{{ $pranotaUangRitKenek->no_pranota }}" readonly>
                        </div>
                        <div>
                            <label for="tanggal" class="{{ $labelClasses }}">Tanggal</label>
                            <input type="text" class="{{ $inputClasses }}" 
                                   value="{{ $pranotaUangRitKenek->tanggal ? $pranotaUangRitKenek->tanggal->format('d/m/Y') : '-' }}" readonly>
                        </div>
                        <div>
                            <label for="status" class="{{ $labelClasses }}">Status</label>
                            <span class="mt-1 inline-flex px-2 py-1 text-xs font-semibold rounded-full
                                {{ $pranotaUangRitKenek->status === 'draft' ? 'bg-gray-100 text-gray-800' : 
                                   ($pranotaUangRitKenek->status === 'submitted' ? 'bg-blue-100 text-blue-800' : 
                                   ($pranotaUangRitKenek->status === 'approved' ? 'bg-green-100 text-green-800' : 
                                   ($pranotaUangRitKenek->status === 'paid' ? 'bg-purple-100 text-purple-800' : 'bg-red-100 text-red-800'))) }}">
                                {{ $pranotaUangRitKenek->status_label }}
                            </span>
                        </div>
                        <div>
                            <label for="created_by" class="{{ $labelClasses }}">Dibuat Oleh</label>
                            <input type="text" class="{{ $inputClasses }}" 
                                   value="{{ $pranotaUangRitKenek->creator->name ?? '-' }}" readonly>
                        </div>
                    </div>
                    @if($pranotaUangRitKenek->keterangan)
                    <div class="mt-2">
                        <label for="keterangan" class="{{ $labelClasses }}">Keterangan</label>
                        <textarea class="{{ $inputClasses }}" rows="2" readonly>{{ $pranotaUangRitKenek->keterangan }}</textarea>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="lg:col-span-2">
                <div class="bg-gray-50 rounded-lg p-3 border border-gray-200">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                        ...
                        <div>
                            <label for="jumlah_Kenek" class="{{ $labelClasses }}">Jumlah Kenek</label>
                            <input type="text" class="{{ $inputClasses }}" value="{{ $KenekDetails->count() ?? 1 }}" readonly>
                        </div>
                        <div>
                            <label for="total_uang_rit" class="{{ $labelClasses }}">Total Uang Kenek</label>
                            <input type="text" class="{{ $inputClasses }} font-bold text-indigo-600" 
                                   value="Rp {{ number_format($pranotaUangRitKenek->total_uang, 0, ',', '.') }}" readonly>
                        </div>
                        <div>
                            <label for="total_utang" class="{{ $labelClasses }}">Total Utang</label>
                            <input type="text" class="{{ $inputClasses }} font-bold text-red-600" 
                                   value="Rp {{ number_format($pranotaUangRitKenek->total_hutang, 0, ',', '.') }}" readonly>
                        </div>
                        <div>
                        ...
                        @if($groupedPranota && $groupedPranota->count() > 0)
                            @foreach($groupedPranota as $pranota)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-2 py-2 whitespace-nowrap text-xs">{{ $pranota->Kenek_nama ?? '-' }}</td>
                                    ...
                                    <td class="px-2 py-2 whitespace-nowrap text-right text-xs font-semibold text-indigo-600">
                                        Rp {{ number_format($pranota->uang_rit_Kenek, 0, ',', '.') }}
                                    </td>
                                </tr>
                            @endforeach
                        @else
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-2 py-2 whitespace-nowrap text-xs font-medium">{{ $pranotaUangRitKenek->no_surat_jalan ?? '-' }}</td>
                                <td class="px-2 py-2 whitespace-nowrap text-xs text-center">{{ $pranotaUangRitKenek->tanggal ? $pranotaUangRitKenek->tanggal->format('d/m/Y') : '-' }}</td>
                                <td class="px-2 py-2 whitespace-nowrap text-xs">{{ $pranotaUangRitKenek->Kenek_nama ?? '-' }}</td>
                                <td class="px-2 py-2 whitespace-nowrap text-xs text-center">1</td>
                                <td class="px-2 py-2 whitespace-nowrap text-right text-xs font-semibold text-indigo-600">
                                    Rp {{ number_format($pranotaUangRitKenek->uang_rit_Kenek, 0, ',', '.') }}
                                </td>
                            </tr>
                        @endif
                    </tbody>
        ...
        @if($pranotaUangRitKenek->created_at || $pranotaUangRitKenek->approved_at)
        <div class="bg-white border border-gray-200 rounded-lg overflow-hidden mt-4">
            <div class="bg-gray-50 px-3 py-2 border-b border-gray-200">
            ...
            <div class="p-3">
                <div class="space-y-2">
                    @if($pranotaUangRitKenek->created_at)
                    <div class="flex items-center text-xs text-gray-600">
                        <div class="w-2 h-2 bg-blue-400 rounded-full mr-2"></div>
                        <span class="font-medium">Dibuat:</span>
                        <span class="ml-1">{{ $pranotaUangRitKenek->created_at->format('d/m/Y H:i') }}</span>
                        @if($pranotaUangRitKenek->creator)
                        <span class="ml-1">oleh {{ $pranotaUangRitKenek->creator->name }}</span>
                        @endif
                    </div>
                    @endif
                    
                    @if($pranotaUangRitKenek->approved_at)
                    <div class="flex items-center text-xs text-gray-600">
                        <div class="w-2 h-2 bg-green-400 rounded-full mr-2"></div>
                        <span class="font-medium">Disetujui:</span>
                        <span class="ml-1">{{ $pranotaUangRitKenek->approved_at ? $pranotaUangRitKenek->approved_at->format('d/m/Y H:i') : '-' }}</span>
                        @if($pranotaUangRitKenek->approver)
                        <span class="ml-1">oleh {{ $pranotaUangRitKenek->approver->name }}</span>
                        @endif
                    </div>
                    @endif
                </div>
