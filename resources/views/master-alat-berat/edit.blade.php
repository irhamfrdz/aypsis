...
@section('page_title', 'Edit Alat Berat')
...
<div class="mx-auto max-w-6xl space-y-6">
    <div class="flex flex-col gap-4 rounded-2xl bg-gradient-to-r from-indigo-700 to-indigo-500 px-6 py-6 text-white shadow-lg sm:flex-row sm:items-center sm:justify-between sm:px-8">
        <div class="flex items-center gap-4"><div class="rounded-xl bg-white/15 p-3"><i class="fa-solid fa-pen-to-square text-2xl"></i></div><div><p class="text-sm text-indigo-100">Master Data</p><h2 class="text-2xl font-bold">Edit Alat Berat</h2><p class="mt-1 text-sm text-indigo-100">Perbarui identitas, sertifikat, dan informasi operasional alat.</p></div></div>
        <div>
            <span class="inline-flex items-center rounded-full bg-white/15 px-3 py-1 text-xs font-semibold text-white ring-1 ring-white/30">
                {{ ucfirst($alatBerat->status) }}
            </span>
        </div>
    </div>
    ...
    <form action="{{ route('master.alat-berat.update', $alatBerat->id) }}" method="POST" class="alat-berat-form space-y-6">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8"><div class="mb-6 border-b border-gray-100 pb-4"><h3 class="font-semibold text-gray-900">Identitas Alat</h3><p class="mt-1 text-sm text-gray-500">Informasi dasar alat berat yang didaftarkan.</p></div><div class="grid grid-cols-1 gap-5 md:grid-cols-2">
            ...
            <div>
                ...
                <input type="text" name="kode_alat" id="kode_alat" value="{{ old('kode_alat', $alatBerat->kode_alat) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm bg-gray-100" readonly>
            </div>
            ...
            <div>
                ...
                <input type="text" name="nickname" id="nickname" value="{{ old('nickname', $alatBerat->nickname) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Contoh: EX-01">
            </div>
            ...
            <div>
                ...
                <input type="text" name="jenis" id="jenis" value="{{ old('jenis', $alatBerat->jenis) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Contoh: Excavator, Forklift">
            </div>

            <!-- Merk -->
            <div>
                <label for="merk" class="block text-sm font-medium text-gray-700">Merek</label>
                <input type="text" name="merk" id="merk" value="{{ old('merk', $alatBerat->merk) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Contoh: Komatsu, Caterpillar">
            </div>

            <!-- Tipe -->
            <div>
                <label for="tipe" class="block text-sm font-medium text-gray-700">Tipe/Model</label>
                <input type="text" name="tipe" id="tipe" value="{{ old('tipe', $alatBerat->tipe) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Contoh: PC200-8">
            </div>

            <!-- Kapasitas -->
            <div>
                <label for="kapasitas" class="block text-sm font-medium text-gray-700">Kapasitas</label>
                <input type="text" name="kapasitas" id="kapasitas" value="{{ old('kapasitas', $alatBerat->kapasitas) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Contoh: 20 Ton, 50 Ton">
            </div>
            ...
            <div>
                ...
                <div class="relative mt-1">
                    ...
                    <input type="number" step="0.01" min="0" name="biaya_sertifikat_sia" id="biaya_sertifikat_sia" value="{{ old('biaya_sertifikat_sia', $alatBerat->biaya_sertifikat_sia) }}" class="mt-1 block w-full rounded-md border-gray-300 pl-10 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="0">
                </div>
            ...
            <div>
                ...
                <input type="text" name="lokasi" id="lokasi" value="{{ old('lokasi', $alatBerat->lokasi) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Contoh: Gudang A, Site B">
            </div>
            ...
            <div>
                ...
                <div class="mt-1 relative rounded-md shadow-sm">
                    ...
                    <input type="number" step="0.01" name="tarif_harian" id="tarif_harian" value="{{ old('tarif_harian', $alatBerat->tarif_harian) }}" class="pl-10 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="0.00">
                </div>
            </div>
            ...
            <div>
                ...
                <div class="mt-1 relative rounded-md shadow-sm">
                    ...
                    <input type="number" step="0.01" name="tarif_bulanan" id="tarif_bulanan" value="{{ old('tarif_bulanan', $alatBerat->tarif_bulanan) }}" class="pl-10 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="0.00">
                </div>
            </div>
            ...
            <div>
                ...
                <select name="status" id="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                    <option value="active" {{ old('status', $alatBerat->status) == 'active' ? 'selected' : '' }}>Active</option>
                    <option value="maintenance" {{ old('status', $alatBerat->status) == 'maintenance' ? 'selected' : '' }}>Maintenance</option>
                    <option value="inactive" {{ old('status', $alatBerat->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                </select>
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="keterangan" id="keterangan" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ old('keterangan', $alatBerat->keterangan) }}</textarea>
            </div>
        </div>
