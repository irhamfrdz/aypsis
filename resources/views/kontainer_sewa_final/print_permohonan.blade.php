...
<head>
    <meta charset="UTF-8">
    <title>PERMOHONAN TRANSFER - {{ $permohonan->nomor }}</title>
    <style>
...
<div class="header">
    ...
    <p style="margin: 5px 0 0 0; font-size: 11px;">Status: {{ $permohonan->status }}</p>
</div>
...
<table class="info-table">
    <tr>
        ...
        <td width="20">:</td>
        <td><strong>{{ $permohonan->nomor }}</strong></td>
        <td width="150" align="right">Tanggal Mohon</td>
        <td width="20" align="center">:</td>
        <td width="120"><strong>{{ date('d-M-Y', strtotime($permohonan->tanggal)) }}</strong></td>
    </tr>
    <tr>
        <td>Vendor</td>
        <td>:</td>
        <td>{{ $permohonan->vendor_name }}</td>
        <td align="right">Jenis Transaksi</td>
        <td align="center">:</td>
        <td>{{ $permohonan->jenis_transaksi }}</td>
    </tr>
    <tr>
        <td>Keterangan</td>
        <td>:</td>
        <td colspan="4">{{ $permohonan->keterangan ?: '-' }}</td>
    </tr>
</table>
...
<table class="items-table">
    ...
    <tfoot>
        <tr>
            ...
            <td align="right"><strong>Rp {{ number_format($permohonan->total_pembayaran, 0, ',', '.') }}</strong></td>
        </tr>
        @if($permohonan->total_penyesuaian != 0)
        <tr>
            <td colspan="3" align="right">
                PENYESUAIAN 
                @if($permohonan->alasan_penyesuaian) <br><small>({{ $permohonan->alasan_penyesuaian }})</small> @endif
            </td>
            <td align="right">
                Rp {{ number_format($permohonan->total_penyesuaian, 0, ',', '.') }}
            </td>
        </tr>
        @endif
        <tr style="font-size: 1.1em;">
            <td colspan="3" align="right"><strong>GRAND TOTAL</strong></td>
            <td align="right" style="background: #f2f2f2;"><strong>Rp {{ number_format($permohonan->grand_total, 0, ',', '.') }}</strong></td>
        </tr>
    </tfoot>
</table>
...
<div class="footer">
    <div class="sign-box">
        <p>Diajukan Oleh,</p>
        <span class="name">{{ $permohonan->creator->name ?? 'Staff Finance' }}</span>
    </div>
    <div class="sign-box">
        <p>Diperiksa Oleh,</p>
...
<!-- HALAMAN 2: LAMPIRAN DETAIL -->
<div style="page-break-before: always; margin-top: 30px;">
    <div class="header">
        <h2>Lampiran Detail Kontainer</h2>
        <p>Nomor Permohonan: {{ $permohonan->nomor }}</p>
    </div>
    ...
    <div style="margin-top: 20px; border-top: 2px solid #000; padding-top: 10px; text-align: right;">
        <p style="font-size: 1.2em;"><strong>Grand Total: Rp {{ number_format($permohonan->grand_total, 0, ',', '.') }}</strong></p>
    </div>
</div>
