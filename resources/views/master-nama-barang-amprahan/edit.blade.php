...
@section('title', 'Edit Nama Barang Amprahan')
@section('page_title', 'Edit Nama Barang Amprahan')
    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
                <h2 class="text-xl font-bold text-gray-800">Edit Nama Barang Amprahan</h2>
                <p class="text-sm text-gray-600">Perbarui data nama barang amprahan</p>
            </div>
            
            <form action="{{ route('master.nama-barang-amprahan.update', $barangAmprahan->id) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_barang" id="nama_barang" value="{{ old('nama_barang', $barangAmprahan->nama_barang) }}" 
                    ...
                    <div>
                        ...
                        <select name="status" id="status" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200" required>
                            <option value="active" {{ old('status', $barangAmprahan->status) == 'active' ? 'selected' : '' }}>Aktif</option>
                            <option value="inactive" {{ old('status', $barangAmprahan->status) == 'inactive' ? 'selected' : '' }}>Tidak Aktif</option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
