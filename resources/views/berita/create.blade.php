...
<div class="container mx-auto px-4 py-6 max-w-2xl">
    ...
    <form id="beritaForm" method="POST" action="{{ route('berita.store') }}" enctype="multipart/form-data"
        ...
        <input type="hidden" id="aspect_ratio" name="aspect_ratio" value="{{ old('aspect_ratio', '2.2:1') }}">
        ...
        <div class="border-t border-gray-100 pt-5 space-y-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-0.5">Unggah gambar dan sesuaikan ukuran tampilannya untuk dashboard PWA</p>
                </div>
                <span class="text-xs text-gray-400 bg-gray-100 px-2.5 py-1 rounded-full font-medium">Maks 5MB</span>
            </div>

            {{-- Dropzone / Upload Box --}}
            <div id="drop-zone" class="relative border-2 border-dashed border-gray-300 rounded-2xl p-6 text-center hover:border-indigo-400 transition-colors cursor-pointer bg-gray-50/50"
                 onclick="document.getElementById('gambar').click()">
                <div id="upload-icon">
                    <div class="w-12 h-12 mx-auto mb-2 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                    </div>
                    <p class="text-sm font-semibold text-gray-700">Klik untuk upload gambar atau tarik file ke sini</p>
                    <p class="text-xs text-gray-400 mt-1">Format didukung: JPG, PNG, WebP</p>
                </div>
            ...
            <div id="cropper-container" class="hidden bg-slate-900 rounded-2xl p-4 text-white shadow-xl space-y-4">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                        <button type="button" class="ratio-btn active p-2 rounded-xl border border-slate-700 bg-slate-800 hover:border-indigo-400 text-slate-200 transition-all text-left" data-ratio="2.2">
                            <span class="block font-bold">2.2 : 1 ⭐</span>
                            <span class="text-[10px] text-slate-400">Banner PWA</span>
                        </button>
                        <button type="button" class="ratio-btn p-2 rounded-xl border border-slate-700 bg-slate-800 hover:border-indigo-400 text-slate-200 transition-all text-left" data-ratio="1.7778">
                            <span class="block font-bold">16 : 9</span>
                            <span class="text-[10px] text-slate-400">Widescreen</span>
                        </button>
                        <button type="button" class="ratio-btn p-2 rounded-xl border border-slate-700 bg-slate-800 hover:border-indigo-400 text-slate-200 transition-all text-left" data-ratio="1.3333">
                            <span class="block font-bold">4 : 3</span>
                            <span class="text-[10px] text-slate-400">Standar Lega</span>
                        </button>
                        <button type="button" class="ratio-btn p-2 rounded-xl border border-slate-700 bg-slate-800 hover:border-indigo-400 text-slate-200 transition-all text-left" data-ratio="1">
                            <span class="block font-bold">1 : 1</span>
                            <span class="text-[10px] text-slate-400">Persegi/Kotak</span>
                        </button>
                        <button type="button" class="ratio-btn p-2 rounded-xl border border-slate-700 bg-slate-800 hover:border-indigo-400 text-slate-200 transition-all text-left" data-ratio="0.8">
                            <span class="block font-bold">4 : 5</span>
                            <span class="text-[10px] text-slate-400">Poster Vertikal</span>
                        </button>
                        <button type="button" class="ratio-btn p-2 rounded-xl border border-slate-700 bg-slate-800 hover:border-indigo-400 text-slate-200 transition-all text-left" data-ratio="free">
                            <span class="block font-bold">Bebas</span>
                            <span class="text-[10px] text-slate-400">Custom Crop</span>
                        </button>
                        <button type="button" class="ratio-btn p-2 rounded-xl border border-slate-700 bg-slate-800 hover:border-indigo-400 text-slate-200 transition-all text-left sm:col-span-2" data-ratio="original">
                            <span class="block font-bold">Asli (Tanpa Potong)</span>
                            <span class="text-[10px] text-slate-400">Pertahankan ukuran file asli</span>
                        </button>
                    </div>
                </div>
        ...
        <div>
            ...
            <textarea id="konten" name="konten" rows="5" placeholder="Tulis isi berita atau keterangan pamflet..."
...
// Mapping rasio
const RATIO_MAP = {
    '2.2':    { key: '2.2:1', label: '2.2 : 1 (Banner PWA)', val: 2.2,    cssRatio: '2.2 / 1' },
    '1.7778': { key: '16:9',  label: '16 : 9 (Widescreen)',  val: 16 / 9, cssRatio: '16 / 9' },
    '1.3333': { key: '4:3',   label: '4 : 3 (Standar)',      val: 4 / 3,  cssRatio: '4 / 3' },
    '1':      { key: '1:1',   label: '1 : 1 (Persegi)',      val: 1,      cssRatio: '1 / 1' },
    '0.8':    { key: '4:5',   label: '4 : 5 (Poster)',       val: 0.8,    cssRatio: '4 / 5' },
    'free':   { key: 'free',  label: 'Bebas (Custom)',       val: NaN,    cssRatio: '2.2 / 1' },
    'original': { key: 'original', label: 'Asli (Tanpa Potong)', val: null, cssRatio: 'auto' },
};
...
function updateCropPreview() {
    ...
    if (isOriginalMode) {
        ...
        inputCropped.value = ''; // Biarkan file asli yang diunggah
    ...
    if (canvas) {
        ...
        // Atur frame mockup
        const ratioConfig = RATIO_MAP[currentRatio] || RATIO_MAP['2.2'];
...
document.querySelectorAll('.ratio-btn').forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        const ratioAttr = this.dataset.ratio;
        if (ratioAttr === 'original') {
            isOriginalMode = true;
            if (cropper) {
                cropper.clear();
            }
        ...
        if (ratioAttr === 'free') {
            ...
            const num = parseFloat(ratioAttr);
            currentRatio = ratioAttr;
            cropper.setAspectRatio(num);
        }
        updateCropPreview();
    });
});
