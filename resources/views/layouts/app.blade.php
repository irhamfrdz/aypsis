...
<body class="bg-gray-100 flex flex-col h-screen">
            ...
            <div class="flex items-center space-x-4">
                ...
                <div class="relative">
                    ...
                    <div id="profileDropdown" class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 hidden z-50">
                        <div class="py-2">
                            <a href="{{ route('profile.show') }}" target="_blank" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
    ...
    <div id="sidebar" class="fixed top-0 bottom-0 left-0 z-50 w-80 lg:w-64 bg-gradient-to-b from-blue-50 via-white to-gray-100 shadow-2xl border-r border-gray-300 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out flex flex-col lg:top-16 rounded-r-2xl lg:rounded-r-2xl overflow-y-auto">
            ...
            <nav class="flex-1 overflow-y-auto p-4 space-y-2 pb-20">
                @php
                    ...
                    $hasMasterPermissions = $user && (
                        ...
                        $user->can('master-pricelist-biaya-dokumen-view') ||
                ...
                @can('chatbox-view')
                @php
                    $isChatbox = Request::routeIs('chat.*');
                @endphp
                <a href="{{ route('chat.index') }}" target="_blank" class="flex items-center py-2 px-5 rounded-xl mb-4 transition-all duration-200 group shadow-sm text-xs {{ $isChatbox ? 'bg-blue-100 text-blue-700 font-bold' : 'text-gray-700 hover:bg-blue-100 hover:text-blue-700' }}">
                    <svg class="w-4 h-4 mr-2 {{ $isChatbox ? 'text-blue-700' : 'text-gray-500 group-hover:text-blue-700' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/>
                    </svg>
                    <span class="text-xs font-medium menu-text">Chatbox</span>
                </a>
                @endcan
                ...
                <!-- Dashboard Gerak Voyage -->
                @if($user && ($user->can('gerak-voyage-view') || $user->is_admin))
                @php
                    $isGerakVoyageDashboard = Request::routeIs('gerak-voyage.dashboard');
                @endphp
                <a href="{{ route('gerak-voyage.dashboard') }}" target="_blank" class="flex items-center py-2 px-5 rounded-xl mb-4 transition-all duration-200 group shadow-sm text-xs {{ $isGerakVoyageDashboard ? 'bg-blue-100 text-blue-700 font-bold' : 'text-gray-700 hover:bg-blue-100 hover:text-blue-700' }}">
                    <i class="fas fa-chart-line w-4 mr-2 {{ $isGerakVoyageDashboard ? 'text-blue-700' : 'text-gray-500 group-hover:text-blue-700' }}"></i>
                    <span class="text-xs font-medium menu-text">Dashboard Gerak Voyage</span>
                </a>
                @endif
                ...
                @php
                    $isAsuransiTandaTerima = Request::is('asuransi-tanda-terima*');
                @endphp
                <a href="{{ route('asuransi-tanda-terima.index') }}" target="_blank" class="flex items-center py-2 px-5 rounded-xl mb-4 transition-all duration-200 group shadow-sm text-xs {{ $isAsuransiTandaTerima ? 'bg-green-100 text-green-700 font-bold' : 'text-gray-700 hover:bg-green-100 hover:text-green-700' }}">
                    <svg class="w-4 h-4 mr-2 {{ $isAsuransiTandaTerima ? 'text-green-700' : 'text-gray-500 group-hover:text-green-700' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/>
                    </svg>
                    <span class="text-xs font-medium menu-text">Asuransi Tanda Terima</span>
                </a>
                @endcan

                <!-- Asuransi Tanda Terima Baru (Multi) -->
                @can('asuransi-tanda-terima-multi-view')
                @php
                    $isAsuransiTandaTerimaMulti = Request::is('asuransi-tanda-terima-multi*');
                @endphp
                <a href="{{ route('asuransi-tanda-terima-multi.index') }}" target="_blank" class="flex items-center py-2 px-5 rounded-xl mb-4 transition-all duration-200 group shadow-sm text-xs {{ $isAsuransiTandaTerimaMulti ? 'bg-blue-100 text-blue-700 font-bold' : 'text-gray-700 hover:bg-blue-100 hover:text-blue-700' }}">
                    <svg class="w-4 h-4 mr-2 {{ $isAsuransiTandaTerimaMulti ? 'text-blue-700' : 'text-gray-500 group-hover:text-blue-700' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/>
                    </svg>
                    <span class="text-xs font-medium menu-text">Asuransi Tanda Terima Baru</span>
                </a>
                @endcan
                ...
                @php
                    $isRekapPerbaikan = Request::routeIs('rekap-perbaikan-kontainer.*');
                @endphp
                <a href="{{ route('rekap-perbaikan-kontainer.index') }}" target="_blank" class="flex items-center py-2 px-5 rounded-xl mb-4 transition-all duration-200 group shadow-sm text-xs {{ $isRekapPerbaikan ? 'bg-red-100 text-red-700 font-bold' : 'text-gray-700 hover:bg-red-100 hover:text-red-700' }}">
                    <svg class="w-4 h-4 mr-2 {{ $isRekapPerbaikan ? 'text-red-700' : 'text-gray-500 group-hover:text-red-700' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="mt-4 mb-2">
                    ...
                    <div id="payroll-menu-content" class="dropdown-content ml-2 mt-3 space-y-2" @if($isPayrollRoute) style="display: block;" @endif>
                        ...
                        @if($user->can('payroll-perhitungan-lembur-view') || $isAdmin)
                        <a href="{{ route('payroll.perhitungan-lembur') }}" class="flex items-center py-2 px-3 rounded-lg text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('payroll.perhitungan-lembur') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600 hover:shadow-sm' }}">
                            <span class="text-xs font-medium">Perhitungan Lembur</span>
                        </a>
                        @endif
                        ...
                        <div class="mx-2 mb-3">
                            ...
                            <div id="master-umum-content" class="dropdown-content ml-4 mt-2 space-y-1" @if($isUmumRoute) style="display: block;" @endif>
                                ...
                                @if($user && $user->can('master-kode-nomor-view'))
                                    <a href="{{ route('master.kode-nomor.index') }}" target="_blank" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('master.kode-nomor.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Kode Penomoran</span>
                                    </a>
                                @endif
                                @if($user && $user->can('master-nomor-terakhir-view'))
                                    <a href="{{ route('master.nomor-terakhir.index') }}" target="_blank" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('master.nomor-terakhir.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Nomor Terakhir</span>
                                    </a>
                                @endif
                                @if($user && $user->can('master-kegiatan-view'))
                                    <a href="{{ route('master.kegiatan.index') }}" target="_blank" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('master.kegiatan.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Jenis Kegiatan</span>
                                    </a>
                                @endif
                                @if($user && $user->can('master-pelabuhan-view'))
                                    <a href="{{ route('master-pelabuhan.index') }}" target="_blank" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('master-pelabuhan.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Pelabuhan</span>
                                    </a>
                                @endif
                                @if($user && $user->can('master-dokumen-perijinan-kapal-view'))
                                    <a href="{{ route('master-dokumen-perijinan-kapal.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('master-dokumen-perijinan-kapal.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                ...
                                @if($user && $user->can('master-klasifikasi-biaya-view'))
                                    <a href="{{ route('klasifikasi-biaya.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('klasifikasi-biaya.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Master Klasifikasi Biaya</span>
                                    </a>
                                @endif
                                ...
                                @if($user && $user->can('master-customer-buruh-view'))
                                    <a href="{{ route('master-customer-buruh.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('master-customer-buruh.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs font-semibold">Customer Buruh</span>
                                    </a>
                                @endif
                                ...
                                @if($isAdmin || ($user && $user->can('master-wa-broadcast-view')))
                                    ...
                                    <a href="{{ route('master.wa-gateway.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('master.wa-gateway.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs font-semibold">Koneksi WA Gateway</span>
                                    </a>
                                @endif
                                </div>
                            </div>
                        ...
                        <div class="mx-2 mb-3">
                            ...
                            <div id="master-akun-content" class="dropdown-content ml-4 mt-2 space-y-1" @if($isAkunRoute) style="display: block;" @endif>
                                @if($user && $user->can('master-coa-view'))
                                    <a href="{{ route('master-coa-index') }}" target="_blank" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('master-coa-*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">COA</span>
                                    </a>
                                @endif
                                @if($user && $user->can('master-tipe-akun-view'))
                                    <a href="{{ route('master.tipe-akun.index') }}" target="_blank" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('master.tipe-akun.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        ...
                        <div class="mx-2 mb-3">
                            ...
                            <div id="master-penjualan-content" class="dropdown-content ml-4 mt-2 space-y-1" @if($isPenjualanRoute) style="display: block;" @endif>
                                @if($user && $user->can('master-pengirim-view'))
                                    <a href="{{ route('pengirim.index') }}" target="_blank" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-orange-50 hover:text-orange-700 transition-all duration-200 {{ Request::routeIs('pengirim.*') ? 'bg-orange-50 text-orange-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                ...
                                @if($user && $user->can('master-jenis-barang-view'))
                                    <a href="{{ route('jenis-barang.index') }}" target="_blank" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-orange-50 hover:text-orange-700 transition-all duration-200 {{ Request::routeIs('jenis-barang.*') ? 'bg-orange-50 text-orange-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                ...
                                @if($user && $user->can('master-tujuan-kirim-view'))
                                    <a href="{{ route('master.tujuan-kegiatan-utama.index') }}" target="_blank" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-orange-50 hover:text-orange-700 transition-all duration-200 {{ Request::routeIs('master.tujuan-kegiatan-utama.*') ? 'bg-orange-50 text-orange-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Master Transportasi</span>
                                    </a>
                                @endif
                                @if($user && $user->can('master-tujuan-kirim-view'))
                                    <a href="{{ route('tujuan-kirim.index') }}" target="_blank" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-orange-50 hover:text-orange-700 transition-all duration-200 {{ Request::routeIs('tujuan-kirim.*') ? 'bg-orange-50 text-orange-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Master Tujuan Kirim</span>
                                    </a>
                                @endif
                                @if($user && $user->can('master-tujuan-view'))
                                    <a href="{{ route('master.tujuan.index') }}" target="_blank" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-orange-50 hover:text-orange-700 transition-all duration-200 {{ Request::routeIs('master.tujuan.*') ? 'bg-orange-50 text-orange-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        ...
                        <div class="mx-2 mb-3">
                            ...
                            <div id="master-karyawan-content" class="dropdown-content ml-4 mt-2 space-y-1" @if($isUserRoute) style="display: block;" @endif>
                                ...
                                @if($user && $user->can('karyawan-tidak-tetap-view'))
                                    ...
                                    <a href="{{ route('karyawan-tidak-tetap.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-blue-50 hover:text-blue-700 transition-all duration-200 {{ Request::routeIs('karyawan-tidak-tetap.*') ? 'bg-blue-50 text-blue-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Karyawan Tidak Tetap</span>
                                    </a>
                                @endif
                                @if($user && $user->can('master-karyawan-view'))
                                    <!-- Master Group BP Jamsostek -->
                                    <a href="{{ route('master-group-bp-jamsostek.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-blue-50 hover:text-blue-700 transition-all duration-200 {{ Request::routeIs('master-group-bp-jamsostek.*') ? 'bg-blue-50 text-blue-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Master Group BP Jamsostek</span>
                                    </a>
                                @endif
                                @if($user && $user->can('master-divisi-view'))
                                    <a href="{{ route('master.divisi.index') }}" target="_blank" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-blue-50 hover:text-blue-700 transition-all duration-200 {{ Request::routeIs('master.divisi.*') ? 'bg-blue-50 text-blue-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Master Divisi</span>
                                    </a>
                                @endif
                                @if($user && $user->can('master-pekerjaan-view'))
                                    <a href="{{ route('master.pekerjaan.index') }}" target="_blank" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-blue-50 hover:text-blue-700 transition-all duration-200 {{ Request::routeIs('master.pekerjaan.*') ? 'bg-blue-50 text-blue-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Master Pekerjaan</span>
                                    </a>
                                @endif
                                @if($user && $user->can('master-pajak-view'))
                                    <a href="{{ route('master.pajak.index') }}" target="_blank" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-blue-50 hover:text-blue-700 transition-all duration-200 {{ Request::routeIs('master.pajak.*') ? 'bg-blue-50 text-blue-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                ...
                                @if($user && $user->can('master-bank-view'))
                                    <a href="{{ route('master-bank-index') }}" target="_blank" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-blue-50 hover:text-blue-700 transition-all duration-200 {{ Request::routeIs('master-bank-*') ? 'bg-blue-50 text-blue-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Master Bank</span>
                                    </a>
                                @endif
                                @if($user && $user->can('master-permission-view'))
                                    <a href="{{ route('master.permission.index') }}" target="_blank" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-blue-50 hover:text-blue-700 transition-all duration-200 {{ Request::routeIs('master.permission.*') ? 'bg-blue-50 text-blue-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                ...
                                @if($hasUserApprovalAccess)
                                    ...
                                    @endphp
                                    <a href="{{ route('admin.user-approval.index') }}" target="_blank" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-blue-50 hover:text-blue-700 transition-all duration-200 {{ Request::routeIs('admin.user-approval.*') ? 'bg-blue-50 text-blue-700 font-medium shadow-sm' : 'text-gray-600' }}">
        ...
        @php
            $isTarifRoute = Request::routeIs('master.master.pricelist-sewa-kontainer.*') || Request::routeIs('master.pricelist-cat.*') || Request::routeIs('uang-jalan-batam.*') || Request::routeIs('master.pricelist-gate-in.*') || Request::routeIs('master.pricelist-ob.*') || Request::routeIs('master.pricelist-ob-antar-gudang.*') || Request::routeIs('pricelist-uang-jalan-batam.*') || Request::routeIs('master.pricelist-uang-jalan.*') || Request::routeIs('master.pricelist-rit.*') || Request::routeIs('master-pelayanan-pelabuhan.*') || Request::routeIs('master.pricelist-biaya-dokumen.*') || Request::routeIs('ongkos-truck.*') || Request::routeIs('master.pricelist-air-tawar.*') || Request::routeIs('pricelist-tkbm.*') || Request::routeIs('master.pricelist-opp-opt.*') || Request::routeIs('master.pricelist-biaya-trucking.*') || Request::routeIs('master.pricelist-lembur.*') || Request::routeIs('master.pricelist-vendor-supir.*') || Request::routeIs('master.pricelist-freight.*') || Request::routeIs('master.pricelist-lolo.*') || Request::routeIs('master.pricelist-biaya-storage.*') || Request::routeIs('master.pricelist-perijinan.*') || Request::routeIs('master.pricelist-meratus.*') || Request::routeIs('master.pricelist-temas.*') || Request::routeIs('master.pricelist-tanto.*') || Request::routeIs('master.pricelist-pelindo.*');
            $hasTarifPermissions = $user && ($user->can('master-pricelist-sewa-kontainer-view') || $user->can('master-pricelist-cat-view') || $user->can('uang-jalan-batam.view') || $user->can('master-pricelist-gate-in-view') || $user->can('master-pricelist-ob-view') || $user->can('master-pricelist-ob-antar-gudang-view') || $user->can('master-pricelist-uang-jalan-batam-view') || $user->can('master-pricelist-uang-jalan-view') || $user->can('master-pricelist-rit-view') || $user->can('master-pelayanan-pelabuhan-view') || $user->can('master-pricelist-biaya-dokumen-view') || $user->can('ongkos-truck-view') || $user->can('master-pricelist-air-tawar-view') || $user->can('master-pricelist-biaya-trucking-view') || $user->can('master-pricelist-tujuan-kontainer-sewa-view') || $user->can('master-pricelist-lembur-view') || $user->can('master-pricelist-vendor-supir-view') || $user->can('master-pricelist-opp-opt-view') || $user->can('master-pricelist-freight-view') || $user->can('master-pricelist-lolo-view') || $user->can('master-pricelist-biaya-storage-view') || $user->can('master-pricelist-perijinan-view') || $user->can('master-pricelist-meratus-view') || $user->can('master-pricelist-temas-view') || $user->can('master-pricelist-tanto-view') || $user->can('master-pricelist-pelindo-view'));
        @endphp
                        ...
                        <div class="mx-2 mb-3">
                            ...
                            <div id="master-tarif-content" class="dropdown-content ml-4 mt-2 space-y-1" @if($isTarifRoute) style="display: block;" @endif>
                                @if($user && $user->can('master-pricelist-sewa-kontainer-view'))
                                    <a href="{{ route('master.master.pricelist-sewa-kontainer.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('master.master.pricelist-sewa-kontainer.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Pricelist Kontainer Sewa</span>
                                    </a>
                                @endif
                                @if($user && $user->can('master-pricelist-cat-view'))
                                    <a href="{{ route('master.pricelist-cat.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('master.pricelist-cat.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                ...
                                @endif
                                @if($user && $user->can('master-pricelist-ob-antar-gudang-view'))
                                    <a href="{{ route('master.pricelist-ob-antar-gudang.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('master.pricelist-ob-antar-gudang.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Pricelist OB Antar Gudang</span>
                                    </a>
                                @endif
                                ...
                                @endif
                                @if($user && $user->can('master-pricelist-uang-jalan-view'))
                                    <a href="{{ route('master-pricelist-uang-jalan.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('master-pricelist-uang-jalan.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Pricelist Uang Jalan</span>
                                    </a>
                                @endif
                                ...
                                @endif
                                @if($user && $user->can('master-pricelist-air-tawar-view'))
                                    <a href="{{ route('master.pricelist-air-tawar.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('master.pricelist-air-tawar.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Pricelist Air Tawar</span>
                                    </a>
                                @endif
                                ...
                                @endif
                                @if($user && $user->can('master-pricelist-buruh-bongkar-view'))
                                    <a href="{{ route('master.pricelist-buruh-bongkar.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('master.pricelist-buruh-bongkar.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Pricelist Buruh Bongkar</span>
                                    </a>
                                @endif
                                 ...
                                 @if($user && $user->can('master-pricelist-opp-opt-view'))
                                     <a href="{{ route('master.pricelist-opp-opt.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('master.pricelist-opp-opt.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                 ...
                                 @endif
                                 @if($user && $user->can('master-pricelist-meratus-view'))
                                     <a href="{{ route('master.pricelist-meratus.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('master.pricelist-meratus.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                         <span class="text-xs">Pricelist Meratus</span>
                                     </a>
                                 @endif
                                 @if($user && $user->can('master-pricelist-temas-view'))
                                     <a href="{{ route('master.pricelist-temas.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('master.pricelist-temas.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                         <span class="text-xs font-bold text-blue-600">Pricelist Temas</span>
                                     </a>
                                 @endif
                                 @if($user && $user->can('master-pricelist-tanto-view'))
                                     <a href="{{ route('master.pricelist-tanto.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('master.pricelist-tanto.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                         <span class="text-xs font-bold text-purple-600">Pricelist Tanto</span>
                                     </a>
                                 @endif
                                 @if($user && $user->can('master-pricelist-lolo-view'))
                                     <a href="{{ route('master.pricelist-lolo.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('master.pricelist-lolo.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                         <span class="text-xs">Pricelist Biaya LOLO</span>
                                     </a>
                                 @endif
                                 @if($user && $user->can('master-pricelist-biaya-storage-view'))
                                     <a href="{{ route('master-pricelist-biaya-storage.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('master-pricelist-biaya-storage.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                         <span class="text-xs">Pricelist Biaya Storage</span>
                                     </a>
                                 @endif
                                ...
                                @if($user && $user->can('master-pricelist-lembur-view'))
                                    <a href="{{ route('master.pricelist-lembur.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('master.pricelist-lembur.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Master Pricelist Lembur</span>
                                    </a>
                                @endif
                                ...
                                @if($user && $user->can('master-kelola-bbm-view'))
                                    <a href="{{ route('kelola-bbm.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('kelola-bbm.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Kelola BBM</span>
                                    </a>
                                @endif
                                @if($user && $user->can('ongkos-truck-view'))
                                    <a href="{{ route('master.ongkos-truck.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('master.ongkos-truck.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Ongkos Truck</span>
                                    </a>
                                @endif
                                @if($user && $user->can('master-pricelist-perijinan-view'))
                                    <a href="{{ route('master.pricelist-perijinan.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('master.pricelist-perijinan.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Pricelist Perijinan</span>
                                    </a>
                                @endif
                                @if($user && $user->can('master-pricelist-pelindo-view'))
                                    <a href="{{ route('master.pricelist-pelindo.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('master.pricelist-pelindo.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Pricelist Pelindo</span>
                                    </a>
                                @endif
                            </div>
                        </div>
                        ...
                        @php
                            $isPemasokRoute = Request::routeIs('master.vendor-bengkel.*') || Request::routeIs('vendor-kontainer-sewa.*') || Request::routeIs('master.vendor-supir.*') || Request::routeIs('master.vendor-asuransi.*') || Request::routeIs('master.vendor-amprahan.*');
                            $hasPemasokPermissions = $user && ($user->can('master-vendor-bengkel.view') || $user->can('vendor-kontainer-sewa-view') || $user->can('master-vendor-supir-view') || $user->can('master-vendor-asuransi-view') || $user->can('master-vendor-amprahan-view'));
                        @endphp
                        ...
                        <div class="mx-2 mb-3">
                            ...
                            <div id="master-pemasok-content" class="dropdown-content ml-4 mt-2 space-y-1" @if($isPemasokRoute) style="display: block;" @endif>
                                @if($user && $user->can('master-vendor-amprahan-view'))
                                    <a href="{{ route('master.vendor-amprahan.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-teal-50 hover:text-teal-700 transition-all duration-200 {{ Request::routeIs('master.vendor-amprahan.*') ? 'bg-teal-50 text-teal-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                        <span class="text-xs">Vendor Amprahan</span>
                                    </a>
                                @endif
...
<div class="mt-4 mb-6">
    ...
    <div id="aktiva-menu-content" class="dropdown-content ml-2 mt-3 space-y-2" @if($isAktivaRoute) style="display: block;" @endif>
        ...
        @php
            $isKontainerRoute = Request::routeIs('master.kontainer.*') || Request::routeIs('master.stock-kontainer.*') || Request::routeIs('master.kontainer.stock-pergudang') || Request::routeIs('history-kontainer.*') || Request::routeIs('gerak-kontainer.*') || Request::routeIs('pergerakan-kapal.*') || Request::routeIs('pergerakan-kontainer.*') || Request::routeIs('master-pelabuhan.*') || Request::routeIs('perbaikan-kontainer.*');
            $hasKontainerPermissions = $user && ($user->can('master-kontainer-view') || $user->can('master-stock-kontainer-view') || $user->can('pergerakan-kontainer-view') || $user->can('perbaikan-kontainer-view'));
        @endphp
        ...
        <div class="mx-2 mb-3">
            ...
            <div id="kontainer-menu-content" class="dropdown-content ml-4 mt-2 space-y-1" @if($isKontainerRoute) style="display: block;" @endif>
                @if($user && $user->can('master-kontainer-view'))
                    <a href="{{ route('master.kontainer.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('kontainer.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        <span class="text-xs">Master Kontainer Sewa</span>
                    </a>
                @endif

                @if($user && $user->can('master-stock-kontainer-view'))
                    <a href="{{ route('master.stock-kontainer.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('master.stock-kontainer.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600' }}">
                ...
                @if($isAdmin || ($user && $user->can('gerak-kontainer-view')))
                    <a href="{{ route('gerak-kontainer.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('gerak-kontainer.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        <span class="text-xs font-bold text-blue-600 italic">Gerak Kontainer (Baru)</span>
                    </a>
                @endif
                ...
                @endif
                @if($user && $user->can('master-pelabuhan-view'))
                    <a href="{{ route('master-pelabuhan.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('master-pelabuhan.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        <span class="text-xs">Master Pelabuhan</span>
                    </a>
                @endif
                @if($user && $user->can('perbaikan-kontainer-view'))
                    <a href="{{ route('perbaikan-kontainer.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('perbaikan-kontainer.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600 font-bold' }}">
        ...
        <div class="mx-2 mb-3">
            <a href="{{ route('master-kapal.index') }}" target="_blank" class="flex items-center py-2 px-3 rounded-lg text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('master-kapal.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600 hover:shadow-sm' }}">
        ...
        @if($isAdmin || ($user && $user->can('master-jadwal-kapal-berlabuh-view')))
        <div class="mx-2 mb-3">
            <a href="{{ route('master-jadwal-kapal-berlabuh.index') }}" target="_blank" class="flex items-center py-2 px-3 rounded-lg text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('master-jadwal-kapal-berlabuh.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600 hover:shadow-sm' }}">
                <span class="text-xs font-medium">Jadwal Kapal Berlabuh</span>
            </a>
        </div>
        @endif

        {{-- Dokumen Kapal Alexindo --}}
        @can('master-dokumen-kapal-alexindo-view')
        <div class="mx-2 mb-3">
        ...
        <div class="mx-2 mb-3">
            <a href="{{ route('biaya-kapal.index') }}" target="_blank" class="flex items-center py-2 px-3 rounded-lg text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('biaya-kapal.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600 hover:shadow-sm' }}">
                <span class="text-xs font-medium">Biaya Kapal</span>
            </a>
        </div>
        @endif
        ...
        <div class="mx-2 mb-3">
            <a href="{{ route('pembayaran-biaya-kapal.index') }}" target="_blank" class="flex items-center py-2 px-3 rounded-lg text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('pembayaran-biaya-kapal.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600 hover:shadow-sm' }}">
                <span class="text-xs font-medium">Bayar Biaya Kapal</span>
            </a>
        </div>
        @endif
        ...
        @if($user && $user->can('biaya-kapal-view'))
        <div class="mx-2 mb-3">
            <a href="{{ route('rekap-biaya-kapal.index') }}" target="_blank" class="flex items-center py-2 px-3 rounded-lg text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('rekap-biaya-kapal.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600 hover:shadow-sm' }}">
                <span class="text-xs font-medium">Rekap Biaya Kapal</span>
            </a>
        </div>
        @endif

        {{-- Rekap Biaya Asset --}}
        @if($user && $user->can('rekap-biaya-asset-view'))
        <div class="mx-2 mb-3">
            <a href="{{ route('rekap-biaya-asset.index') }}" target="_blank" class="flex items-center py-2 px-3 rounded-lg text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('rekap-biaya-asset.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600 hover:shadow-sm' }}">
                <span class="text-xs font-medium">Rekap Biaya Asset</span>
            </a>
        </div>
        @endif
        ...
        <div class="mx-2 mb-3">
            <a href="{{ route('rekap-pemakaian-barang.index') }}" target="_blank" class="flex items-center py-2 px-3 rounded-lg text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('rekap-pemakaian-barang.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600 hover:shadow-sm' }}">
                <span class="text-xs font-medium">Rekap Pemakaian Barang</span>
            </a>
        </div>
        @endif
        ...
        {{-- Master Merk Ban --}}
        @if($isAdmin || ($user && $user->can('stock-ban-view')))
        <div class="mx-2 mb-3">
            <a href="{{ route('master.merk-ban.index') }}" target="_blank" class="flex items-center py-2 px-3 rounded-lg text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('master.merk-ban.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600 hover:shadow-sm' }}">
                <span class="text-xs font-medium">Master Merk Ban</span>
            </a>
        </div>
        @endif
        ...
        @if($isAdmin || ($user && $user->can('stock-ban-view')))
        <div class="mx-2 mb-3">
            <a href="{{ route('master.nama-stock-ban.index') }}" target="_blank" class="flex items-center py-2 px-3 rounded-lg text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('master.nama-stock-ban.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600 hover:shadow-sm' }}">
                <span class="text-xs font-medium">Master Nama Stock Ban</span>
            </a>
        </div>
        @endif
        ...
        @if($isAdmin || ($user && $user->can('stock-ban-view')))
        <div class="mx-2 mb-3">
            <a href="{{ route('master.tipe-stock-ban.index') }}" target="_blank" class="flex items-center py-2 px-3 rounded-lg text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('master.tipe-stock-ban.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600 hover:shadow-sm' }}">
                <span class="text-xs font-medium">Master Tipe Stock Ban</span>
            </a>
        </div>
        @endif
        ...
        {{-- Invoice Kanisir Ban --}}
        <div class="mx-2 mb-3">
            <a href="{{ route('invoice-kanisir-ban.index') }}" target="_blank" class="flex items-center py-2 px-3 rounded-lg text-xs hover:bg-green-50 hover:text-green-700 transition-all duration-200 {{ Request::routeIs('invoice-kanisir-ban.*') ? 'bg-green-50 text-green-700 font-medium shadow-sm' : 'text-gray-600 hover:shadow-sm' }}">
                <span class="text-xs font-medium">Invoice Kanisir Ban</span>
            </a>
        </div>
        @endif
        ...
        @if($user && ($user->can('pranota-stock-view') || $isAdmin))
        <div class="mx-2 mb-3">
            <a href="{{ route('pranota-stock.index') }}" target="_blank" class="flex items-center py-2 px-3 rounded-lg text-xs hover:bg-indigo-50 hover:text-indigo-700 transition-all duration-200 {{ Request::routeIs('pranota-stock.*') ? 'bg-indigo-50 text-indigo-700 font-medium shadow-sm' : 'text-gray-600 hover:shadow-sm' }}">
        ...
        {{-- Bayar Pranota Stock --}}
        @if(Route::has('pembayaran-pranota-stock.index') && $user && ($user->can('pembayaran-pranota-stock-view') || $isAdmin))
        <div class="mx-2 mb-3">
...
@php
    ...
    $hasAktivitasPermissions = $user && (
        ...
        $user->can('uang-jalan-bongkaran-delete') ||
        $user->can('pranota-lembur-view') ||
        ...
        $user->can('pranota-ob-view') ||
        $user->can('pembayaran-ob-view') ||
        $user->can('pembayaran-pranota-ob-view') ||
        $user->can('tanda-terima-view') ||
        $user->can('tanda-terima-batam-view') ||
        ...
        $user->can('kontainer-sewa-final-view') ||
        $user->can('surat-jalan-kontainer-sewa-view')
    );
    ...
    <div id="aktivitas-menu-content" class="dropdown-content ml-2 mt-3 space-y-2" @if($isAktivitasRoute) style="display: block;" @endif>
        ...
        @php
            ...
            $hasAktivitasSupirPermissions = $user && (
                ...
                $user->can('pranota-uang-jalan-view') || $user->can('uang-jalan-view') || $user->can('pembayaran-pranota-uang-jalan-view') || 
                $user->can('pranota-rit-view') || $user->can('pranota-uang-rit-view') || $user->can('pembayaran-pranota-rit-view') || 
                $user->can('pranota-uang-rit-kenek-view') || $user->can('pembayaran-pranota-rit-kenek-view') ||
                $user->can('surat-jalan-view') || $user->can('surat-jalan-create') || $user->can('surat-jalan-update') || $user->can('surat-jalan-delete') || 
                ...
                $user->can('uang-jalan-bongkaran-view') || $user->can('uang-jalan-bongkaran-create') || $user->can('uang-jalan-bongkaran-update') || $user->can('uang-jalan-bongkaran-delete') || 
                $user->can('pranota-lembur-view') || $user->can('tagihan-supir-vendor-view') || $user->can('invoice-tagihan-vendor-view') || $user->can('pranota-invoice-vendor-supir-view') || 
                true
            );
        @endphp
        ...
        <div class="mx-2 mb-3">
            ...
            <div id="aktivitas-supir-menu-content" class="dropdown-content ml-4 mt-2 space-y-1" @if($isAktivitasSupirRoute) style="display: block;" @endif>
                ...
                <div class="mx-1 mb-2">
                    ...
                    <div id="memo-menu-content" class="dropdown-content ml-3 mt-1 space-y-1" @if($isMemoRoute) style="display: block;" @endif>
                        ...
                        @if($user && $user->can('permohonan-memo-view'))
                            <a href="{{ route('permohonan.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-indigo-50 hover:text-indigo-700 transition-all duration-200 {{ Request::routeIs('permohonan.*') ? 'bg-indigo-50 text-indigo-700 font-medium shadow-sm' : 'text-gray-600' }}">
                ...
                {{-- Rit Sub-Dropdown --}}
                @php
                    $isRitRoute = Request::routeIs('pranota-rit.*') || 
                ...
                <div class="mx-1 mb-2">
                    ...
                    <div id="rit-menu-content" class="dropdown-content ml-3 mt-1 space-y-1" @if($isRitRoute) style="display: block;" @endif>
                        ...
                        @if($user && $user->can('pranota-rit-view'))
                            <a href="{{ route('pranota-rit.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('pranota-rit.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                ...
                @if($user && $user->can('pranota-uang-kenek-view'))
                    <a href="{{ route('pranota-uang-kenek.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-blue-50 hover:text-blue-700 transition-all duration-200 {{ Request::routeIs('pranota-uang-kenek.*') ? 'bg-blue-50 text-blue-700 font-medium shadow-sm' : 'text-gray-600' }}">
                ...
                <div class="mx-1 mb-2">
                    ...
                    <div id="surat-jalan-menu-content" class="dropdown-content ml-3 mt-1 space-y-1" @if($isSuratJalanRoute) style="display: block;" @endif>
                        ...
                        @if($user && ($user->can('order-batam-view') || $user->can('order-batam-create') || $user->can('order-batam-update') || $user->can('order-batam-delete')))
                        ...
                        @if($user && ($user->can('surat-jalan-view') || $user->can('surat-jalan-create') || $user->can('surat-jalan-update') || $user->can('surat-jalan-delete')))
                            <a href="{{ route('surat-jalan.index') }}" target="_blank" class="flex items-center py-1 px-3 rounded-md text-xs hover:bg-orange-50 hover:text-orange-700 transition-all duration-200 {{ Request::routeIs('surat-jalan.*') && !Request::routeIs('surat-jalan-batam.*') ? 'bg-orange-50 text-orange-700 font-medium' : 'text-gray-600' }}">
                        ...
                        {{-- Uang Jalan Batam --}}
                        @if($user && ($user->can('uang-jalan-batam-view') || $user->can('uang-jalan-batam-create') || $user->can('uang-jalan-batam-update') || $user->can('uang-jalan-batam-delete')))
                            <a href="{{ route('uang-jalan-batam.index') }}" target="_blank" class="flex items-center py-1 px-3 rounded-md text-xs hover:bg-orange-50 hover:text-orange-700 transition-all duration-200 {{ Request::routeIs('uang-jalan-batam.*') ? 'bg-orange-50 text-orange-700 font-medium' : 'text-gray-600' }}">
                        ...
                        @if($user && $user->can('pranota-uang-jalan-batam-view'))
                            <a href="{{ route('pranota-uang-jalan-batam.index') }}" target="_blank" class="flex items-center py-1 px-3 rounded-md text-xs hover:bg-orange-50 hover:text-orange-700 transition-all duration-200 {{ Request::routeIs('pranota-uang-jalan-batam.*') ? 'bg-orange-50 text-orange-700 font-medium' : 'text-gray-600' }}">
                                
                                <span class="text-xs">Pranota Uang Jalan Batam</span>
                            </a>
                        @endif
                        ...
                        @if(Route::has('pembayaran-pranota-uang-jalan-batam.index') && $user && ($user->can('pembayaran-pranota-uang-jalan-batam-view') || $user->is_admin))
                            <a href="{{ route('pembayaran-pranota-uang-jalan-batam.index') }}" target="_blank" class="flex items-center py-1 px-3 rounded-md text-xs hover:bg-orange-50 hover:text-orange-700 transition-all duration-200 {{ Request::routeIs('pembayaran-pranota-uang-jalan-batam.*') ? 'bg-orange-50 text-orange-700 font-medium' : 'text-gray-600' }}">
                                <span class="text-xs">Bayar Pranota Batam</span>
                            </a>
                        @endif
                ...
                <div class="mx-1 mb-2">
                    ...
                    <div id="surat-jalan-bongkaran-menu-content" class="dropdown-content ml-3 mt-1 space-y-1" @if($isSuratJalanBongkaranRoute) style="display: block;" @endif>
                        ...
                        @if($user && ($user->can('surat-jalan-bongkaran-batam-view') || $user->can('surat-jalan-bongkaran-batam-create')))
                            <a href="{{ route('surat-jalan-bongkaran-batam.dashboard') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-teal-50 hover:text-teal-700 transition-all duration-200 {{ Request::routeIs('surat-jalan-bongkaran-batam.dashboard') ? 'bg-teal-50 text-teal-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        ...
                        @if($user && ($user->can('surat-jalan-bongkaran-batam-view') || $user->can('surat-jalan-bongkaran-batam-create')))
                            <a href="{{ route('surat-jalan-bongkaran-batam.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-teal-50 hover:text-teal-700 transition-all duration-200 {{ Request::routeIs('surat-jalan-bongkaran-batam.index') ? 'bg-teal-50 text-teal-700 font-medium shadow-sm' : 'text-gray-600' }}">
                ...
                {{-- Pranota Lembur --}}
                @if($user && $user->can('pranota-lembur-view'))
                    <a href="{{ route('pranota-lembur.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('pranota-lembur.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        <span class="text-xs">Pranota Lembur</span>
                    </a>
                @endif
                ...
                @if($user && $user->can('tagihan-supir-vendor-view'))
                    <a href="{{ Route::has('tagihan-supir-vendor.index') ? route('tagihan-supir-vendor.index') : '#' }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-indigo-50 hover:text-indigo-700 transition-all duration-200 {{ Request::routeIs('tagihan-supir-vendor.*') ? 'bg-indigo-50 text-indigo-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        <span class="text-xs">Tagihan Supir Vendor</span>
                    </a>
                @endif

                {{-- Invoice Tagihan Vendor --}}
                @if($user && $user->can('invoice-tagihan-vendor-view'))
                    <a href="{{ Route::has('invoice-tagihan-vendor.index') ? route('invoice-tagihan-vendor.index') : '#' }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-orange-50 hover:text-orange-700 transition-all duration-200 {{ Request::routeIs('invoice-tagihan-vendor.*') ? 'bg-orange-50 text-orange-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        <span class="text-xs">Invoice Tagihan Vendor</span>
                    </a>
                @endif

                {{-- Pranota Invoice Vendor Supir --}}
                @if($user && $user->can('pranota-invoice-vendor-supir-view'))
                    <a href="{{ Route::has('pranota-invoice-vendor-supir.index') ? route('pranota-invoice-vendor-supir.index') : '#' }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-rose-50 hover:text-rose-700 transition-all duration-200 {{ Request::routeIs('pranota-invoice-vendor-supir.*') ? 'bg-rose-50 text-rose-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        <span class="text-xs">Pranota Invoice Vendor Supir</span>
                    </a>
                @endif
                ...
                {{-- Pranota Biaya Bensin --}}
                @if($user && ($user->can('pranota-biaya-bensin-view') || $user->is_admin))
                    <a href="{{ route('pranota-biaya-bensin.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-emerald-50 hover:text-emerald-700 transition-all duration-200 {{ Request::routeIs('pranota-biaya-bensin.*') ? 'bg-emerald-50 text-emerald-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        <span class="text-xs">Pranota Biaya Bensin</span>
                    </a>
                @endif
        ...
        @php
            ...
            $hasAktivitasKontainerPermissions = $user && ($user->can('tagihan-kontainer-sewa-index') || $user->can('kontainer-sewa-final-view') || $user->can('pranota-kontainer-sewa-view') || $user->can('surat-jalan-kontainer-sewa-view') || $user->can('tanda-terima-surat-jalan-kontainer-sewa-view') || $user->can('pranota.view') || $user->can('tagihan-cat-view') || $user->can('pranota-cat-view') || $user->can('pembayaran-pranota-cat-view'));
        @endphp
                ...
                <div class="mb-2">
                    ...
                    <div id="sewa-menu-content" class="dropdown-content ml-4 mt-1 space-y-1" @if($isSewaRoute) style="display: block;" @endif>
                        ...
                        @if($user && $user->can('tagihan-kontainer-sewa-index'))
                            <a href="{{ route('daftar-tagihan-kontainer-sewa.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-cyan-50 hover:text-cyan-700 transition-all duration-200 {{ Request::routeIs('daftar-tagihan-kontainer-sewa.*') ? 'bg-cyan-50 text-cyan-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                <span class="text-xs">Tagihan Kontainer Sewa</span>
                            </a>
                        @endif

                        {{-- Tagihan Kontainer Sewa 2 --}}
                        @if($user && $user->can('tagihan-kontainer-sewa-index'))
                        ...
                        @if($user && $user->can('pranota-kontainer-sewa-view'))
                            <a href="{{ route('pranota-kontainer-sewa.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-cyan-50 hover:text-cyan-700 transition-all duration-200 {{ Request::routeIs('pranota-kontainer-sewa.*') ? 'bg-cyan-50 text-cyan-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        ...
                        @if($user && $user->can('approval-view'))
                            <a href="{{ route('approval.dashboard') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-cyan-50 hover:text-cyan-700 transition-all duration-200 {{ Request::routeIs('approval.dashboard') || Request::routeIs('approval.create') || Request::routeIs('approval.riwayat') ? 'bg-cyan-50 text-cyan-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                <span class="text-xs">Approval Tugas 1</span>
                            </a>
                        @endif
                        ...
                        @if($user && $user->can('approval-view'))
                            <a href="{{ route('approval-ii.dashboard') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-cyan-50 hover:text-cyan-700 transition-all duration-200 {{ Request::routeIs('approval-ii.dashboard') || Request::routeIs('approval-ii.create') || Request::routeIs('approval-ii.riwayat') ? 'bg-cyan-50 text-cyan-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                <span class="text-xs">Approval Tugas 2</span>
                            </a>
                        @endif
                    </div>
                ...
                <div class="mb-2">
                    ...
                    <div id="cat-menu-content" class="dropdown-content ml-4 mt-1 space-y-1" @if($isCatRoute) style="display: block;" @endif>
                        ...
                        @if($user && $user->can('tagihan-cat-view'))
                            <a href="{{ route('tagihan-cat.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-rose-50 hover:text-rose-700 transition-all duration-200 {{ Request::routeIs('tagihan-cat.*') ? 'bg-rose-50 text-rose-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        ...
                        @if($user && $user->can('pranota-cat-view'))
                            <a href="{{ route('pranota-cat.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-rose-50 hover:text-rose-700 transition-all duration-200 {{ Request::routeIs('pranota-cat.*') ? 'bg-rose-50 text-rose-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        ...
                        @if(Route::has('pembayaran-pranota-cat.index') && ($isAdmin || auth()->user()->can('pembayaran-pranota-cat-view')))
                            <a href="{{ route('pembayaran-pranota-cat.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-rose-50 hover:text-rose-700 transition-all duration-200 {{ Request::routeIs('pembayaran-pranota-cat.*') ? 'bg-rose-50 text-rose-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        ...
                        @if($user && $user->can('approval-view'))
                            <a href="{{ route('approval.dashboard') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-rose-50 hover:text-rose-700 transition-all duration-200 {{ Request::routeIs('approval.dashboard') || Request::routeIs('approval.create') || Request::routeIs('approval.riwayat') ? 'bg-rose-50 text-rose-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                <span class="text-xs">Approval Tugas 1</span>
                            </a>
                        @endif
                        ...
                        @if($user && $user->can('approval-view'))
                            <a href="{{ route('approval-ii.dashboard') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-rose-50 hover:text-rose-700 transition-all duration-200 {{ Request::routeIs('approval-ii.dashboard') || Request::routeIs('approval-ii.create') || Request::routeIs('approval-ii.riwayat') ? 'bg-rose-50 text-rose-700 font-medium shadow-sm' : 'text-gray-600' }}">
                                <span class="text-xs">Approval Tugas 2</span>
                            </a>
                        @endif
                    </div>
        ...
        <div class="mx-2 mb-3">
            ...
            <div id="aktivitas-kapal-menu-content" class="dropdown-content ml-4 mt-2 space-y-1" @if($isAktivitasKapalRoute) style="display: block;" @endif>
                ...
                {{-- Stowage Plan --}}
                @if($user && $user->can('stowage-plan-view'))
                    <a href="{{ route('stowage-plan.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('stowage-plan.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        <span class="text-xs">Stowage Plan</span>
                    </a>
                @endif
                ...
                @if($user && ($user->can('pranota-ob-antar-gudang-view') || $user->can('pranota-ob-view')))
                    <a href="{{ route('pranota-ob-antar-gudang.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-rose-50 hover:text-rose-700 transition-all duration-200 {{ Request::routeIs('pranota-ob-antar-gudang.*') && !Request::routeIs('pembayaran-pranota-ob-antar-gudang.*') ? 'bg-rose-50 text-rose-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        <span class="text-xs font-medium text-teal-600">Pranota OB Antar Gudang</span>
                    </a>
                @endif

                {{-- Pembayaran Pranota OB Antar Gudang --}}
                @if($user && ($user->can('pranota-ob-antar-gudang-view') || $user->can('pranota-ob-view')))
                    <a href="{{ route('pembayaran-pranota-ob-antar-gudang.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-rose-50 hover:text-rose-700 transition-all duration-200 {{ Request::routeIs('pembayaran-pranota-ob-antar-gudang.*') ? 'bg-rose-50 text-rose-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        <span class="text-xs font-medium text-emerald-600">Pembayaran Pranota OB Antar Gudang</span>
                    </a>
                @endif
                ...
                @if($user && $user->can('bl-view'))
                    <a href="{{ route('bl.select') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('bl.*') && !Request::routeIs('bl.rekap-bongkaran*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                ...
                {{-- Pergerakan Kapal --}}
                @if($user && $user->can('pergerakan-kapal-view'))
                    <a href="{{ route('pergerakan-kapal.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-purple-50 hover:text-purple-700 transition-all duration-200 {{ Request::routeIs('pergerakan-kapal.*') ? 'bg-purple-50 text-purple-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        <span class="text-xs">Pergerakan Kapal</span>
                    </a>
                @endif
        ...
        <div class="mx-2 mb-3">
            ...
            <div id="aktivitas-pelabuhan-menu-content" class="dropdown-content ml-4 mt-2 space-y-1" @if($isAktivitasPelabuhanRoute) style="display: block;" @endif>
                ...
                @if($user && ($user->can('tanda-terima-view') || $user->can('tanda-terima-update') || $user->can('tanda-terima-delete')))
                    <a href="{{ route('tanda-terima.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-emerald-50 hover:text-emerald-700 transition-all duration-200 {{ Request::routeIs('tanda-terima.*') && !Request::routeIs('tanda-terima-batam.*') ? 'bg-emerald-50 text-emerald-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        <span class="text-xs">Tanda Terima</span>
                    </a>
                @endif

                {{-- Tanda Terima Batam --}}
                @if($user && ($user->can('tanda-terima-batam-view') || $isAdmin))
                    <a href="{{ route('tanda-terima-batam.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-emerald-50 hover:text-emerald-700 transition-all duration-200 {{ Request::routeIs('tanda-terima-batam.*') ? 'bg-emerald-50 text-emerald-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        <span class="text-xs">Tanda Terima Batam</span>
                    </a>
                @endif
                ...
                @if($user && ($user->can('prospek-batam-view') || $user->can('prospek-batam-edit') || $isAdmin))
                    <a href="{{ route('prospek-batam.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-emerald-50 hover:text-emerald-700 transition-all duration-200 {{ Request::routeIs('prospek-batam.*') ? 'bg-emerald-50 text-emerald-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        <span class="text-xs">Prospek Batam</span>
                    </a>
                @endif
                ...
                @if($user && $user->can('dokumen-tanda-terima-view'))
                ...
                @if($user && ($user->can('gate-in-view') || $user->can('gate-in-create') || $user->can('gate-in-update') || $user->can('gate-in-delete')))
                    <a href="{{ route('gate-in.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-emerald-50 hover:text-emerald-700 transition-all duration-200 {{ Request::routeIs('gate-in.*') ? 'bg-emerald-50 text-emerald-700 font-medium shadow-sm' : 'text-gray-600' }}">
                ...
                @if($user && ($user->can('checkpoint-kontainer-keluar-view') || $user->can('checkpoint-kontainer-keluar-create') || $user->can('checkpoint-kontainer-keluar-delete')))
                    <a href="{{ route('kontainer-perjalanan.index') }}" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-blue-50 hover:text-blue-700 transition-all duration-200 {{ Request::routeIs('kontainer-perjalanan.*') ? 'bg-blue-50 text-blue-700 font-medium shadow-sm' : 'text-gray-600' }}">
        ...
        <div class="mx-2 mb-3">
            ...
            <div id="approval-tugas-menu-content" class="dropdown-content ml-4 mt-2 space-y-1" @if($isApprovalTugasRoute) style="display: block;" @endif>
                ...
                @if(Route::has('approval.dashboard'))
                    <a href="{{ route('approval.dashboard') }}" target="_blank" class="flex items-center py-1 px-3 rounded-md text-xs hover:bg-gray-50 hover:text-gray-900 transition-all duration-200 {{ Request::routeIs('approval.dashboard') ? 'bg-orange-50 text-orange-700 font-medium' : 'text-gray-600' }}">
                ...
                @if(Route::has('approval-ii.dashboard'))
                    <a href="{{ route('approval-ii.dashboard') }}" target="_blank" class="flex items-center py-1 px-3 rounded-md text-xs hover:bg-gray-50 hover:text-gray-900 transition-all duration-200 {{ Request::routeIs('approval-ii.dashboard') ? 'bg-orange-50 text-orange-700 font-medium' : 'text-gray-600' }}">
                        <svg class="w-2.5 h-2.5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                        Approval Tugas II
        ...
        @php
        $isAktivitasLainRoute = Request::routeIs('pembayaran-aktivitas-lain.*') || Request::routeIs('invoice-aktivitas-lain.*') || Request::routeIs('pembayaran-uang-muka.*') || Request::routeIs('realisasi-uang-muka.*');
        $hasAktivitasLainPermissions = $isAdmin || ($user && ($user->can('pembayaran-aktivitas-lain-view') || $user->can('pembayaran-aktivitas-lain-create') || $user->can('pembayaran-aktivitas-lain-update') || $user->can('pembayaran-aktivitas-lain-delete') || $user->can('invoice-aktivitas-lain-view') || $user->can('invoice-aktivitas-lain-create') || $user->can('invoice-aktivitas-lain-update') || $user->can('invoice-aktivitas-lain-delete') || $user->can('pembayaran-uang-muka-view') || $user->can('realisasi-uang-muka-view') || $user->can('realisasi-uang-muka-create') || $user->can('realisasi-uang-muka-update') || $user->can('realisasi-uang-muka-delete')));
        @endphp
        ...
        <div class="mx-2 mb-3">
            ...
            <div id="aktivitas-lain-menu-content" class="dropdown-content ml-4 mt-2 space-y-1" @if($isAktivitasLainRoute) style="display: block;" @endif>
                ...
                @if($isAdmin || ($user && $user->can('pembayaran-uang-muka-view')))
                    <a href="{{ route('pembayaran-uang-muka.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-pink-50 hover:text-pink-700 transition-all duration-200 {{ Request::routeIs('pembayaran-uang-muka.*') ? 'bg-pink-50 text-pink-700 font-medium shadow-sm' : 'text-gray-600' }}">
                        <span class="text-xs">Pembayaran Uang Muka</span>
                    </a>
                @endif
                
                {{-- Realisasi Uang Muka --}}
                @if($isAdmin || ($user && $user->can('realisasi-uang-muka-view')))
                    <a href="{{ route('realisasi-uang-muka.index') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-pink-50 hover:text-pink-700 transition-all duration-200 {{ Request::routeIs('realisasi-uang-muka.*') ? 'bg-pink-50 text-pink-700 font-medium shadow-sm' : 'text-gray-600' }}">
...
<div class="mt-4 mb-4">
    ...
    <div id="report-menu-content" class="dropdown-content ml-8 space-y-2 mt-2" @if($isReportRoute) style="display: block;" @endif>
        {{-- Report History Cuti --}}
        <a href="{{ route('report-history-cuti.select-karyawan') }}" class="flex items-center py-1 px-3 rounded-md text-xs hover:bg-gray-50 hover:text-gray-900 transition-all duration-200 {{ Request::routeIs('report-history-cuti.*') ? 'bg-purple-50 text-purple-700 font-medium' : 'text-gray-600' }}">
        ...
        @if($user && ($isAdmin || $user->can('gaji-supir-batam-view')))
        <a href="{{ route('report-gaji-supir-batam.index') }}" class="flex items-center py-1 px-3 rounded-md text-xs hover:bg-gray-50 hover:text-gray-900 transition-all duration-200 {{ Request::routeIs('report-gaji-supir-batam.*') ? 'bg-purple-50 text-purple-700 font-medium' : 'text-gray-600' }}">
            <svg class="w-3 h-3 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
            </svg>
            Report Gaji Supir Batam
        ...
        <a href="{{ route('report.lembur.index') }}" class="flex items-center py-1 px-3 rounded-md text-xs hover:bg-gray-50 hover:text-gray-900 transition-all duration-200 {{ Request::routeIs('report.lembur.*') ? 'bg-purple-50 text-purple-700 font-medium' : 'text-gray-600' }}">
            <svg class="w-3 h-3 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {{-- Report Surat Jalan --}}
        <a href="{{ route('report.surat_jalan.index') }}" class="flex items-center py-1 px-3 rounded-md text-xs hover:bg-gray-50 hover:text-gray-900 transition-all duration-200 {{ Request::routeIs('report.surat_jalan.*') ? 'bg-purple-50 text-purple-700 font-medium' : 'text-gray-600' }}">
            <svg class="w-3 h-3 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
            </svg>
            Report Surat Jalan
        ...
        {{-- Report Pranota OB --}}
        <a href="{{ route('report.pranota-ob.index') }}" class="flex items-center py-1 px-3 rounded-md text-xs hover:bg-gray-50 hover:text-gray-900 transition-all duration-200 {{ Request::routeIs('report.pranota-ob.*') ? 'bg-purple-50 text-purple-700 font-medium' : 'text-gray-600' }}">
            <svg class="w-3 h-3 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
            </svg>
            Report Pranota OB
        ...
        {{-- Perincian --}}
        @if($user && ($isAdmin || $user->can('perincian-view')))
        <a href="{{ route('report.perincians.index') }}" class="flex items-center py-1 px-3 rounded-md text-xs hover:bg-gray-50 hover:text-gray-900 transition-all duration-200 {{ Request::routeIs('report.perincians.*') ? 'bg-purple-50 text-purple-700 font-medium' : 'text-gray-600' }}">
            <svg class="w-3 h-3 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
            </svg>
            Perincian
...
<div class="mt-4">
    <a href="{{ route('audit-logs.index') }}" target="_blank" class="w-full flex items-center py-2 px-5 rounded-lg text-gray-700 hover:bg-gray-50 hover:text-gray-900 transition-all duration-200 group text-xs {{ Request::routeIs('audit-logs.*') ? 'bg-orange-50 text-orange-700 font-medium' : '' }}">
...
@endif
    ...
    <script>
        ...
        function deleteItem(event, form) {
            ...
            if (submitBtn) {
                ...
                submitBtn.innerHTML = '<svg class="animate-spin h-4 w-4 mx-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>';
            }
