...
{{-- OB Section --}}
@php
    $isObRoute = Request::routeIs('ob.*');
    $hasObPermissions = $user && ($user->can('ob-view') || $user->can('tagihan-ob-view') || $user->can('pranota-ob-view'));
@endphp

@if($hasObPermissions)
<!-- OB Menu -->
<a href="{{ route('ob.index') }}" class="flex items-center py-2 px-5 rounded-xl mb-4 transition-all duration-200 group shadow-sm text-xs {{ $isObRoute ? 'bg-orange-100 text-orange-700 font-bold' : 'text-gray-700 hover:bg-orange-100 hover:text-orange-700' }}">
    <svg class="w-4 h-4 mr-2 {{ $isObRoute ? 'text-orange-700' : 'text-gray-500 group-hover:text-orange-700' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
    </svg>
    <span class="text-xs font-medium menu-text">OB</span>
</a>
@endif
...
<div class="mt-4 mb-6">
    ...
    <div id="aktivitas-menu-content" class="dropdown-content ml-2 mt-3 space-y-2" @if($isAktivitasRoute) style="display: block;" @endif>
                ...
                <div class="mx-1 mb-2">
                    ...
                    <div id="surat-jalan-bongkaran-menu-content" class="dropdown-content ml-3 mt-1 space-y-1" @if($isSuratJalanBongkaranRoute) style="display: block;" @endif>
                        ...
                        @if($user && $user->can('surat-jalan-bongkaran-view'))
                            <a href="{{ route('surat-jalan-bongkaran.select-ship') }}" target="_blank" class="flex items-center py-1.5 px-3 mx-1 rounded-md text-xs hover:bg-teal-50 hover:text-teal-700 transition-all duration-200 {{ Request::routeIs('surat-jalan-bongkaran.*') ? 'bg-teal-50 text-teal-700 font-medium shadow-sm' : 'text-gray-600' }}">
        ...
        @php
            ...
            $hasAktivitasKapalPermissions = $user && ($user->can('aktivitas-kapal-view') || $user->can('pergerakan-kapal-view') || $user->can('voyage-view') || $user->can('jadwal-kapal-view') || $user->can('status-kapal-view') || $user->can('log-aktivitas-kapal-view') || $user->can('monitoring-kapal-view') || $user->can('prospek-edit') || $user->can('prospek-view') || $user->can('bl-view') || $user->can('tagihan-ob-view') || $user->can('pranota-ob-view'));
        @endphp
        ...
        @php
            $isAktivitasLainRoute = Request::routeIs('pembayaran-aktivitas-lain.*') || Request::routeIs('realisasi-uang-muka.*');
            $hasAktivitasLainPermissions = $isAdmin || ($user && ($user->can('pembayaran-aktivitas-lain-view') || $user->can('pembayaran-aktivitas-lain-create') || $user->can('pembayaran-aktivitas-lain-update') || $user->can('pembayaran-aktivitas-lain-delete') || $user->can('realisasi-uang-muka-view') || $user->can('realisasi-uang-muka-create') || $user->can('realisasi-uang-muka-update') || $user->can('realisasi-uang-muka-delete')));
        @endphp
