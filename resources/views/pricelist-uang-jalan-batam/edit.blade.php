...
@section('title', 'Edit Pricelist Uang Jalan Batam')
@section('page_title', 'Edit Pricelist Uang Jalan Batam')

@section('content')
<div class="mb-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Edit Pricelist</h2>
            <p class="mt-1 text-sm text-gray-600">Update data pricelist uang jalan Batam</p>
        </div>
...
<div class="bg-white shadow-md rounded-lg p-6">
    <form action="{{ route('pricelist-uang-jalan-batam.update', $pricelistUangJalanBatam) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" 
                       name="expedisi" 
                       id="expedisi" 
                       value="{{ old('expedisi', $pricelistUangJalanBatam->expedisi) }}"
            ...
            <div>
                ...
                <input type="text" 
                       name="ring" 
                       id="ring" 
                       value="{{ old('ring', $pricelistUangJalanBatam->ring) }}"
            ...
            <div>
                ...
                <select name="size" 
                    ...
                    <option value="20FT" {{ old('size', $pricelistUangJalanBatam->size) == '20FT' ? 'selected' : '' }}>20FT</option>
                    <option value="40FT" {{ old('size', $pricelistUangJalanBatam->size) == '40FT' ? 'selected' : '' }}>40FT</option>
                    <option value="45FT" {{ old('size', $pricelistUangJalanBatam->size) == '45FT' ? 'selected' : '' }}>45FT</option>
                </select>
                @error('size')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" 
                       ...
                       value="{{ old('tarif_antar_lokasi', $pricelistUangJalanBatam->tarif_antar_lokasi) }}"
            ...
            <div>
                ...
                <select name="status" 
                    ...
                    <option value="AQUA" {{ old('status', $pricelistUangJalanBatam->status) == 'AQUA' ? 'selected' : '' }}>AQUA</option>
                    <option value="CHASIS PB" {{ old('status', $pricelistUangJalanBatam->status) == 'CHASIS PB' ? 'selected' : '' }}>CHASIS PB</option>
                </select>
                @error('status')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
