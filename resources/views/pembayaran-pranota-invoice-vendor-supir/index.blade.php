        ...
        <form action="{{ route('pembayaran-pranota-invoice-vendor-supir.index') }}" method="GET" class="flex flex-col md:flex-row gap-4">
            <div class="flex-1">
                <div class="relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" class="block w-full pl-10 pr-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent" placeholder="Cari nomor pembayaran, nomor accurate atau vendor...">
                </div>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                <thead class="bg-gray-50 text-gray-600 font-medium border-b border-gray-100">
                    <tr>
                        <th class="px-4 py-4 whitespace-nowrap">Pembayaran & Bank</th>
                        <th class="px-4 py-4 whitespace-nowrap">Tanggal</th>
                        <th class="px-4 py-4 whitespace-nowrap">Vendor</th>
                        <th class="px-4 py-4 text-right whitespace-nowrap">Total Tagihan</th>
                        <th class="px-4 py-4 text-right whitespace-nowrap">Penyesuaian</th>
                        <th class="px-4 py-4 text-right whitespace-nowrap">Total Bayar</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        <td class="px-4 py-4">
                            <div class="font-medium text-emerald-600">{{ $pembayaran->nomor_pembayaran }}</div>
                            @if($pembayaran->nomor_accurate)
                                <div class="text-xs text-gray-500 mt-0.5">Acc: {{ $pembayaran->nomor_accurate }}</div>
                            @endif
                        ...
                        <td class="px-4 py-4">
                            <div class="font-medium text-gray-900">{{ $pembayaran->vendor->nama_vendor ?? '-' }}</div>
                            @if($pembayaran->keterangan)
                                <div class="text-xs text-gray-500 truncate mt-1 max-w-[200px]" title="{{ $pembayaran->keterangan }}">{{ $pembayaran->keterangan }}</div>
                            @endif
                        </td>
                        <td class="px-4 py-4 text-right text-gray-600 whitespace-nowrap">
                            Rp {{ number_format($pembayaran->total_pembayaran, 0, ',', '.') }}
                        </td>
                        <td class="px-4 py-4 text-right whitespace-nowrap">
                            @if($pembayaran->total_tagihan_penyesuaian != 0)
                                <div class="{{ $pembayaran->total_tagihan_penyesuaian < 0 ? 'text-rose-600' : 'text-emerald-600' }} font-medium">
                                    {{ $pembayaran->total_tagihan_penyesuaian < 0 ? '-' : '+' }} Rp {{ number_format(abs($pembayaran->total_tagihan_penyesuaian), 0, ',', '.') }}
                                </div>
                                @if($pembayaran->alasan_penyesuaian)
                                    <div class="text-[10px] text-gray-500 mt-0.5" title="{{ $pembayaran->alasan_penyesuaian }}">{{ \Illuminate\Support\Str::limit($pembayaran->alasan_penyesuaian, 20) }}</div>
                                @endif
                            @else
                        ...
                        <td class="px-4 py-4 text-center space-x-2 whitespace-nowrap w-24">
