...
@section('page_title', 'Edit Pekerjaan')
    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            <div class="flex items-center">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Edit Pekerjaan</h1>
                    <p class="mt-1 text-sm text-gray-600">Ubah informasi pekerjaan dalam sistem</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <form action="{{ route('master.pekerjaan.update', $pekerjaan->id) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text"
                           name="kode_pekerjaan"
                           id="kode_pekerjaan"
                           value="{{ old('kode_pekerjaan', $pekerjaan->kode_pekerjaan) }}"
                           required
                           maxlength="20"
                           class="w-full px-3 py-2 border-2 border-gray-400 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200"
                ...
                <div>
                    ...
                    <textarea name="deskripsi"
                              ...
                              maxlength="500"
                              class="w-full px-3 py-2 border-2 border-gray-400 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200"
                              placeholder="Masukkan deskripsi divisi (opsional)">{{ old('deskripsi', $pekerjaan->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="divisi"
                        ...
                        @foreach($divisis as $divisi)
                            <option value="{{ $divisi->nama_divisi }}" {{ old('divisi', $pekerjaan->divisi) === $divisi->nama_divisi ? 'selected' : '' }}>
                    ...
                    <div class="flex items-center space-x-4">
                        <label class="flex items-center">
                            <input type="checkbox"
                                   name="is_active"
                                   value="1"
                                   {{ old('is_active', $pekerjaan->is_active) ? 'checked' : '' }}
