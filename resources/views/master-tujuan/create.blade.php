    ...
    <form action="{{route('master.tujuan.store')}}" method="POST">
            ...
            <div>
                ...
                <select name="cabang" id="cabang" class="mt-1 block w-full bg-white rounded-md border border-gray-200 p-2.5 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    ...
                    @foreach($cabangs as $cabang)
                        <option value="{{ $cabang->nama_cabang }}" {{ old('cabang') == $cabang->nama_cabang ? 'selected' : '' }}>
                            {{ $cabang->nama_cabang }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="mb-4">
            ...
            <div class="mt-1 flex rounded-md shadow-sm">
                ...
                <input type="text" id="uang_jalan_formatted" value="{{ old('uang_jalan') !== null ? number_format(old('uang_jalan'), 0, ',', '.') : '' }}" class="flex-1 block w-full bg-gray-100 rounded-r-md border border-gray-200 p-2.5 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="0">
            </div>
            <!-- Hidden numeric input submitted -->
            <input type="hidden" name="uang_jalan" id="uang_jalan" value="{{ old('uang_jalan') ?? 0 }}">
        </div>
