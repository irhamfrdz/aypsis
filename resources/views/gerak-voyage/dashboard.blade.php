    ...
    <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
        <div class="overflow-hidden rounded-2xl bg-gradient-to-r from-slate-900 via-blue-900 to-indigo-800 p-6 text-white shadow-lg sm:p-8">
            <div class="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p class="mt-2 max-w-2xl text-sm text-blue-100">Pantau urutan tanggal gerak dari voyage terbaru masing-masing kapal berdasarkan data manifest.</p>
                </div>
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-5 shadow-sm">
                <p class="text-sm font-medium text-emerald-700">Jadwal Sudah Diisi</p>
            ...
            <div class="rounded-xl border border-amber-200 bg-amber-50 p-5 shadow-sm">
                <p class="text-sm font-medium text-amber-700">Jadwal Belum Diisi</p>
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
                ...
                <div>
                    ...
                    <select id="status" name="status" class="w-full rounded-lg border-slate-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <option value="terisi" @selected(($filters['status'] ?? '') === 'terisi')>Sudah diisi</option>
                        <option value="belum_terisi" @selected(($filters['status'] ?? '') === 'belum_terisi')>Belum diisi</option>
                    </select>
                </div>
        ...
        <div class="flex flex-wrap items-end justify-between gap-2">
            ...
            <span class="text-sm text-slate-500">{{ number_format($voyages->total(), 0, ',', '.') }} kapal ditampilkan</span>
        </div>

        <div class="grid grid-cols-1 gap-5 xl:grid-cols-2">
            @forelse($voyages as $voyage)
                @php
                    $filledCount = collect($tanggalFields)->filter(fn ($field) => ! empty($voyage->{$field}))->count();
                @endphp
                <article class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <div class="border-b border-slate-100 px-5 py-5 sm:px-6">
                        <div class="flex flex-wrap items-start justify-between gap-3">
                            ...
                            @if($filledCount)
                                <span class="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-800">{{ $filledCount }} dari 6 tanggal terisi</span>
                            @else
                                <span class="rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-800">Jadwal belum diisi</span>
                            @endif
                        </div>
                        <div class="mt-4 h-1.5 overflow-hidden rounded-full bg-slate-100" role="progressbar" aria-valuenow="{{ $filledCount }}" aria-valuemin="0" aria-valuemax="6" aria-label="Tanggal gerak terisi">
                            <div class="h-full rounded-full bg-blue-600" style="width: {{ $filledCount / 6 * 100 }}%"></div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3 p-5 sm:grid-cols-3 sm:p-6">
                        @foreach($tanggalLabels as $field => $label)
                            <div class="rounded-xl border {{ $voyage->{$field} ? 'border-blue-100 bg-blue-50' : 'border-slate-200 bg-slate-50' }} p-3">
                                ...
                                <p class="mt-3 text-sm font-bold {{ $voyage->{$field} ? 'text-slate-900' : 'text-slate-400' }}">
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 bg-slate-50 px-5 py-3 sm:px-6">
                        ...
                        <a href="{{ route('gerak-voyage.create', ['nama_kapal' => $voyage->nama_kapal, 'no_voyage' => $voyage->no_voyage]) }}" class="text-sm font-semibold text-blue-700 hover:text-blue-900 hover:underline">Lihat / Atur Tanggal <i class="fas fa-arrow-right ml-1"></i></a>
                    </div>
                </article>
            @empty
        ...
        @if($voyages->hasPages())
            <div class="rounded-xl border border-slate-200 bg-white px-5 py-4">{{ $voyages->links() }}</div>
        @endif
    </div>
</div>
@endsection
