...
<div class="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 py-6 px-4 sm:px-6 lg:px-8">
    <div class="max-w-4xl mx-auto">
        <div class="text-center mb-8 lg:text-left">
            ...
            <p class="text-gray-600">Perbarui data tagihan kontainer sewa</p>
        </div>
        ...
        <div class="bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-100">
            <form action="{{ route('daftar-tagihan-kontainer-sewa.update', $item->id ?? 0) }}" method="POST" class="divide-y divide-gray-100">
                ...
                @php
                    ...
                    $currencyClasses = "mt-1 block w-full rounded-xl border-gray-300 bg-gray-50 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 text-base p-3 lg:p-4 transition-all duration-200 min-h-[48px] text-right font-mono";
                @endphp
                ...
                <div class="p-6 lg:p-8 space-y-6">
                    <div class="flex items-center mb-6">
                        ...
                        <h3 class="text-lg lg:text-xl font-bold text-gray-800">Informasi Dasar</h3>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="size" value="{{ old('size', $item->size ?? '') }}" class="{{ $inputClasses }}" placeholder="20ft, 40ft, dll" />
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal_harga_awal" value="{{ old('tanggal_harga_awal', isset($item->tanggal_harga_awal) ? (is_string($item->tanggal_harga_awal) ? $item->tanggal_harga_awal : $item->tanggal_harga_awal->format('Y-m-d')) : '') }}" class="{{ $inputClasses }}" />
                        </div>
                        <div>
                            <label class="{{ $labelClasses }}">Tanggal Selesai Sewa</label>
                            <input type="date" name="tanggal_harga_akhir" value="{{ old('tanggal_harga_akhir', isset($item->tanggal_harga_akhir) ? (is_string($item->tanggal_harga_akhir) ? $item->tanggal_harga_akhir : $item->tanggal_harga_akhir->format('Y-m-d')) : '') }}" class="{{ $inputClasses }}" />
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 lg:p-8 space-y-6">
                    <div class="flex items-center mb-6">
                        ...
                        <h3 class="text-lg lg:text-xl font-bold text-gray-800">Informasi Keuangan</h3>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
                        <div>
                            ...
                            <div class="relative">
                                <span class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500 font-medium z-10">Rp</span>
                                <input type="text"
                                       name="dpp"
                                       value="{{ old('dpp', number_format($item->dpp ?? 0, 2, ',', '.')) }}"
                                       class="{{ $currencyClasses }} pl-12 pr-16"
                                       placeholder="0,00"
                                       data-currency />
                            </div>
                        ...
                        <div class="lg:col-span-2">
                            ...
                            <div class="relative">
                                <span class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500 font-bold z-10">Rp</span>
                                <input type="text"
                                       name="grand_total"
                                       value="{{ old('grand_total', number_format($item->grand_total ?? 0, 2, ',', '.')) }}"
                                       class="{{ $currencyClasses }} pl-12 pr-16 bg-yellow-50 border-yellow-200 font-bold text-lg"
                                       placeholder="0,00"
                                       data-currency />
                            </div>
                            <div class="mt-1 text-xs text-gray-400 text-right">
                ...
                <div class="bg-gray-50 px-6 py-6 lg:px-8 lg:py-8">
                    <div class="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-4">
                        ...
                        <button type="submit"
                                class="w-full sm:w-auto inline-flex justify-center items-center rounded-xl border-2 border-transparent bg-gradient-to-r from-green-600 to-emerald-600 py-3 px-6 text-base font-semibold text-white shadow-lg hover:from-green-700 hover:to-emerald-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-all duration-200 transform hover:scale-105 min-h-[48px]">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
