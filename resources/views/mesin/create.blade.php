...
@section('page_title', 'Tambah Mesin')
    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            <div class="flex items-center">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Tambah Mesin Baru</h1>
                    <p class="mt-1 text-sm text-gray-600">Daftarkan data mesin/engine baru dalam sistem</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200">
                ...
                <p class="mt-1 text-sm text-gray-600">Lengkapi detail informasi mesin</p>
            </div>

            <form action="{{ route('master.mesin.store') }}" method="POST" class="p-6 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input type="text"
                           ...
                           placeholder="Masukkan kode mesin (misal: MSN-01)"
                ...
                <div>
                    ...
                    <input type="text"
                           ...
                           required
                           class="w-full px-3 py-2 border-2 border-gray-400 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200"
                           placeholder="Masukkan tipe mesin (misal: Caterpillar 3512, Mitsubishi S6R)">
                    @error('tipe_mesin')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <div>
                        ...
                        <input type="number" name="port" id="port" value="{{ old('port', 4370) }}"
                               required
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200 text-xs">
                        @error('port')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="number" name="comm_key" id="comm_key" value="{{ old('comm_key', 0) }}"
                               required
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200 text-xs">
                        @error('comm_key')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <textarea name="keterangan"
                              ...
                              rows="4"
                              class="w-full px-3 py-2 border-2 border-gray-400 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200"
                              placeholder="Masukkan catatan atau keterangan tambahan (opsional)">{{ old('keterangan') }}</textarea>
                    @error('keterangan')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
