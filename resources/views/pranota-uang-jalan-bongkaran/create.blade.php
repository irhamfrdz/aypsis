...
<div class="container mx-auto px-3 py-2">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-4">
            <div>
                <h1 class="text-lg font-semibold text-gray-900">Buat Pranota Uang Jalan Bongkaran</h1>
                <p class="text-xs text-gray-600 mt-0.5">Pilih uang jalan bongkaran yang akan dimasukkan ke pranota</p>
            </div>
            <a href="{{ route('pranota-uang-jalan-bongkaran.index') }}" 
        ...
        <form action="{{ route('pranota-uang-jalan-bongkaran.store') }}" method="POST" id="pranotaForm">
            @csrf
            ...
            <!-- Available Uang Jalan Bongkaran -->
            <div class="bg-white rounded border border-gray-200 p-4 mb-4">
                <div class="flex justify-between items-center mb-3">
                    <h3 class="text-sm font-medium text-gray-900">Pilih Uang Jalan Bongkaran</h3>
                ...
                @if($availableUangJalanBongkarans->count() > 0)
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($availableUangJalanBongkarans as $uangJalan)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-3 py-2">
                                            <input type="checkbox" 
                                                   name="uang_jalan_bongkaran_ids[]" 
                                                   value="{{ $uangJalan->id }}" 
                                                   data-amount="{{ $uangJalan->jumlah_total }}"
                                                   onchange="updateTotal()"
                                                   class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded uang-jalan-bongkaran-checkbox"
                                                   {{ in_array($uangJalan->id, old('uang_jalan_bongkaran_ids', [])) ? 'checked' : '' }}>
                                        </td>
                                        <td class="px-3 py-2">
                                        ...
                                        <td class="px-3 py-2">
                                            @if($uangJalan->suratJalanBongkaran)
                                                <div class="text-sm text-gray-900">{{ $uangJalan->suratJalanBongkaran->no_surat_jalan }}</div>
                                                <div class="text-xs text-gray-500">{{ $uangJalan->suratJalanBongkaran->kegiatan }}</div>
                                            @else
                                        ...
                                        <td class="px-3 py-2">
                                            @if($uangJalan->suratJalanBongkaran)
                                                <div class="text-xs text-gray-900">
                                                    <div>
                                                        <span class="font-medium">Supir:</span> {{ $uangJalan->suratJalanBongkaran->supir ?? '-' }}
                                                        @if($uangJalan->suratJalanBongkaran->supir_nik)
                                                            <span class="text-gray-500">({{ $uangJalan->suratJalanBongkaran->supir_nik }})</span>
                                                        @endif
                                                    </div>
                                                    <div>
                                                        <span class="font-medium">Kenek:</span> {{ $uangJalan->suratJalanBongkaran->kenek ?? '-' }}
                                                        @if($uangJalan->suratJalanBongkaran->kenek_nik)
                                                            <span class="text-gray-500">({{ $uangJalan->suratJalanBongkaran->kenek_nik }})</span>
                                                        @endif
                                                    </div>
                                                </div>
                                            @else
                    ...
                    <div class="mt-4 p-3 bg-gray-50 rounded">
                        <div class="flex justify-between items-center">
                            <div class="text-sm text-gray-600">
                                <span id="selectedCount">0</span> uang jalan bongkaran dipilih
                            </div>
                            <div class="text-sm font-semibold text-gray-900">
                ...
                @else
                    <div class="text-center py-8">
                        ...
                        <p class="text-gray-500 text-sm">Tidak ada uang jalan bongkaran yang tersedia</p>
                        <p class="text-xs text-gray-400 mt-1">Semua uang jalan bongkaran sudah masuk pranota atau belum ada yang dibuat</p>
                        <a href="{{ route('uang-jalan-bongkaran.select-surat-jalan-bongkaran') }}" 
                           class="mt-2 inline-block text-indigo-600 hover:text-indigo-500 text-sm">
                            Buat uang jalan bongkaran baru
                        </a>
                    </div>
                @endif
            </div>
...
window.updateTotal = function() {
    ...
    const allCheckboxes = document.querySelectorAll('.uang-jalan-bongkaran-checkbox');
