        ...
        section.innerHTML = `
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="text" class="admin-display-meratus w-full px-3 py-2 border border-blue-300 rounded-lg focus:ring-2 focus:ring-blue-500" value="${data ? 'Rp ' + parseInt(data.biaya_admin || 0).toLocaleString('id-ID') : 'Rp 0'}">
                    <input type="hidden" name="meratus[${sectionIndex}][biaya_admin]" class="admin-value-meratus" value="${data ? (data.biaya_admin || 0) : 0}">
                </div>
                <div>
    ...
    function loadContainerInfoForSection(sectionIndex, kapalNama, voyage) {
        ...
        .then(data => {
            if (data.success && data.counts) {
                ...
                section.querySelectorAll('.container-select-meratus').forEach(select => {
                    const currentVal = select.value;
                    select.innerHTML = getContainerOptionsForSection(sectionIndex, currentVal);
                    updateMeratusContainerSize(select);
                });
