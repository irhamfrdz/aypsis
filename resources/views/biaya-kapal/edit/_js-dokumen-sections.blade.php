...
function addDokumenSection(existingData = null) {
    ...
    let initialVendor = existingData ? existingData.vendor_id : '';
    let initialBl = existingData ? existingData.nomor_bl : '';
    ...
    if (typeof pricelistBiayaDokumenData !== 'undefined') {
        pricelistBiayaDokumenData.forEach(v => {
            const selected = (initialVendor == v.id) ? 'selected' : '';
            vendorOptions += `<option value="${v.id}" data-biaya="${v.biaya}" ${selected}>${v.nama_vendor} - Rp ${parseInt(v.biaya).toLocaleString('id-ID')}</option>`;
        });
    }
    ...
    // If initialVoyage is set but we haven't manually triggered change, we might need to fetch BLs.
    // In edit view, initial loads will need BLs.
    if (initialVoyage && !isManualVoyage) {
