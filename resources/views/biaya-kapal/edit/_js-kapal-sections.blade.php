            ...
            section.innerHTML = `
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="kapal_sections[${sectionIndex}][tanggal_bayar]" class="tanggal-bayar-input w-full px-3 py-2 border border-gray-300 rounded-lg bg-white focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
    ...
    function calculateTotalFromAllSections() {
        ...
        document.querySelectorAll('.kapal-section').forEach(section => {
            ...
            // PPh is calculated from the section total and deducted from the payable amount.
            const pphPercentSelect = section.querySelector('.pph-percent-select');
    ...
    window.addBarangToSectionWithValue = function(sectionIndex, barangId, jumlah) {
        ...
        const section = document.querySelector(`[data-section-index="${sectionIndex}"]`);
        const container = section.querySelector('.barang-container-section');
        const groups = container.querySelectorAll('.flex');
        const lastGroup = groups[groups.length - 1];
        
        if (lastGroup) {
            const select = lastGroup.querySelector('.barang-select-item');
            const input = lastGroup.querySelector('.jumlah-input-item');
    ...
    window.addBuruhToSectionWithValue = function(sectionIndex, buruhId, nominal) {
        addBuruhToSection(sectionIndex);
        const section = document.querySelector(`[data-section-index="${sectionIndex}"]`);
        const container = section.querySelector('.buruh-container-section');
        const groups = container.querySelectorAll('.flex');
        const lastGroup = groups[groups.length - 1];
        ...
        if (lastGroup) {
            const select = lastGroup.querySelector('.buruh-select-item');
            const input = lastGroup.querySelector('.buruh-nominal-item');
            if (select) select.value = buruhId;
            if (input) input.value = Math.round(nominal).toLocaleString('id-ID');
            if (typeof jQuery !== 'undefined' && $.fn.select2) {
                $(select).trigger('change');
            }
        }
    ...
    window.addKontainerToSectionWithValue = function(sectionIndex, blId, nominal, nomorKontainer, size) {
        addKontainerToSection(sectionIndex);
        const section = document.querySelector(`[data-section-index="${sectionIndex}"]`);
        const container = section.querySelector('.kontainer-container-section');
        const groups = container.querySelectorAll('.flex');
        const lastGroup = groups[groups.length - 1];
        ...
        if (lastGroup) {
            const select = lastGroup.querySelector('.kontainer-select-item');
            const inputNominal = lastGroup.querySelector('.kontainer-nominal-item');
            const hiddenNomor = lastGroup.querySelector('.kontainer-nomor-hidden');
            const hiddenSize = lastGroup.querySelector('.kontainer-size-hidden');
            
            if (select) select.value = blId;
            if (inputNominal) inputNominal.value = Math.round(nominal).toLocaleString('id-ID');
            if (hiddenNomor) hiddenNomor.value = nomorKontainer;
