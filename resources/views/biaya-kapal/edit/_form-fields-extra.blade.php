                ...
                <!-- Bank Selection (for Biaya Buruh) -->
                <div id="bank_id_wrapper" class="hidden">
                    <label for="bank_id" class="block text-sm font-medium text-gray-700 mb-2">
                        Pilih Bank
                    </label>
                    <select id="bank_id" 
                            name="bank_id" 
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('bank_id') border-red-500 @enderror">
                        <option value="">-- Pilih Bank --</option>
                        @foreach($banks as $bank)
                            <option value="{{ $bank->id }}" {{ old('bank_id', $biayaKapal->bank_id) == $bank->id ? 'selected' : '' }}>
                                {{ $bank->name }}
                            </option>
                        @endforeach
                    </select>
