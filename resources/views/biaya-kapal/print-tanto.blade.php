...
@php
    ...
    $vendorDisplay = $biayaKapal->nama_vendor ?? ($tantoDetails->pluck('vendor')->filter()->unique()->values()->first() ?? 'TANTO');
...
<head>
    ...
    <title>Invoice Biaya Tanto - {{ $biayaKapal->nomor_invoice }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="container">
        ...
        @php
            $penerimaDisplay = $biayaKapal->penerima ?? ($tantoDetails->pluck('penerima')->filter()->unique()->values()->first() ?? '-');
            $rekeningDisplay = $biayaKapal->nomor_rekening ?? ($tantoDetails->pluck('nomor_rekening')->filter()->unique()->values()->first() ?? '-');
            ...
            foreach($tantoDetails as $detail) {
                ...
                $typeKey = ($detail->jenis_biaya ?? 'BIAYA TANTO') . $status;
                
                if (!isset($tantoByType[$typeKey])) {
                    $tantoByType[$typeKey] = [
                        'qty' => 0, 
                        'cost' => 0
                    ];
                ...
                $tantoByType[$typeKey]['qty'] += $detail->kuantitas;
                $tantoByType[$typeKey]['cost'] += $detail->sub_total;
        ...
        <table class="custom-table">
            ...
            <tbody>
                @php
                    $perSection = $tantoDetails->groupBy(function($item) {
        ...
        <table class="custom-table">
            ...
            <tbody>
                ...
                @foreach($tantoByType as $typeName => $data)
