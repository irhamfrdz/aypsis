...
<head>
    ...
    <title>Biaya LOLO - {{ $biayaKapal->nomor_invoice }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="container">
        ...
        <table class="detail-table">
            ...
            <tbody>
                ...
                @if($biayaKapal->loloDetails && $biayaKapal->loloDetails->count() > 0)
                    @foreach($biayaKapal->loloDetails as $index => $detail)
                    <tr>
                        ...
                        <td>
                            <div class="rincian-item">
                                <span>Subtotal LOLO:</span>
                                <span>{{ number_format($detail->subtotal, 0, ',', '.') }}</span>
                            </div>
                            @if($detail->ppn > 0)
                            <div class="rincian-item">
                                <span>PPN 11%:</span>
                                <span>{{ number_format($detail->ppn, 0, ',', '.') }}</span>
                            </div>
                            @endif
                            ...
                            <div class="rincian-item">
                                ...
                                <span>{{ $detail->adjustment < 0 ? '-' : '+' }} {{ number_format(abs($detail->adjustment), 0, ',', '.') }}</span>
                            </div>
                            @if($detail->notes_adjustment)
                            <div style="font-size: 0.8em; color: #555; text-align: left; margin-left:10px;">
                                Ket: {{ $detail->notes_adjustment }}
                            </div>
                            @endif
                            @endif
                        </td>
                        <td class="right">
                            <div style="font-weight: bold;">Rp {{ number_format(round($detail->total_biaya + $detail->pph, -3), 0, ',', '.') }}</div>
                        </td>
                    </tr>
                    @php 
                        ...
                        $grandTotalFinal += round($detail->total_biaya + $detail->pph, -3);
                    @endphp
                    @endforeach
                @else
                    <tr>
                        <td colspan="5" class="center">Tidak ada detail LOLO.</td>
                    </tr>
                @endif
            </tbody>
        </table>
