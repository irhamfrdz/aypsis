            ...
            <table class="table" style="margin-top: 4px; margin-bottom: 0;">
                <thead>
                    <tr>
                        ...
                        <th style="width: 25%;">Detail Kontainer</th>
                        ...
                        @foreach($biayaKapal->buruhBongkarDetails as $detail)
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @else
                                        Manual Input: Rp {{ number_format($detail->nominal, 0, ',', '.') }}
                                    @endif
                                </td>
                                <td class="text-right">{{ $detail->adjustment != 0 ? ($detail->adjustment > 0 ? '+' : '') . number_format($detail->adjustment, 0, ',', '.') : '-' }}</td>
                                <td class="text-right">
                                    @if($detail->pph_amount > 0)
                                        {{ (float)$detail->pph_percent }}%<br>
                                        (-{{ number_format($detail->pph_amount, 0, ',', '.') }})
                                    @else
                                        -
                                    @endif
                                </td>
                                <td class="text-right">Rp {{ number_format($detail->total_nominal, 0, ',', '.') }}</td>
                            </tr>
                        @endforeach
                    @endif
