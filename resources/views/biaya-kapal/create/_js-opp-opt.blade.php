    ...
    // ============= FETCH BY DATE RANGE =============
    const fetchOppOptBtn = document.getElementById('fetch_opp_opt_by_date_btn');
    if (fetchOppOptBtn) {
        fetchOppOptBtn.addEventListener('click', function() {
            const startDate = document.getElementById('opp_opt_start_date').value;
            const endDate = document.getElementById('opp_opt_end_date').value;
            ...
            const originalText = fetchOppOptBtn.innerHTML;
            fetchOppOptBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Loading...';
            fetchOppOptBtn.disabled = true;
            
            fetch(`/naik-kapal/get-kapal-voyage-by-date?start_date=${startDate}&end_date=${endDate}`)
                .then(response => response.json())
                .then(result => {
                    if (result.success && result.data && result.data.length > 0) {
                        clearAllOppOptSections();
                        result.data.forEach(item => {
                            addOppOptSection();
                            // Select kapal and voyage
                            const currentSection = oppOptSectionsContainer.lastElementChild;
                            const kapalSelect = currentSection.querySelector('.opp-opt-kapal-select');
                            
                            // Check if option exists in select
                            let optionExists = false;
                            for (let i = 0; i < kapalSelect.options.length; i++) {
                                if (kapalSelect.options[i].value === item.nama_kapal) {
                                    optionExists = true;
                                    break;
                                }
                            }
                            ...
                            if (!optionExists) {
                                ...
                                kapalSelect.add(new Option(item.nama_kapal, item.nama_kapal));
                            }
                            
                            kapalSelect.value = item.nama_kapal;
                            
                            // Set voyage directly via manual input to avoid extra AJAX call
                            const voyageSelect = currentSection.querySelector('.opp-opt-voyage-select');
                            const voyageInput = currentSection.querySelector('.opp-opt-voyage-input');
                            ...
                            voyageInput.classList.remove('hidden');
                            voyageInput.disabled = false;
                            voyageInput.value = item.no_voyage;
                        });
                    } else {
                        alert('Tidak ada Kapal & Voyage yang ditemukan pada rentang tanggal tersebut.');
                    }
                })
                .catch(error => {
                    console.error('Error fetching kapal & voyage:', error);
                    alert('Terjadi kesalahan saat mengambil data.');
                })
                .finally(() => {
                    fetchOppOptBtn.innerHTML = originalText;
                    fetchOppOptBtn.disabled = false;
                });
        });
    }
