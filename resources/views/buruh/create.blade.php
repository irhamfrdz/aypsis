...
@section('title', 'Tambah Buruh')
@section('page_title', 'Tambah Buruh Baru')
...
<div class="min-h-screen bg-gray-50 py-12">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            ...
            <h1 class="mt-2 text-3xl font-extrabold text-gray-900 tracking-tight">Tambah Buruh Baru</h1>
            <p class="mt-2 text-sm text-gray-600">Masukkan detail informasi tenaga buruh untuk didaftarkan ke sistem.</p>
        </div>

        <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
            <form action="{{ route('master.buruh.store') }}" method="POST" class="p-8">
                @csrf
                    ...
                    <div>
                        ...
                        <div class="flex gap-4">
                            <label class="relative flex items-center p-3 rounded-xl border border-gray-200 cursor-pointer hover:bg-gray-50 transition-colors w-1/2">
                                <input type="radio" name="status" value="aktif" {{ old('status', 'aktif') == 'aktif' ? 'checked' : '' }} class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300">
                            ...
                            <label class="relative flex items-center p-3 rounded-xl border border-gray-200 cursor-pointer hover:bg-gray-50 transition-colors w-1/2">
                                <input type="radio" name="status" value="non-aktif" {{ old('status') == 'non-aktif' ? 'checked' : '' }} class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300">
                                <span class="ml-3 flex flex-col">
                ...
                <div class="mt-10 pt-6 border-t border-gray-100 flex items-center justify-end gap-3">
                    ...
                    <button type="submit" class="px-8 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 focus:ring-4 focus:ring-indigo-200 shadow-lg shadow-indigo-200 transition-all duration-200">
                        Simpan Data
                    </button>
                </div>
            </form>
