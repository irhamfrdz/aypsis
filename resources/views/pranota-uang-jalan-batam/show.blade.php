...
<div class="container mx-auto px-3 py-2">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-4">
            <div>
                <h1 class="text-lg font-semibold text-gray-900">Detail Pranota Uang Jalan Batam Batam</h1>
                <p class="text-xs text-gray-600 mt-0.5">{{ $pranotaUangJalanBatam->nomor_pranota }}</p>
            </div>
            <div class="flex gap-2">
                <a href="{{ route('pranota-uang-jalan-batam.print', $pranotaUangJalanBatam) }}" 
                ...
                @if(in_array($pranotaUangJalanBatam->status_pembayaran, ['unpaid', 'approved', 'paid']))
                    <form action="{{ route('pranota-uang-jalan-batam.update-total', $pranotaUangJalanBatam) }}" method="POST">
                        @csrf
                        <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded text-sm flex items-center" onclick="return confirm('Apakah Anda yakin ingin memperbarui total pranota ini? Total akan dihitung ulang berdasarkan Uang Jalan Batam yang tersisa.')">
                            <svg class="h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @endif
                @if(in_array($pranotaUangJalanBatam->status_pembayaran, ['unpaid', 'approved']))
                    <a href="{{ route('pranota-uang-jalan-batam.edit', $pranotaUangJalanBatam) }}" 
        ...
        <div class="bg-white rounded border border-gray-200 p-4 mb-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <div class="text-sm font-semibold text-gray-900 mt-1">{{ $pranotaUangJalanBatam->nomor_pranota }}</div>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-500">Tanggal Pranota</label>
                    <div class="text-sm text-gray-900 mt-1">{{ $pranotaUangJalanBatam->tanggal_pranota->format('d/m/Y') }}</div>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-500">Periode Tagihan</label>
                    <div class="text-sm text-gray-900 mt-1">{{ $pranotaUangJalanBatam->periode_tagihan }}</div>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-500">Jumlah Uang Jalan Batam</label>
                    <div class="text-sm text-gray-900 mt-1">{{ $pranotaUangJalanBatam->jumlah_uang_jalan }} item</div>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-500">Subtotal Uang Jalan Batam</label>
                    <div class="text-sm text-gray-900 mt-1">{{ $pranotaUangJalanBatam->formatted_total }}</div>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-500">Penyesuaian</label>
                    <div class="text-sm text-gray-900 mt-1">{{ $pranotaUangJalanBatam->formatted_penyesuaian }}</div>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-500">Total Akhir</label>
                    <div class="text-lg font-bold text-gray-900 mt-1">{{ $pranotaUangJalanBatam->formatted_total_with_penyesuaian }}</div>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-500">Status Pembayaran</label>
                    <div class="mt-1">
                        <span class="inline-flex items-center px-2 py-1 rounded text-xs font-medium {{ $pranotaUangJalanBatam->status_badge }}">
                            {{ $pranotaUangJalanBatam->status_text }}
                        </span>
                    </div>
                </div>
                @if($pranotaUangJalanBatam->catatan)
                <div class="md:col-span-3">
                    <label class="block text-xs font-medium text-gray-500">Catatan</label>
                    <div class="text-sm text-gray-900 mt-1">{{ $pranotaUangJalanBatam->catatan }}</div>
                </div>
                @endif
                @if($pranotaUangJalanBatam->keterangan_penyesuaian)
                <div class="md:col-span-3">
                    <label class="block text-xs font-medium text-gray-500">Keterangan Penyesuaian</label>
                    <div class="text-sm text-gray-900 mt-1">{{ $pranotaUangJalanBatam->keterangan_penyesuaian }}</div>
                </div>
                @endif
                <div class="md:col-span-3 text-xs text-gray-500 border-t border-gray-200 pt-2">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <span class="font-medium">Dibuat oleh:</span> {{ $pranotaUangJalanBatam->creator->name ?? 'N/A' }}
                            <br><span class="font-medium">Pada:</span> {{ $pranotaUangJalanBatam->created_at->format('d/m/Y H:i') }}
                        </div>
                        @if($pranotaUangJalanBatam->updated_at != $pranotaUangJalanBatam->created_at)
                        <div>
                            <span class="font-medium">Diperbarui oleh:</span> {{ $pranotaUangJalanBatam->updater->name ?? 'N/A' }}
                            <br><span class="font-medium">Pada:</span> {{ $pranotaUangJalanBatam->updated_at->format('d/m/Y H:i') }}
                        </div>
                        @endif
                    </div>
                </div>
        ...
        <!-- Uang Jalan Batam List -->
        <div class="bg-white rounded border border-gray-200 p-4">
            <h3 class="text-sm font-medium text-gray-900 mb-3 border-b border-gray-200 pb-2">Daftar Uang Jalan Batam</h3>
                    ...
                    <div class="flex items-start">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="text-xs text-blue-700 grid grid-cols-1 md:grid-cols-3 gap-2">
                                ...
                                <div><strong>Status:</strong> {{ $pranotaUangJalanBatam->status_text }}</div>
                            </div>
                        </div>
                    </div>
                </div>
            @else
                <div class="text-center py-8">
                    ...
                    <p class="text-gray-500 text-sm">Tidak ada Uang Jalan Batam dalam pranota ini</p>
                </div>
            @endif
        </div>
        ...
        @if($pranotaUangJalanBatam->status_pembayaran === 'unpaid')
            <div class="flex justify-end gap-2 mt-4">
                <form action="{{ route('pranota-uang-jalan-batam.destroy', $pranotaUangJalanBatam) }}" 
                      method="POST" 
                      onsubmit="return confirm('Yakin ingin menghapus pranota ini? Semua Uang Jalan Batam akan dikembalikan ke status belum dibayar.')">
                    @csrf
                    @method('DELETE')
                    <button type="submit" 
