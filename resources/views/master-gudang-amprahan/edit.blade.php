...
@section('title', 'Edit Gudang Amprahan')
@section('page_title', 'Edit Gudang Amprahan')
    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow-sm overflow-hidden">
            <div class="p-6 border-b border-gray-200">
                <h2 class="text-xl font-bold text-gray-800">Edit Gudang Amprahan</h2>
                <p class="text-sm text-gray-600 mt-1">Perbarui informasi gudang amprahan</p>
            </div>

            <form action="{{ route('master.gudang-amprahan.update', $gudangAmprahan->id) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" 
                               name="nama_gudang" 
                               id="nama_gudang" 
                               value="{{ old('nama_gudang', $gudangAmprahan->nama_gudang) }}" 
                    ...
                    <div>
                        ...
                        <textarea name="keterangan" 
                                  ...
                                  placeholder="Deskripsi atau catatan tambahan tentang gudang...">{{ old('keterangan', $gudangAmprahan->keterangan) }}</textarea>
                        @error('keterangan')
                            <p class="mt-2 text-xs text-red-500">
                                <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
                            </p>
                    ...
                    <div>
                        ...
                        <select name="status" 
                                ...
                                required>
                            <option value="active" {{ old('status', $gudangAmprahan->status) == 'active' ? 'selected' : '' }}>Aktif</option>
                            <option value="inactive" {{ old('status', $gudangAmprahan->status) == 'inactive' ? 'selected' : '' }}>Nonaktif</option>
                        </select>
                        @error('status')
                            <p class="mt-2 text-xs text-red-500">
                                <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
