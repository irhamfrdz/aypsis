...
@section('page_title', 'Daftar Uang Jalan Batam')
...
<div class="min-h-screen bg-gray-50 py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
            <div class="px-6 py-4 border-b border-gray-200">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900">Daftar Uang Jalan Batam</h1>
                        <p class="text-gray-600 mt-1">Kelola data uang jalan Batam yang telah diinput</p>
                    </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
            <div class="px-6 py-4">
                <form method="GET" action="{{ route('uang-jalan-batam.index') }}" id="filterForm">
        ...
        <!-- Tabel Uang Jalan Batam -->
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            @if($uangJalans->count() > 0)
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 text-xs table-fixed resizable-table" id="uangJalanBatamTable">
                        <colgroup>
                            <col class="w-12">   <!-- No -->
                            ...
                            @foreach($uangJalans as $index => $uangJalan)
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td class="px-2 py-1 whitespace-nowrap text-xs font-medium text-gray-900 w-32">
                                        {{ $uangJalan->suratJalanBatam->no_surat_jalan ?? '-'}}
                                    </td>
                                    ...
                                    <td class="px-2 py-1 whitespace-nowrap text-xs text-gray-900 w-24">
                                        {{ Str::limit($uangJalan->suratJalanBatam->supir ?? '-', 15) }}
                                    </td>
                                    ...
                                    <td class="px-1 py-1 whitespace-nowrap text-center w-20">
                                        <div class="flex justify-center space-x-0.5">
                                            @can('uang-jalan-batam-view')
                                            <a href="{{ route('uang-jalan-batam.show', $uangJalan->id) }}" 
                                            ...
                                            @can('uang-jalan-batam-delete')
                                            @if(in_array($uangJalan->status, ['belum_dibayar', 'belum_masuk_pranota']))
                                            <button type="button" 
                                                    class="inline-flex items-center p-1.5 text-red-600 hover:text-red-800 hover:bg-red-50 rounded transition-colors"
                                                    title="Hapus"
                                                    onclick="confirmDelete('{{ $uangJalan->id }}', '{{ $uangJalan->nomor_uang_jalan ?? $uangJalan->suratJalanBatam->no_surat_jalan ?? '' }}')">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <h3 class="text-lg font-medium text-gray-900 mb-2">Belum ada data uang jalan Batam</h3>
                    <p class="text-gray-500 mb-6 max-w-md mx-auto">
                        Belum ada uang jalan Batam yang diinput atau tidak ada yang memenuhi kriteria filter yang Anda tentukan.
                    </p>
                    @can('uang-jalan-batam-create')
                    <a href="{{ route('uang-jalan-batam.select-surat-jalan') }}" 
                       class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors duration-200 shadow-sm">
...
function confirmDelete(id, identifier) {
    ...
    document.getElementById('deleteForm').action = '{{ route('uang-jalan-batam.destroy', ':id', false) }}'.replace(':id', id);
