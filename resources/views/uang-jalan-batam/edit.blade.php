...
<div class="container mx-auto px-3 py-2">
    <div class="max-w-4xl mx-auto bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 p-3 border-b border-gray-200">
            <div>
                <h1 class="text-lg font-semibold text-gray-900">Edit Uang Jalan Batam</h1>
                <p class="text-xs text-gray-600 mt-0.5">Perbarui data pembayaran uang jalan Batam</p>
            </div>
        ...
        <!-- Surat Jalan Info (Read Only) -->
        @php $suratJalan = $uangJalan->suratJalanBatam; @endphp
        <div class="mx-3 mt-4">
                ...
                <div class="flex items-start">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
                            ...
                            <div class="space-y-1">
                                <span class="text-blue-600 font-medium uppercase tracking-wider">Tanggal SJ</span>
                                <div class="text-gray-900 font-bold text-sm">{{ $suratJalan->tanggal_surat_jalan ? $suratJalan->tanggal_surat_jalan->format('d/m/Y') : '-' }}</div>
                            </div>
                            <div class="space-y-1">
                                <span class="text-blue-600 font-medium uppercase tracking-wider">Supir / No Plat</span>
                                <div class="text-gray-900 font-bold text-sm">{{ $suratJalan->supir ?? '-' }} / {{ $suratJalan->no_plat ?? '-' }}</div>
                            </div>
                            <div class="space-y-1">
                                <span class="text-blue-600 font-medium uppercase tracking-wider">Order</span>
                                <div class="text-gray-900 font-bold text-sm">{{ $suratJalan->orderBatam->nomor_order ?? '-' }}</div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <!-- Form -->
        <form action="{{ route('uang-jalan-batam.update', $uangJalan->id) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 p-4 bg-gray-50 rounded-lg border border-gray-200">
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="nomor_uang_jalan" value="{{ old('nomor_uang_jalan', $uangJalan->nomor_uang_jalan) }}" readonly
                               class="w-full px-3 py-2 bg-gray-100 border border-gray-300 rounded-md text-sm font-bold text-gray-600">
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-gray-700 mb-1">Tanggal Pembayaran <span class="text-red-500">*</span></label>
                        <input type="date" name="tanggal_uang_jalan" value="{{ old('tanggal_uang_jalan', $uangJalan->tanggal_uang_jalan ? $uangJalan->tanggal_uang_jalan->format('Y-m-d') : date('Y-m-d')) }}" required
                    ...
                    <div>
                        ...
                        <select name="status" required class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-blue-500 focus:border-blue-500">
                            @foreach(\App\Models\UangJalanBatam::getStatusOptions() as $key => $label)
                                <option value="{{ $key }}" {{ old('status', $uangJalan->status) == $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="memo" rows="4" placeholder="Tambahkan catatan jika diperlukan..."
                                  class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-blue-500 focus:border-blue-500">{{ old('memo', $uangJalan->memo) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="jumlah_uang_jalan" id="jumlah_uang_jalan" 
                                   value="{{ old('jumlah_uang_jalan', intval($uangJalan->jumlah_uang_jalan)) }}" required
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <span class="absolute left-3 top-2 text-gray-400 text-sm">Rp</span>
                            <input type="number" name="jumlah_mel" id="jumlah_mel" value="{{ old('jumlah_mel', intval($uangJalan->jumlah_mel)) }}"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <span class="absolute left-3 top-2 text-gray-400 text-sm">Rp</span>
                            <input type="number" name="jumlah_pelancar" id="jumlah_pelancar" value="{{ old('jumlah_pelancar', intval($uangJalan->jumlah_pelancar)) }}"
                                   class="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-blue-500 focus:border-blue-500"
                                   oninput="calculateTotal()">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <span class="absolute left-3 top-2 text-gray-400 text-sm">Rp</span>
                            <input type="number" name="jumlah_kawalan" id="jumlah_kawalan" value="{{ old('jumlah_kawalan', intval($uangJalan->jumlah_kawalan)) }}"
                                   class="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-blue-500 focus:border-blue-500"
                                   oninput="calculateTotal()">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <span class="absolute left-3 top-2 text-gray-400 text-sm">Rp</span>
                            <input type="number" name="jumlah_parkir" id="jumlah_parkir" value="{{ old('jumlah_parkir', intval($uangJalan->jumlah_parkir)) }}"
                                   class="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-blue-500 focus:border-blue-500"
                                   oninput="calculateTotal()">
                        </div>
                    </div>
                ...
                <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="alasan_penyesuaian" value="{{ old('alasan_penyesuaian', $uangJalan->alasan_penyesuaian) }}"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-blue-500 focus:border-blue-500">
                        </div>
                        <div>
                            ...
                            <div class="relative">
                                <span class="absolute left-3 top-2 text-gray-400 text-sm">Rp</span>
                                <input type="number" name="jumlah_penyesuaian" id="jumlah_penyesuaian" value="{{ old('jumlah_penyesuaian', intval($uangJalan->jumlah_penyesuaian)) }}"
                                       class="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-blue-500 focus:border-blue-500"
                                       oninput="calculateTotal()">
                            </div>
                        </div>
