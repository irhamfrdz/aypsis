...
@section('title', 'Edit Customer Buruh')
@section('page_title', 'Edit Customer Buruh Bongkar Muat')

@section('content')
<div class="bg-white shadow-md rounded-lg p-6 font-sans">
    <div class="flex justify-between items-center mb-6 border-b pb-4">
        <h2 class="text-xl font-bold text-gray-800">Form Edit Customer Buruh: {{ $masterCustomerBuruh->kode }}</h2>
    ...
    <form action="{{ route('master-customer-buruh.update', $masterCustomerBuruh->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <input type="text" name="nama_customer" id="nama_customer" value="{{ old('nama_customer', $masterCustomerBuruh->nama_customer) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
            </div>
            ...
            <div>
                ...
                <div class="mt-2">
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-offset-0 focus:ring-indigo-200 focus:ring-opacity-50" {{ old('is_active', $masterCustomerBuruh->is_active) ? 'checked' : '' }}>
