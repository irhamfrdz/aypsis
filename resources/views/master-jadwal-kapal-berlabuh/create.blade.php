...
@section('title', 'Tambah Jadwal Kapal Berlabuh')
@section('page_title', 'Tambah Jadwal Kapal Berlabuh')
...
<div class="container mx-auto px-4 py-4 max-w-4xl">
    ...
    <div class="mb-6 flex items-center justify-between">
        <div class="flex items-center gap-2">
            ...
            <div>
                <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Tambah Jadwal Kapal Berlabuh</h1>
                <p class="text-xs sm:text-sm text-gray-500">Input jadwal baru kedatangan dan keberangkatan kapal</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <form action="{{ route('master-jadwal-kapal-berlabuh.store') }}" method="POST" class="p-6">
            @csrf
            ...
            <div class="border-b border-gray-100 pb-6 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <div class="space-y-2">
                            <select id="master_pelabuhan_id" name="master_pelabuhan_id"
                                ...
                                @foreach($masterPelabuhans as $p)
                                    <option value="{{ $p->id }}" data-nama="{{ $p->nama_pelabuhan }}" {{ old('master_pelabuhan_id') == $p->id ? 'selected' : '' }}>
                                        {{ $p->nama_pelabuhan }} ({{ $p->kota }})
                                    </option>
                                @endforeach
                            </select>
                            <input type="text" id="pelabuhan" name="pelabuhan" value="{{ old('pelabuhan', request('pelabuhan')) }}"
                        ...
                        <p class="text-[11px] text-gray-400 mt-1">Nama pelabuhan / rute akan dijadikan judul grup jadwal (misal: TANJUNG PINANG)</p>
                    </div>
                        ...
                        <div class="space-y-2">
                            <select id="master_kapal_id" name="master_kapal_id"
                                ...
                                @foreach($masterKapals as $k)
                                    <option value="{{ $k->id }}" data-nama="{{ $k->nama_kapal }}" {{ old('master_kapal_id') == $k->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="text" id="no_voyage" name="no_voyage" value="{{ old('no_voyage') }}"
                    ...
                    <div>
                        ...
                        <select id="status" name="status" required
                                class="w-full text-xs border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            <option value="aktif" {{ old('status', 'aktif') == 'aktif' ? 'selected' : '' }}>Aktif / Berjalan</option>
            ...
            <div class="border-b border-gray-100 pb-6 mb-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                    ...
                    <div class="bg-sky-50/60 p-4 rounded-xl border border-sky-100">
                        ...
                        <input type="date" id="tanggal_closing" name="tanggal_closing" value="{{ old('tanggal_closing') }}"
                    ...
                    <div class="bg-amber-50 p-4 rounded-xl border border-amber-200">
                        ...
                        <input type="date" id="tanggal_etd" name="tanggal_etd" value="{{ old('tanggal_etd') }}"
                    ...
                    <div class="bg-rose-50 p-4 rounded-xl border border-rose-200">
                        ...
                        <input type="date" id="tanggal_eta" name="tanggal_eta" value="{{ old('tanggal_eta') }}"
            ...
            <div class="mb-6">
                ...
                <textarea id="keterangan" name="keterangan" rows="3" placeholder="Tambahkan catatan khusus terkait jadwal ini..."
                          class="w-full text-xs border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('keterangan') }}</textarea>
            </div>
