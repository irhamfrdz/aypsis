...
<div class="space-y-4 max-w-2xl mx-auto">
    <div class="flex justify-between items-center">
        <div>
            <h2 class="text-xl font-bold text-gray-800">Edit Invoice Tagihan Vendor</h2>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <form action="{{ route('invoice-tagihan-vendor.update', $invoice->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="p-4 bg-gray-50 border border-gray-200 rounded-lg">
                    <h3 class="text-sm font-semibold text-gray-800 mb-3 pb-2 border-b border-gray-200 border-dashed">Detail Rekod Invoice #{{ $invoice->no_invoice }}</h3>
                    <div class="grid grid-cols-2 gap-4 text-sm mt-3">
                        <div>
                            <span class="text-gray-500 block mb-0.5">Nama Vendor</span>
                            <span class="font-medium text-gray-900">{{ $invoice->vendor->nama_vendor ?? '-' }}</span>
                        </div>
                        <div>
                            <span class="text-gray-500 block mb-0.5">Tanggal Invoice</span>
                            <span class="font-medium text-gray-900">{{ optional($invoice->tanggal_invoice)->format('d F Y') }}</span>
                        </div>
                        <div>
                            <span class="text-gray-500 block mb-0.5">Jumlah Tagihan</span>
                            <span class="font-medium text-gray-900">{{ $invoice->tagihanSupirVendors->count() }} item tagihan</span>
                        </div>
                        <div>
                            <span class="text-gray-500 block mb-0.5">Total Nominal</span>
                            <span class="font-bold text-blue-800">Rp {{ number_format($invoice->total_nominal, 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-5">
                    <div class="form-group">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                            <!-- Belum Dibayar -->
                            <div>
                                <input type="radio" name="status_pembayaran" id="status_belum_dibayar" value="belum_dibayar" class="peer hidden" {{ old('status_pembayaran', $invoice->status_pembayaran) == 'belum_dibayar' ? 'checked' : '' }} required>
                            ...
                            <!-- Sebagian -->
                            <div>
                                <input type="radio" name="status_pembayaran" id="status_sebagian" value="sebagian" class="peer hidden" {{ old('status_pembayaran', $invoice->status_pembayaran) == 'sebagian' ? 'checked' : '' }} required>
                            ...
                            <!-- Lunas -->
                            <div>
                                <input type="radio" name="status_pembayaran" id="status_lunas" value="lunas" class="peer hidden" {{ old('status_pembayaran', $invoice->status_pembayaran) == 'lunas' ? 'checked' : '' }} required>
                        ...
                        <p class="mt-2 text-xs text-gray-500 italic">Mengubah status ini juga akan mengubah status semua tagihan supir yang ada di dalamnya.</p>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="keterangan" id="keterangan" rows="3" 
                            ...
                            placeholder="Catatan tambahan invoice...">{{ old('keterangan', $invoice->keterangan) }}</textarea>
                        @error('keterangan')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-end space-x-3">
                <a href="{{ route('invoice-tagihan-vendor.index') }}" class="px-5 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors">Batal</a>
