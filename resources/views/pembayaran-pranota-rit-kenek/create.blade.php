...
@section('title', 'Form Pembayaran Pranota Rit Kenek')
...
@section('page_title', 'Form Pembayaran Pranota Rit Kenek')
...
@section('content')
    <div class="bg-white shadow-lg rounded-lg p-4 max-w-6xl mx-auto">
        ...
        <div class="bg-gray-50 rounded-lg p-3 mb-3 border border-gray-200">
            <form action="{{ route('pembayaran-pranota-rit-kenek.create') }}" method="GET" class="flex flex-col sm:flex-row gap-2 items-end">
                ...
                <div class="flex gap-1">
                    ...
                    <a href="{{ route('pembayaran-pranota-rit-kenek.create') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded text-sm hover:bg-gray-300">Reset</a>
                </div>
            </form>
        </div>

        <form id="pembayaranForm" action="{{ route('pembayaran-pranota-rit-kenek.store') }}" method="POST" class="space-y-4">
            @csrf

            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <!-- Data Pembayaran -->
                <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    <h4 class="text-sm font-semibold mb-3 border-b pb-2">Data Pembayaran Rit Kenek</h4>
                ...
                <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="text-[10px] text-blue-600 bg-blue-50 p-2 rounded">
                            <strong>Info:</strong> Otomatis jurnal ke akun <strong>"Biaya Ritasi"</strong>.
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="border rounded-lg overflow-hidden">
                <div class="bg-gray-100 px-4 py-2 border-b flex justify-between items-center">
                    <h4 class="text-sm font-semibold">Pilih Pranota Rit Kenek</h4>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50 sticky top-0">
                            <tr>
                                ...
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Kenek</th>
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Tanggal</th>
                                <th class="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase">Total Bill</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($pranotaRits as $pranota)
                                @php
                                    $totalBill = $pranota->grand_total_bersih;
                                @endphp
                                <tr class="hover:bg-gray-50 cursor-pointer" onclick="toggleRow(this)">
                                    <td class="px-4 py-2" onclick="event.stopPropagation()">
                                        <input type="checkbox" name="pranota_uang_rit_kenek_ids[]" value="{{ $pranota->id }}" 
                                               class="pranota-checkbox rounded border-gray-300" 
                                               data-amount="{{ $totalBill }}"
                                               {{ request('pranota_id') == $pranota->id ? 'checked' : '' }}>
                                    </td>
                                    <td class="px-4 py-2 text-xs font-medium">{{ $pranota->no_pranota }}</td>
                                    <td class="px-4 py-2 text-xs">{{ $pranota->kenek_nama ?? '-' }}</td>
                ...
                <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    ...
                    <div class="space-y-3">
                        <div class="flex justify-between items-center">
                            <span class="text-sm text-gray-600">Subtotal Tagihan (Rit Kenek):</span>
                            <div class="relative">
                                <span class="absolute left-3 top-2 text-sm text-gray-400">Rp</span>
                                <input type="text" id="subtotal_display_Formatted" class="{{ $readonlyInputClasses }} text-right w-48 font-bold pl-8" value="0" readonly>
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <div class="relative">
                                <span class="absolute left-3 top-2 text-sm text-gray-400">Rp</span>
                                <input type="number" name="total_tagihan_penyesuaian" id="total_tagihan_penyesuaian" class="{{ $inputClasses }} text-right w-48 bg-white pl-8" value="0">
                            </div>
                        </div>
                        <div class="flex justify-between items-center pt-2 border-t">
                            <span class="font-bold text-gray-800">TOTAL AKHIR:</span>
                            <div class="relative">
                                <span class="absolute left-3 top-2 text-sm text-blue-400 font-bold">Rp</span>
                                <input type="text" id="total_akhir_Formatted" class="{{ $readonlyInputClasses }} text-right w-48 font-black text-blue-700 bg-blue-50 pl-8" value="0" readonly>
                                <input type="hidden" name="total_tagihan_setelah_penyesuaian" id="total_akhir" value="0">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t">
                <a href="{{ route('pembayaran-pranota-rit-kenek.index') }}" class="px-6 py-2 border rounded text-gray-600 hover:bg-gray-100 transition-colors">Batal</a>
                <button type="submit" class="px-8 py-2 bg-green-600 text-white font-bold rounded hover:bg-green-700 shadow-md">Simpan Pembayaran Rit Kenek</button>
            </div>
        </form>
    </div>
    ...
    <script>
        ...
        const subtotalDisplay = document.getElementById('subtotal_display_Formatted');
        ...
        const totalAkhirDisplay = document.getElementById('total_akhir_Formatted');
        ...
        function calculateTotal() {
            ...
            const finalTotal = subtotal + penyesuaian;
            totalAkhirInput.value = finalTotal;
            totalAkhirDisplay.value = formatRupiah(finalTotal);
        }
        ...
        document.getElementById('generateNomorBtn').addEventListener('click', function() {
            const btn = this;
            btn.disabled = true;
            fetch('{{ route("pembayaran-pranota-rit-kenek.generate-nomor") }}')
                .then(r => r.json())
                .then(data => {
                    if(data.success) {
        ...
        document.getElementById('pembayaranForm').addEventListener('submit', function(e) {
            ...
            if(count === 0) {
                e.preventDefault();
                alert('Silakan pilih minimal satu pranota yang akan dibayar.');
                return;
            }
