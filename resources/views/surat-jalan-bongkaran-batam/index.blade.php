...
<div class="flex-1 p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="mb-2 text-sm font-medium text-gray-600">Total Manifest</p>
                    <p class="text-2xl font-semibold text-gray-900">{{ number_format($stats->total_manifest) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="mb-2 text-sm font-medium text-gray-600">Sudah Surat Jalan</p>
                    <p class="text-2xl font-semibold text-gray-900">{{ number_format($stats->sudah_sj) }}</p>
                </div>
            </div>
            <div class="mt-4">
                <div class="w-full bg-gray-200 rounded-full h-1.5">
                    <div class="bg-green-600 h-1.5 rounded-full" style="width: {{ $stats->persentase_selesai }}%"></div>
                </div>
                <p class="mt-2 text-xs text-gray-500">{{ $stats->persentase_selesai }}% Selesai</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="mb-2 text-sm font-medium text-gray-600">Belum Surat Jalan</p>
                    <p class="text-2xl font-semibold text-gray-900">{{ number_format($stats->belum_sj) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="mb-2 text-sm font-medium text-gray-600">Avg Leadtime</p>
                    <p class="text-lg font-semibold text-gray-900">
                        @if($stats->avg_leadtime_days > 0)
                            {{ $stats->avg_leadtime_days }}h 
                        @endif
                        {{ $stats->avg_leadtime_hours }}j
                    </p>
                </div>
            </div>
            <p class="mt-4 text-xs text-gray-500">Dari Manifest ke Surat Jalan</p>
        </div>
    </div>
    @endif
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="p-6">
            <!-- Search Form -->
            <form method="GET" action="{{ route('surat-jalan-bongkaran-batam.list') }}" class="mb-6">
                ...
                <div class="flex flex-col gap-4">
                    ...
                    <div class="flex flex-col md:flex-row gap-4">
                        ...
                        <div class="flex items-end gap-2">
                            ...
                            <a href="{{ route('surat-jalan-bongkaran-batam.list', ['nama_kapal' => $selectedKapal, 'no_voyage' => $selectedVoyage, 'mode' => request('mode', 'manifest')]) }}" 
                               class="inline-flex items-center px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white text-sm font-medium rounded-lg transition-colors duration-200">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                                </svg>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($manifests as $index => $manifest)
                                <tr class="hover:bg-gray-50">
                                                ...
                                                <div class="py-1">
                                                    ...
                                                    <a href="#" onclick="buatSuratJalan({{ $manifest->id }}); return false;" 
                                                       class="group flex items-center px-3 py-2 text-xs text-indigo-700 hover:bg-indigo-50 hover:text-indigo-900">
                                                        ...
                                                        @if($manifest->suratJalanBongkaranBatam)
                                                            Tambah Surat Jalan
                                                        @else
                                                            Buat Surat Jalan
                                                        @endif
                                                    ...
                                                    @if($manifest->suratJalanBongkaranBatam)
                                                        <a href="#" onclick="editSuratJalan({{ $manifest->suratJalanBongkaranBatam->id }}); return false;" 
                                                        ...
                                                        <a href="#" onclick="deleteSuratJalan({{ $manifest->suratJalanBongkaranBatam->id }}); return false;" 
                                                           class="group flex items-center px-3 py-2 text-xs text-red-700 hover:bg-red-50 hover:text-red-900">
                                                            <svg class="mr-2 h-4 w-4 text-red-400 group-hover:text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                                            </svg>
                                    ...
                                    <td class="px-4 py-3 text-sm text-gray-900">
                                        @if($manifest->suratJalanBongkaranBatam)
                                            <span class="font-semibold text-blue-600">{{ $manifest->suratJalanBongkaranBatam->nomor_surat_jalan }}</span>
                                        @else
    ...
    <div id="modalBuatSuratJalan" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
        <div class="relative top-10 mx-auto p-5 border w-11/12 max-w-4xl shadow-lg rounded-lg bg-white">
            ...
            <!-- Modal Body -->
            <form id="formBuatSuratJalan" action="{{ route('surat-jalan-bongkaran-batam.store') }}" method="POST" class="mt-4" onsubmit="return handleFormSubmit(event)">
                @csrf
