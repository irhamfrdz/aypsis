...
@section('title', 'Detail Pembayaran Pranota Ongkos Truk')
@section('page_title', 'Detail Pembayaran Pranota Ongkos Truk')
...
<div class="max-w-4xl mx-auto">
    <div class="bg-white shadow-xl rounded-lg overflow-hidden border border-gray-200">
        ...
        <div class="bg-indigo-700 px-6 py-4 flex justify-between items-center text-white">
            <div>
                <h3 class="text-lg font-bold">Pembayaran: {{ $item->nomor_pembayaran }}</h3>
        ...
        <div class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between border-b pb-2 text-indigo-600">
                    ...
                    <div class="flex justify-between border-b pb-2 bg-indigo-50 p-1 rounded">
                        <span class="text-sm font-bold text-gray-800">TOTAL AKHIR:</span>
                        <span class="text-lg font-black text-indigo-700">Rp {{ number_format($item->total_tagihan_setelah_penyesuaian, 0, ',', '.') }}</span>
                    </div>
                </div>
            </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="divide-y divide-gray-100 bg-white">
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-2 text-sm text-gray-500">{{ $pranota->tanggal_pranota ? $pranota->tanggal_pranota->format('d/m/Y') : '-' }}</td>
        ...
        <div class="bg-gray-50 px-6 py-4 flex justify-between border-t border-gray-200">
            <a href="{{ route('pembayaran-pranota-ongkos-truk.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-100">
            ...
            <div class="space-x-2">
                ...
                <a href="{{ route('pembayaran-pranota-ongkos-truk.edit', $item->id) }}" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-yellow-600 hover:bg-yellow-700">
                    <i class="fas fa-edit mr-2"></i> Edit
                </a>
                @endcan
                @can('pembayaran-pranota-ongkos-truk-delete')
                <form action="{{ route('pembayaran-pranota-ongkos-truk.destroy', $item->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data pembayaran ini? Status pranota akan kembali menjadi submitted.')">
                    @csrf
                    @method('DELETE')
