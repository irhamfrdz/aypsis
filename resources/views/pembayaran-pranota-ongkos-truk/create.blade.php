...
@section('title', 'Form Pembayaran Pranota Ongkos Truk')
@section('page_title', 'Form Pembayaran Pranota Ongkos Truk')
...
@section('content')
    <div class="bg-white shadow-lg rounded-lg p-4 max-w-6xl mx-auto">
        ...
        <div class="bg-gray-50 rounded-lg p-3 mb-3 border border-gray-200">
            <form action="{{ route('pembayaran-pranota-ongkos-truk.create') }}" method="GET" class="flex flex-col sm:flex-row gap-2 items-end">
                ...
                <div class="flex gap-1">
                    ...
                    <a href="{{ route('pembayaran-pranota-ongkos-truk.create') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded text-sm hover:bg-gray-300">Reset</a>
                </div>
            </form>
        </div>

        <form id="pembayaranForm" action="{{ route('pembayaran-pranota-ongkos-truk.store') }}" method="POST" class="space-y-4">
            @csrf
                ...
                <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="text-[10px] text-blue-600 bg-blue-50 p-2 rounded">
                            <strong>Info:</strong> Otomatis jurnal ke akun <strong>"Biaya Trucking"</strong>.
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="border rounded-lg overflow-hidden">
                <div class="bg-gray-100 px-4 py-2 border-b flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 flex-1">
                        <h4 class="text-sm font-semibold">Pilih Pranota Ongkos Truk</h4>
                        <div class="relative w-full sm:w-80">
                            <input type="text" id="pranotaSearch" placeholder="Cari No. Pranota..." class="w-full text-xs border border-gray-300 rounded-md pl-8 pr-3 py-1.5 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" autocomplete="off">
                ...
                <div class="overflow-x-auto max-h-72">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody id="pranotaTableBody" class="bg-white divide-y divide-gray-200">
                            @forelse($pranotaOngkosTruks as $pranota)
                                <tr class="pranota-row hover:bg-gray-50 cursor-pointer" onclick="toggleRow(this)">
                                    <td class="px-4 py-2" onclick="event.stopPropagation()">
                                        <input type="checkbox" name="pranota_ongkos_truk_ids[]" value="{{ $pranota->id }}" class="pranota-checkbox rounded border-gray-300" data-amount="{{ $pranota->total_nominal }}">
                                    </td>
                                    ...
                                    <td class="px-4 py-2 text-xs text-right font-bold">Rp {{ number_format($pranota->total_nominal, 0, ',', '.') }}</td>
                                </tr>
                            @empty
                                <tr>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t">
                <a href="{{ route('pembayaran-pranota-ongkos-truk.index') }}" class="px-6 py-2 border rounded text-gray-600 hover:bg-gray-100 transition-colors">Batal</a>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if(generateBtn) {
                generateBtn.addEventListener('click', function() {
                    const btn = this;
                    btn.disabled = true;
                    fetch('{{ route("pembayaran-pranota-ongkos-truk.generate-nomor") }}')
                        .then(r => r.json())
                        .then(data => {
                            if(data.success) {
            ...
            document.getElementById('pembayaranForm').addEventListener('submit', function(e) {
                ...
                if(count === 0) {
                    e.preventDefault();
                    alert('Silakan pilih minimal satu pranota ongkos truk yang akan dibayar.');
                    return;
                }
                if(!bankValue.value) {
                    e.preventDefault();
