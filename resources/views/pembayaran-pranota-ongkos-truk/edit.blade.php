...
@section('title', 'Edit Pembayaran Pranota Ongkos Truk')
@section('page_title', 'Edit Pembayaran Pranota Ongkos Truk')
...
@section('content')
    <div class="bg-white shadow-lg rounded-lg p-4 max-w-6xl mx-auto">
        ...
        <form id="pembayaranForm" action="{{ route('pembayaran-pranota-ongkos-truk.update', $item->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
                            ...
                            @forelse($pranotaOngkosTruks as $pranota)
                                ...
                                <tr class="pranota-row hover:bg-gray-50 cursor-pointer" onclick="toggleRow(this)">
                                    <td class="px-4 py-2" onclick="event.stopPropagation()">
                                        <input type="checkbox" name="pranota_ongkos_truk_ids[]" value="{{ $pranota->id }}" class="pranota-checkbox rounded border-gray-300" data-amount="{{ $pranota->total_nominal }}" {{ $isChecked ? 'checked' : '' }}>
                                    </td>
            ...
            if (pranotaSearch) {
                pranotaSearch.addEventListener('input', function() {
                    ...
                    pranotaRows.forEach(row => {
                        ...
                        row.classList.toggle('hidden', !noPranota.includes(term));
                    });
                });
            }
