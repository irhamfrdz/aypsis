...
<div class="min-h-screen bg-slate-50/50 py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-5xl mx-auto">
        ...
        <div class="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-50 text-amber-700 text-xs font-semibold uppercase tracking-wider mb-2.5 border border-amber-200/50">
                    ...
                    <i class="fas fa-gas-pump text-[10px]"></i> Operasional Kendaraan
                </div>
                <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Catat Biaya Bensin</h1>
                <p class="text-slate-500 mt-1 text-sm">Lengkapi data pengisian bahan bakar kendaraan di bawah ini.</p>
            </div>
        ...
        <!-- Main Form -->
        <form action="{{ route('biaya-bensin.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
                    ...
                    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-6">
                        <div class="flex items-center gap-3 pb-4 border-b border-slate-100">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-slate-400">Pilih supir, unit kendaraan, dan kartu bensin</p>
                            </div>
                        </div>
                                ...
                                <optgroup label="Alat Berat">
                                    @foreach($alatBerats as $alat)
                                        @if(!empty(trim($alat->kode_alat)) || !empty(trim($alat->nama)))
                                            <option value="alat_{{ $alat->id }}" data-last-km-akhir="{{ $alat->last_km_akhir }}" {{ old('kendaraan_id') == 'alat_'.$alat->id ? 'selected' : '' }}>
                    ...
                    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-6">
                        <div class="flex items-center gap-3 pb-4 border-b border-slate-100">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-slate-400">Masukkan detail volume bensin, harga, dan KM kendaraan</p>
                            </div>
                        </div>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" name="km_awal" id="km_awal" value="{{ old('km_awal') }}" placeholder="0" readonly
                                           class="block w-full pl-10 pr-4 py-3 bg-slate-100 border border-slate-200 rounded-xl text-slate-500 font-semibold text-sm cursor-not-allowed outline-none">
                                </div>
                            </div>
                            <div>
                        ...
                        <div>
                            ...
                            <textarea name="keterangan" id="keterangan" rows="3" placeholder="Tulis catatan tambahan di sini jika ada..."
                                      class="block w-full px-4 py-3 bg-slate-50/50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all text-slate-900 text-sm resize-none">{{ old('keterangan') }}</textarea>
                        </div>
            ...
            <div class="mt-8 bg-white border border-slate-100 rounded-2xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm">
                <div class="text-xs text-slate-400 flex items-center gap-2">
                    ...
                    <span>Setelah disimpan, data operasional bensin ini akan langsung memotong saldo kartu terkait.</span>
                </div>
                <button type="submit" 
                    ...
                    <i class="fas fa-save"></i> Simpan Catatan Bensin
                </button>
            </div>
        </form>
