...
@section('title', 'Master Pricelist Biaya Storage')

@section('content')
<div class="flex justify-between items-center mb-4">
    <h2 class="text-xl font-bold text-gray-800">Master Pricelist Biaya Storage</h2>
    @can('master-pricelist-biaya-storage-create')
    <a href="{{ route('master-pricelist-biaya-storage.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-md shadow-sm transition-colors">
        <i class="fas fa-plus mr-2"></i> Tambah Tarif Storage
    </a>
    @endcan
</div>
...
<div class="mb-6 bg-white p-4 rounded-lg shadow-sm border">
    <form method="GET" action="{{ route('master-pricelist-biaya-storage.index') }}" class="space-y-4">
        <div class="flex flex-wrap gap-4 items-end">
            ...
            {{-- Ukuran Filter --}}
            <div class="min-w-32">
                <label for="size_kontainer" class="block text-sm font-medium text-gray-700 mb-1">Ukuran</label>
                <select id="size_kontainer" name="size_kontainer" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    ...
                    <option value="45" {{ request('size_kontainer') == '45' ? 'selected' : '' }}>45'</option>
                </select>
            </div>
            ...
            <div class="flex space-x-2">
                ...
                <a href="{{ route('master-pricelist-biaya-storage.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md shadow-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
    ...
    <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50 sticky top-0 z-10 shadow-sm">
            <tr>
                ...
                <th scope="col" class="px-4 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">Vendor / Terminal</th>
                ...
                <th scope="col" class="px-4 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">Tarif Massa 1</th>
                <th scope="col" class="px-4 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">Tarif Massa 2</th>
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr class="hover:bg-gray-50 transition-colors">
                ...
                <td class="px-4 py-3 whitespace-nowrap text-center text-sm font-bold text-indigo-600">Rp {{ number_format($pricelist->tarif_massa_1, 0, ',', '.') }}</td>
                <td class="px-4 py-3 whitespace-nowrap text-center text-sm font-bold text-blue-600">Rp {{ number_format($pricelist->tarif_massa_2, 0, ',', '.') }}</td>
                ...
                <td class="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                    <div class="flex items-center justify-end space-x-3 text-[10px]">
                        @can('master-pricelist-biaya-storage-update')
                        <a href="{{ route('master-pricelist-biaya-storage.edit', $pricelist->id) }}" class="text-blue-600 hover:text-blue-800 hover:underline font-medium">Edit</a>
                        @endcan
                        @can('master-pricelist-biaya-storage-delete')
                        <span class="text-gray-300">|</span>
                        <form action="{{ route('master-pricelist-biaya-storage.destroy', $pricelist->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Hapus data pricelist ini?')">
                            @csrf
                            @method('DELETE')
            ...
            <tr>
                <td colspan="10" class="px-4 py-10 text-center text-sm text-gray-500">
                    ...
                    Tidak ada data pricelist Biaya Storage.
                </td>
            </tr>
            @endforelse
        </tbody>
