...
<div class="container mx-auto p-1">
    ...
    <div class="mb-6 bg-white rounded-lg shadow-sm border border-gray-200 p-4">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div class="flex flex-wrap items-center gap-3">
                ...
                <a href="{{ route('daftar-tagihan-kontainer-sewa-2.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-2 py-2 rounded-lg transition-colors duration-150 flex items-center">
                ...
                <!-- Buat Group -->
                @can('tagihan-kontainer-sewa-create')
                <a href="{{ route('daftar-tagihan-kontainer-sewa-2.create-group') }}" class="bg-purple-600 hover:bg-purple-700 text-white px-2 py-2 rounded-lg transition-colors duration-150 flex items-center">
                    <svg class="h-4 w-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Import Data (Old) -->
                @can('tagihan-kontainer-sewa-create')
                <a href="{{ route('daftar-tagihan-kontainer-sewa-2.import') }}" class="bg-green-600 hover:bg-green-700 text-white px-2 py-2 rounded-lg transition-colors duration-150 flex items-center">
                    <svg class="h-4 w-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Export Template -->
                @can('tagihan-kontainer-sewa-create')
                <a href="{{ route('daftar-tagihan-kontainer-sewa-2.export-template') }}" class="bg-orange-600 hover:bg-orange-700 text-white px-2 py-2 rounded-lg transition-colors duration-150 flex items-center">
                    <svg class="h-4 w-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                    </svg>
                    Download Template
            ...
            <div class="flex items-center gap-2">
                ...
                <!-- Filter Ongoing -->
                <a href="{{ route('daftar-tagihan-kontainer-sewa-2.index', ['status' => 'ongoing']) }}"
                   onclick="if(typeof saveCheckboxState === 'function') saveCheckboxState();"
                ...
                <a href="{{ route('daftar-tagihan-kontainer-sewa-2.index', ['status_pranota' => 'null']) }}"
                   onclick="if(typeof saveCheckboxState === 'function') saveCheckboxState();"
                ...
                <!-- Filter Vendor ZONA -->
                <a href="{{ route('daftar-tagihan-kontainer-sewa-2.index', ['vendor' => 'ZONA']) }}"
                   onclick="if(typeof saveCheckboxState === 'function') saveCheckboxState();"
                ...
                <!-- Filter 40ft -->
                <a href="{{ route('daftar-tagihan-kontainer-sewa-2.index', ['size' => '40']) }}"
                   onclick="if(typeof saveCheckboxState === 'function') saveCheckboxState();"
    ...
    <div class="mb-6 bg-white rounded-lg shadow-sm border border-gray-200 p-4">
        <form action="{{ route('daftar-tagihan-kontainer-sewa-2.index') }}" method="GET" class="space-y-4" id="searchFilterForm" onsubmit="if(typeof saveCheckboxState === 'function') saveCheckboxState();">
            @php
                // Precompute relatedGroups early so toggle buttons can reflect auto-detect state
                if (request('q')) {
                    $searchTerm = request('q');
                    $matchingContainers = \App\Models\DaftarTagihanKontainerSewaDua::where('nomor_kontainer', 'LIKE', '%' . $searchTerm . '%')
                        ->whereNotNull('group')
                        ->where('group', '!=', '')
                        ->get();
            ...
            @if(request('q'))
                ...
                @if($isGroupSearch)
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 flex items-center gap-2">
                        ...
                        <a href="{{ route('daftar-tagihan-kontainer-sewa-2.index') }}" class="ml-auto text-blue-600 hover:text-blue-800">
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="flex items-center gap-2 ml-auto">
                    ...
                    @if(request()->anyFilled(['q', 'vendor', 'size', 'periode', 'status', 'status_pranota']))
                        <a href="{{ route('daftar-tagihan-kontainer-sewa-2.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-2 rounded-lg transition-colors duration-150 flex items-center text-sm">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($tagihans ?? [] as $index => $tagihan)
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-1 py-0.5 whitespace-nowrap text-center text-[8px] text-gray-900">
                            <div class="flex items-center justify-center space-x-1">
                                <form action="{{ route('daftar-tagihan-kontainer-sewa-2.create-next-periode', $tagihan->id) }}" method="POST" class="inline" onsubmit="return confirm('Buat periode selanjutnya untuk kontainer ini?')">
                                    @csrf
                                ...
                                <a href="{{ route('daftar-tagihan-kontainer-sewa-2.show', $tagihan->id) }}" class="inline-flex items-center px-2 py-1 rounded text-[7px] font-medium bg-indigo-100 text-indigo-700 hover:bg-indigo-200 transition-colors">
                                    <svg class="w-2 h-2 mr-0.5" fill="currentColor" viewBox="0 0 20 20">
                                ...
                                <a href="{{ route('daftar-tagihan-kontainer-sewa-2.edit', $tagihan->id) }}" class="inline-flex items-center px-2 py-1 rounded text-[7px] font-medium bg-yellow-100 text-yellow-700 hover:bg-yellow-200 transition-colors">
                                    <svg class="w-2 h-2 mr-0.5" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                                    </svg>
                                    Edit
                                </a>
                                <form action="{{ route('daftar-tagihan-kontainer-sewa-2.destroy', $tagihan->id) }}" method="POST" onsubmit="return confirm('Hapus tagihan kontainer ini? Tindakan ini tidak dapat dibatalkan.')" class="inline">
                                    @csrf
                                    @method('DELETE')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (btnExport && btnExport !== null) {
        btnExport.addEventListener('click', function() {
            ...
            let exportUrl = '{{ route("daftar-tagihan-kontainer-sewa-2.export", [], false) }}';
            const params = [];
    ...
    document.querySelectorAll('a[href*="daftar-tagihan-kontainer-sewa-2.index"]').forEach(link => {
    ...
    document.querySelectorAll('a[href*="status="], a[href*="status_pranota="], a[href*="daftar-tagihan-kontainer-sewa-2"], .pagination a').forEach(link => {
    ...
    if (btnBulkDelete && btnBulkDelete !== null) {
        btnBulkDelete.addEventListener('click', function() {
            ...
            if (confirm(message)) {
                ...
                form.method = 'POST';
                form.action = '{{ route("daftar-tagihan-kontainer-sewa-2.bulk-delete", [], false) }}';
        ...
        btnBulkStatus.addEventListener('click', function() {
            ...
            if (statusValue) {
                ...
                if (confirm(message)) {
                    ...
                    const form = document.createElement('form');
                    form.method = 'POST';
                    form.action = '{{ route("daftar-tagihan-kontainer-sewa-2.bulk-update-status", [], false) }}';
...
window.ungroupSelectedContainers = function() {
    ...
    // Send AJAX request
    fetch('{{ route("daftar-tagihan-kontainer-sewa-2.ungroup-containers", [], false) }}', {
        method: 'POST',
        body: formData,
        headers: {
...
window.openDeleteGroupModal = function() {
    ...
    fetch('{{ route("daftar-tagihan-kontainer-sewa-2.groups", [], false) }}', {
        method: 'GET',
        headers: {
...
window.deleteGroups = function(groupNames) {
    ...
    // Send AJAX request
    fetch('{{ route("daftar-tagihan-kontainer-sewa-2.delete-groups", [], false) }}', {
        method: 'POST',
        body: formData,
        headers: {
            'X-Requested-With': 'XMLHttpRequest'
...
window.editAdjustment = function(tagihanId, currentAdjustment, currentAdjustmentNote = '') {
    ...
    if (form) {
        ...
        // Send AJAX request
        fetch(`{{ url('daftar-tagihan-kontainer-sewa-2') }}/${tagihanId}/adjustment`, {
            method: 'POST',
            body: formData,
            headers: {
                'X-Requested-With': 'XMLHttpRequest'
...
window.editVendorInfo = function(tagihanId, currentInvoice, currentTanggal) {
    ...
    if (form) {
        ...
        // Send AJAX request
        fetch(`{{ url('daftar-tagihan-kontainer-sewa-2') }}/${tagihanId}/vendor-info`, {
            method: 'POST',
            body: formData,
            headers: {
                'X-Requested-With': 'XMLHttpRequest'
...
window.editGroupInfo = function(tagihanId, currentGroup) {
    ...
    if (form) {
        ...
        // Send AJAX request
        fetch(`{{ url('daftar-tagihan-kontainer-sewa-2') }}/${tagihanId}/group-info`, {
            method: 'POST',
            body: formData,
            headers: {
                'X-Requested-With': 'XMLHttpRequest'
...
window.bulkEditVendorInfo = function() {
    ...
    if (form) {
        ...
        selectedIds.forEach((id, index) => {
            ...
            fetch(`{{ url('daftar-tagihan-kontainer-sewa-2') }}/${id}/vendor-info`, {
                method: 'POST',
                body: formData,
                headers: {
                    'X-Requested-With': 'XMLHttpRequest'
...
window.bulkEditGroupInfo = function() {
    ...
    if (form) {
        ...
        const updatePromises = selectedIds.map(id => {
            ...
            return fetch(`{{ url('daftar-tagihan-kontainer-sewa-2') }}/${id}/group-info`, {
                method: 'POST',
                body: formData,
                headers: {
                    'X-Requested-With': 'XMLHttpRequest'
...
window.generateInvoiceNumber = function() {
    ...
    fetch('{{ route("daftar-tagihan-kontainer-sewa-2.generate-invoice-number", [], false) }}', {
        method: 'GET',
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
            'Accept': 'application/json',
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (importForm) {
        importForm.addEventListener('submit', function(e) {
            ...
            // Send AJAX request
            fetch('{{ route("daftar-tagihan-kontainer-sewa-2.import-csv", [], false) }}', {
                method: 'POST',
                body: formData,
                headers: {
                    'X-Requested-With': 'XMLHttpRequest'
