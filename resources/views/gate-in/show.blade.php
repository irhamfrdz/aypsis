                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500 mt-1">{{ $gateIn->tanggal_formatted }} - {{ $gateIn->kontainers->count() }} kontainer</p>
                        </div>
                    </div>
                    <div class="flex items-center space-x-3">
            ...
            <div class="px-6 py-4 bg-gray-50">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="text-center">
                        <div class="text-2xl font-bold text-purple-600">{{ $gateIn->kontainers->count() }}</div>
                        <div class="text-xs font-medium text-gray-500 uppercase tracking-wider">Total Kontainer</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold text-green-600">{{ $gateIn->kontainers->where('status_gate_in', 'masuk')->count() }}</div>
                        <div class="text-xs font-medium text-gray-500 uppercase tracking-wider">Sudah Masuk</div>
                    </div>
                    <div class="text-2xl font-bold text-blue-600 text-center">
                        <div class="text-2xl font-bold text-blue-600">{{ $gateIn->kontainers->where('status_gate_in', 'keluar')->count() }}</div>
                        <div class="text-xs font-medium text-gray-500 uppercase tracking-wider">Sudah Keluar</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold text-gray-600">{{ $gateIn->kontainers->where('status_gate_in', 'pending')->count() }}</div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                    <div class="px-6 py-4 border-b border-gray-200">
                        <div class="flex items-center justify-between">
                            <h3 class="text-lg font-medium text-gray-900">Daftar Kontainer ({{ $gateIn->kontainers->count() }})</h3>
                            <div class="flex items-center space-x-2">
                        ...
                        @if($gateIn->kontainers->count() > 0)
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4" id="kontainer-grid">
                                ...
                                <div class="kontainer-item border border-gray-200 rounded-lg p-4 hover:border-purple-300 transition-colors duration-200"
                                     data-status="{{ $kontainer->status_gate_in ?? 'pending' }}">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0">
                                            ...
                                            <div class="ml-3">
                                                ...
                                                @if($kontainer->pelanggan)
                                                    <div class="text-xs text-blue-600 mt-1">{{ $kontainer->pelanggan }}</div>
                                                @endif
                                            </div>
                                        </div>
                                        <div class="flex flex-col items-end space-y-1">
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                                {{ ($kontainer->status_gate_in ?? 'pending') === 'masuk' ? 'bg-green-100 text-green-800' :
                                                   (($kontainer->status_gate_in ?? 'pending') === 'keluar' ? 'bg-blue-100 text-blue-800' : 'bg-yellow-100 text-yellow-800') }}">
                                                {{ ucfirst($kontainer->status_gate_in ?? 'Pending') }}
                                            </span>
                                            @if($kontainer->status_checkpoint_supir)
                                                <span class="text-xs text-gray-500">Checkpoint: {{ ucfirst($kontainer->status_checkpoint_supir) }}</span>
                                            @endif
                                        </div>
                                    </div>

                                    @if($kontainer->pivot && ($kontainer->pivot->waktu_masuk || $kontainer->pivot->waktu_keluar))
                                        <div class="mt-3 pt-3 border-t border-gray-100">
                                            <div class="grid grid-cols-2 gap-4 text-xs">
                                                @if($kontainer->pivot->waktu_masuk)
                                                    <div>
                                                        <span class="text-gray-500">Masuk:</span>
                                                        <span class="text-gray-900 ml-1">{{ \Carbon\Carbon::parse($kontainer->pivot->waktu_masuk)->format('d/m H:i') }}</span>
                                                    </div>
                                                @endif
                                                @if($kontainer->pivot->waktu_keluar)
                                                    <div>
                                                        <span class="text-gray-500">Keluar:</span>
                                                        <span class="text-gray-900 ml-1">{{ \Carbon\Carbon::parse($kontainer->pivot->waktu_keluar)->format('d/m H:i') }}</span>
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
