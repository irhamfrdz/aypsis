...
<div class="container mx-auto px-4 py-4 overflow-y-auto h-full pb-24">
    <div class="max-w-3xl mx-auto bg-white rounded-xl shadow-sm border border-gray-200">
        ...
        <div class="flex justify-between items-center p-6 border-b border-gray-150">
            <div>
                <h1 class="text-lg font-bold text-gray-900">Edit Perbaikan Kontainer</h1>
                <p class="text-xs text-gray-500 mt-1">Perbarui data perbaikan untuk nomor perbaikan: <strong class="text-blue-600 font-semibold">{{ $perbaikanKontainer->no_perbaikan }}</strong></p>
            </div>
            <div>
        ...
        <div class="p-6">
            ...
            <form action="{{ route('perbaikan-kontainer.update', $perbaikanKontainer->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="bg-gray-50 p-4 rounded-xl border border-gray-100">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="hidden" name="no_kontainer" id="no_kontainer" value="{{ old('no_kontainer', $perbaikanKontainer->no_kontainer) }}">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="ukuran" id="ukuran" value="{{ old('ukuran', $perbaikanKontainer->ukuran) }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="tipe_kontainer" id="tipe_kontainer" value="{{ old('tipe_kontainer', $perbaikanKontainer->tipe_kontainer) }}"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500 focus:outline-none"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="vendor_bengkel_id" id="vendor_bengkel_id" required
                                ...
                                @foreach($bengkels as $bengkel)
                                    <option value="{{ $bengkel->id }}" {{ old('vendor_bengkel_id', $perbaikanKontainer->vendor_bengkel_id) == $bengkel->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="status" id="status" required
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500 focus:outline-none">
                                <option value="pending" {{ old('status', $perbaikanKontainer->status) === 'pending' ? 'selected' : '' }}>Pending (Draft)</option>
                                <option value="proses" {{ old('status', $perbaikanKontainer->status) === 'proses' ? 'selected' : '' }}>Proses Perbaikan</option>
                    ...
                    <div class="bg-blue-50/50 p-4 rounded-xl border border-blue-100/80 space-y-4">
                        ...
                        <div id="paint_fields_container" class="grid grid-cols-1 md:grid-cols-3 gap-4 hidden">
                            ...
                            <div>
                                ...
                                <input type="text" name="vendor_cat" id="vendor_cat" value="{{ old('vendor_cat', $perbaikanKontainer->vendor_cat) }}"
                                       placeholder="Nama vendor cat..."
                                       class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-blue-500 focus:border-blue-500 focus:outline-none">
                            </div>
                            ...
                            <div>
                                ...
                                <select name="jenis_cat" id="jenis_cat"
                                    ...
                                    <option value="cat_sebagian" {{ old('jenis_cat', $perbaikanKontainer->jenis_cat) === 'cat_sebagian' ? 'selected' : '' }}>Sebagian</option>
                                    <option value="cat_full" {{ old('jenis_cat', $perbaikanKontainer->jenis_cat) === 'cat_full' ? 'selected' : '' }}>Full</option>
                                </select>
                            </div>
                    ...
                    <div id="selesai_fields_container" class="bg-green-50 p-4 rounded-xl border border-green-150 space-y-4 hidden">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <div class="relative rounded-lg shadow-sm">
                                    ...
                                    <input type="number" name="biaya_riil" id="biaya_riil" 
                                           value="{{ old('biaya_riil', intval($perbaikanKontainer->biaya_riil ?: $perbaikanKontainer->estimasi_biaya)) }}" min="0"
                        ...
                        <div>
                            ...
                            <textarea name="keterangan_perbaikan" id="keterangan_perbaikan" rows="3"
                                      ...
                                      class="w-full px-3 py-2 border border-green-300 rounded-lg text-sm focus:ring-green-500 focus:border-green-500 focus:outline-none bg-white">{{ old('keterangan_perbaikan', $perbaikanKontainer->keterangan_perbaikan) }}</textarea>
                        </div>
                    </div>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        var currentUkuran = '{{ $perbaikanKontainer->ukuran }}';
        
        var defaultOption = new Option(currentNo + (currentUkuran ? ' - ' + currentUkuran + 'ft' : ''), currentNo, true, true);
        $('#no_kontainer_select').append(defaultOption).trigger('change');
    });
</script>
@endpush
