...
@section('title', 'Perincian - ' . $namaKapal . ' - ' . $noVoyage)
@section('page_title', 'Perincian')
...
<div class="min-h-screen bg-gray-50 py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                <div class="mb-4 sm:mb-0">
                    <h1 class="text-3xl font-bold text-gray-900">Perincian</h1>
                    <p class="mt-1 text-sm text-gray-600">Kelola data perincian pengiriman kontainer</p>
                </div>
                ...
                <div class="flex gap-2">
                    ...
                    <button onclick="autoUpdateTanggalBerangkat('{{ $namaKapal }}', '{{ $noVoyage }}', '{{ $perincians->first()->tanggal_berangkat ?? '' }}')"
                    ...
                    @can('perincian-edit')
                    <button type="button" onclick="updatePerincianData(true)" 
                    ...
                    <button type="button" onclick="updatePerincianData(false)" 
                    ...
                    @endcan
                    <form action="{{ route('report.perincians.sync') }}" method="POST" class="inline">
                        @csrf
                        <input type="hidden" name="nama_kapal" value="{{ $namaKapal }}">
                        <input type="hidden" name="no_voyage" value="{{ $noVoyage }}">
                        <button type="submit"
                           onclick="return confirm('Apakah Anda yakin ingin melakukan sinkronisasi data Perincian dari data Naik Kapal untuk voyage ini? Proses ini mungkin akan memakan waktu sejenak dan memperbarui data nama barang yang kosong atau belum sesuai dengan Tanda Terima.')"
                    ...
                    <a href="{{ route('report.perincians.export', request()->all()) }}"
                ...
                <div class="flex flex-wrap items-center justify-between">
                    <div class="flex items-center space-x-6">
                        ...
                        <div class="flex items-center">
                            ...
                            <div>
                                <div class="text-xs text-purple-100">Total Perincian</div>
                                <div class="font-bold">{{ $perincians->total() }} dokumen</div>
                            </div>
                        </div>
                    </div>
                    <div>
                        <a href="{{ route('report.perincians.select-ship') }}" 
        ...
        <!-- Search Section -->
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            <form method="GET" action="{{ route('report.perincians.index') }}">
                ...
                <div class="flex justify-end gap-2">
                    <a href="{{ route('report.perincians.index') }}"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-2 py-3">
                                ...
                                <div class="flex items-center gap-1">
                                    <input type="number" 
                                           ...
                                           data-perincian-id="{{ $perincian->id }}"
                                           data-original-value="{{ $perincian->nomor_urut }}"
                                           value="{{ $perincian->nomor_urut }}"
                                ...
                                @else
                                <div class="text-[11px] font-bold text-gray-900">{{ $perincian->nomor_urut ?? '-' }}</div>
                                @endcan
                            </td>
                            <td class="px-2 py-3">
                                @can('perincian-edit')
                                <div class="flex flex-col gap-1">
                                    <input type="text" 
                                           ...
                                           data-perincian-id="{{ $perincian->id }}"
                                           data-original-value="{{ $perincian->nomor_bl }}"
                                           value="{{ $perincian->nomor_bl }}"
                                           title="Edit nomor BL">
                                ...
                                @else
                                <div class="text-[11px] font-medium text-gray-900">{{ $perincian->nomor_bl }}</div>
                                @endcan
                            </td>
                            <td class="px-2 py-3 text-[11px] text-gray-900">
                                {{ $perincian->nomor_tanda_terima_display }}
                            </td>
                            <td class="px-2 py-3">
                                <div class="text-[11px] font-medium text-gray-900">{{ $perincian->nomor_kontainer }}</div>
                                <div class="text-[10px] text-gray-500">Seal: {{ $perincian->no_seal }}</div>
                            </td>
                            <td class="px-2 py-3 text-[11px] text-gray-900">
                                {{ $perincian->tipe_kontainer }}<br><span class="text-gray-500">{{ $perincian->size_kontainer }}'</span>
                            </td>
                            <td class="px-2 py-3 text-[11px] text-gray-900 whitespace-normal leading-tight">
                                {{ $perincian->nama_barang }}
                            </td>
                            <td class="px-2 py-3">
                                @can('perincian-edit')
                                <div class="flex flex-col gap-1">
                                    <div class="flex items-center gap-1">
                                        <input type="number" step="any"
                                               ...
                                               data-perincian-id="{{ $perincian->id }}"
                                               data-original-value="{{ $perincian->kuantitas }}"
                                               value="{{ $perincian->kuantitas }}">
                                        <button type="button" class="save-kuantitas-btn p-1 bg-green-500 text-white rounded hidden" data-perincian-id="{{ $perincian->id }}">
                                    ...
                                    <div class="flex items-center gap-1">
                                        <input type="text" 
                                               ...
                                               data-perincian-id="{{ $perincian->id }}"
                                               data-original-value="{{ $perincian->satuan }}"
                                               value="{{ $perincian->satuan }}">
                                        <button type="button" class="save-satuan-btn p-1 bg-green-500 text-white rounded hidden" data-perincian-id="{{ $perincian->id }}">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                                        </button>
                                    </div>
                                </div>
                                @else
                                <div class="text-[11px] text-gray-900">{{ $perincian->kuantitas ?? '-' }} {{ $perincian->satuan }}</div>
                                @endcan
                            </td>
                            <td class="px-2 py-3 text-[11px] text-gray-900 whitespace-normal leading-tight">
                                <div class="font-bold">{{ $perincian->pengirim }}</div>
                            </td>
                            <td class="px-2 py-3 text-[11px] text-gray-900 whitespace-normal leading-tight">
                                @if($perincian->alamat_pengirim)
                                    <div class="text-[10px] text-gray-500 mt-0.5 line-clamp-2" title="{{ $perincian->alamat_pengirim }}">
                                        {{ $perincian->alamat_pengirim }}
                                    </div>
                                @else
                            ...
                            <td class="px-2 py-3 text-[11px] text-gray-900 whitespace-normal leading-tight">
                                <div class="font-bold">{{ $perincian->penerima }}</div>
                                @if($perincian->alamat_penerima)
                                    <div class="text-[10px] text-gray-500 mt-0.5 line-clamp-1" title="{{ $perincian->alamat_penerima }}">
                                        {{ $perincian->alamat_penerima }}
                                    </div>
                                @endif
                            </td>
                            <td class="px-2 py-3 text-[11px] text-gray-900 whitespace-normal leading-tight">
                                @if($perincian->notify_party)
                                    <div class="font-bold">{{ $perincian->notify_party }}</div>
                                    @if($perincian->alamat_notify_party)
                                        <div class="text-[10px] text-gray-500 mt-0.5 line-clamp-1" title="{{ $perincian->alamat_notify_party }}">
                                            {{ $perincian->alamat_notify_party }}
                                        </div>
                                    @endif
                                @else
                                    <span class="text-gray-400">-</span>
                            ...
                            <td class="px-2 py-3 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    @can('perincian-view')
                                    <a href="{{ route('report.perincians.show', $perincian->id) }}"
                                    ...
                                    @endcan
                                    @can('perincian-delete')
                                    <form action="{{ route('report.perincians.destroy', $perincian->id) }}" method="POST" class="inline"
                                          onsubmit="return confirm('Apakah Anda yakin ingin menghapus perincian ini?');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="12" class="px-6 py-12 text-center">
                                ...
                                <p class="mt-1 text-sm text-gray-500">Mulai dengan menambahkan perincian baru.</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
            ...
            <!-- Pagination -->
            @if($perincians->hasPages())
            <div class="bg-white px-4 py-3 border-t border-gray-200 sm:px-6">
                {{ $perincians->withQueryString()->links() }}
            </div>
            @endif
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
        ...
        <div class="flex items-center justify-between p-6 border-b border-gray-200">
            <div class="flex items-center">
                ...
                <h3 class="text-xl font-bold text-gray-900">Import Perincian</h3>
            </div>
        ...
        <!-- Modal Body -->
        <form action="{{ route('report.perincians.import') }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            <input type="hidden" name="nama_kapal" value="{{ $namaKapal }}">
            <input type="hidden" name="no_voyage" value="{{ $noVoyage }}">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    blInputs.forEach(input => {
        const perincianId = input.dataset.perincianId;
        const saveBtn = document.querySelector(`.save-bl-btn[data-perincian-id="${perincianId}"]`);
        const cancelBtn = document.querySelector(`.cancel-bl-btn[data-perincian-id="${perincianId}"]`);
        ...
        input.addEventListener('keydown', function(e) {
            if (e.key === 'Enter') {
                ...
                if (newValue !== originalValue && newValue !== '') {
                    updateNomorBl(perincianId, newValue, this);
                }
            }
    ...
    urutInputs.forEach(input => {
        const perincianId = input.dataset.perincianId;
        const saveBtn = document.querySelector(`.save-urut-btn[data-perincian-id="${perincianId}"]`);
        ...
        input.addEventListener('keydown', function(e) {
            if (e.key === 'Enter') {
                e.preventDefault();
                updateNomorUrut(perincianId, this.value, this);
            }
            if (e.key === 'Escape') {
                e.preventDefault();
                this.value = this.dataset.originalValue;
    ...
    kuantitasInputs.forEach(input => {
        const perincianId = input.dataset.perincianId;
        const saveBtn = document.querySelector(`.save-kuantitas-btn[data-perincian-id="${perincianId}"]`);
        ...
        input.addEventListener('keydown', function(e) {
            if (e.key === 'Enter') {
                e.preventDefault();
                updateKuantitas(perincianId, this.value, this);
            }
            if (e.key === 'Escape') {
                e.preventDefault();
                this.value = this.dataset.originalValue;
    ...
    satuanInputs.forEach(input => {
        const perincianId = input.dataset.perincianId;
        const saveBtn = document.querySelector(`.save-satuan-btn[data-perincian-id="${perincianId}"]`);
        ...
        input.addEventListener('keydown', function(e) {
            if (e.key === 'Enter') {
                e.preventDefault();
                updateSatuan(perincianId, this.value, this);
            }
            if (e.key === 'Escape') {
                e.preventDefault();
                this.value = this.dataset.originalValue;
    ...
    document.querySelectorAll('.save-kuantitas-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            const perincianId = this.dataset.perincianId;
            const input = document.querySelector(`.kuantitas-input[data-perincian-id="${perincianId}"]`);
            updateKuantitas(perincianId, input.value, input);
        });
    });

    function updateKuantitas(perincianId, newValue, element) {
        element.classList.add('opacity-50');
        const saveBtn = document.querySelector(`.save-kuantitas-btn[data-perincian-id="${perincianId}"]`);
        
        const csrfToken = document.querySelector('meta[name="csrf-token"]').content;
        
        fetch(`/report/perincians/${perincianId}/update-kuantitas`, {
            method: 'POST',
            headers: {
    ...
    document.querySelectorAll('.save-satuan-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            const perincianId = this.dataset.perincianId;
            const input = document.querySelector(`.satuan-input[data-perincian-id="${perincianId}"]`);
            updateSatuan(perincianId, input.value, input);
        });
    });

    function updateSatuan(perincianId, newValue, element) {
        element.classList.add('opacity-50');
        const saveBtn = document.querySelector(`.save-satuan-btn[data-perincian-id="${perincianId}"]`);
        
        const csrfToken = document.querySelector('meta[name="csrf-token"]').content;
        
        fetch(`/report/perincians/${perincianId}/update-satuan`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': csrfToken,
    ...
    document.querySelectorAll('.save-urut-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            const perincianId = this.dataset.perincianId;
            const input = document.querySelector(`.urut-input[data-perincian-id="${perincianId}"]`);
            updateNomorUrut(perincianId, input.value, input);
        });
    });

    function updateNomorUrut(perincianId, newValue, element) {
        element.classList.add('opacity-50');
        const saveBtn = document.querySelector(`.save-urut-btn[data-perincian-id="${perincianId}"]`);
        
        const csrfToken = document.querySelector('meta[name="csrf-token"]').content;
        
        fetch(`/report/perincians/${perincianId}/update-nomor-urut`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': csrfToken,
    ...
    saveBtns.forEach(btn => {
        btn.addEventListener('click', function(e) {
            ...
            const perincianId = this.dataset.perincianId;
            const input = document.querySelector(`.bl-input[data-perincian-id="${perincianId}"]`);
            const newValue = input.value.trim();
            
            console.log('Save clicked for perincian:', perincianId, 'Value:', newValue);
            
            if (newValue !== '') {
                updateNomorBl(perincianId, newValue, input);
            } else {
    ...
    window.updatePerincianData = function(isDryRun) {

        const actionText = isDryRun ? 'Preview Update' : 'Update Data';
        const confirmText = isDryRun ? 
            'Anda akan melihat preview data Perincian yang akan diupdate dari Tanda Terima tanpa mengubah data apapun. Lanjutkan?' : 
            'Anda akan mengupdate data penerima dan alamat pada Perincian berdasarkan data Tanda Terima. Lanjutkan?';
        ...
        .then(data => {
            ...
            if (data.success) {
                // Show result modal
                showPerincianUpdateResult(data, isDryRun);
            } else {
    ...
    function showPerincianUpdateResult(data, isDryRun) {
        const modalTitle = isDryRun ? 'Preview Update Perincian' : 'Hasil Update Perincian';
        ...
        const warningNote = isDryRun && data.total_with_changes > 0 ? 
            `<div class="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                <p class="text-sm text-yellow-800"><strong>Catatan:</strong> Ini adalah preview. Data belum diupdate. Klik tombol "Update Perincian" untuk melakukan update sebenarnya.</p>
            </div>` : '';
        
        const successNote = !isDryRun && data.total_updated > 0 ? 
            `<div class="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                <p class="text-sm text-green-800"><strong>Berhasil!</strong> Data Perincian telah diupdate dari Tanda Terima.</p>
            </div>` : '';
        
        const noChangeNote = data.total_with_changes === 0 ? 
            `<div class="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <p class="text-sm text-blue-800"><strong>Info:</strong> Semua data penerima dan alamat pada Perincian sudah sama dengan data di Tanda Terima.</p>
            </div>` : '';
        
        const modalHtml = `
            <div id="perincian-update-result-modal" class="fixed inset-0 bg-gray-600 bg-opacity-50 z-50 flex items-center justify-center p-4">
                <div class="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div class="bg-blue-50 p-4 rounded-lg">
                                <p class="text-sm text-blue-600 font-medium">Total Perincian Diproses</p>
                                <p class="text-3xl font-bold text-blue-900">${data.total_perincians}</p>
                            </div>
                            <div class="bg-purple-50 p-4 rounded-lg">
                                <p class="text-sm text-purple-600 font-medium">Dengan Tanda Terima</p>
                                <p class="text-3xl font-bold text-purple-900">${data.total_perincian_with_tt}</p>
                            </div>
                    ...
                    <div class="p-6 border-t border-gray-200 flex justify-end gap-3">
                        <button onclick="closePerincianUpdateModal()" class="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700">
                            Tutup
                        </button>
                        ${isDryRun && data.total_with_changes > 0 ? 
                            '<button onclick="closePerincianUpdateModal(); updatePerincianData(false);" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">Update Sekarang</button>' : 
                            ''}
                    </div>
                </div>
            </div>
    ...
    window.closePerincianUpdateModal = function() {
        const modal = document.getElementById('perincian-update-result-modal');
        if (modal) {
            modal.remove();
        }
    }

    function updateNomorBl(perincianId, newValue, element) {
        // Show loading state
        element.classList.add('opacity-50');
        const saveBtn = document.querySelector(`.save-bl-btn[data-perincian-id="${perincianId}"]`);
        if (saveBtn) saveBtn.disabled = true;
        
        console.log('Sending update request for perincian:', perincianId, 'with value:', newValue);
        ...
        fetch(`/report/perincians/${perincianId}/update-nomor-bl`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
