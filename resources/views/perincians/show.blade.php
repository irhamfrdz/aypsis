...
@section('title', 'Detail Perincian')
@section('page_title', 'Detail Perincian')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6">
            ...
            <div class="mt-2 flex items-center justify-between">
                <h1 class="text-3xl font-bold text-gray-900">Detail Perincian</h1>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <p class="text-base text-gray-900">{{ $perincian->nomor_bl ?? '-' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">Prospek</label>
                    <p class="text-base text-gray-900">{{ $perincian->prospek->nama_perusahaan ?? '-' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">No. Kontainer</label>
                    <p class="text-base text-gray-900">{{ $perincian->nomor_kontainer ?? '-' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">No. Seal</label>
                    <p class="text-base text-gray-900">{{ $perincian->no_seal ?? '-' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">Tipe Kontainer</label>
                    <p class="text-base text-gray-900">{{ $perincian->tipe_kontainer ?? '-' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">Size Kontainer</label>
                    <p class="text-base text-gray-900">{{ $perincian->size_kontainer ? $perincian->size_kontainer . "'" : '-' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <p class="text-base text-gray-900">{{ $perincian->nama_kapal ?? '-' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">No. Voyage</label>
                    <p class="text-base text-gray-900">{{ $perincian->no_voyage ?? '-' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">Pelabuhan Asal</label>
                    <p class="text-base text-gray-900">{{ $perincian->pelabuhan_asal ?? '-' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">Pelabuhan Tujuan</label>
                    <p class="text-base text-gray-900">{{ $perincian->pelabuhan_tujuan ?? '-' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">Tanggal Berangkat</label>
                    <p class="text-base text-gray-900">{{ $perincian->tanggal_berangkat ? $perincian->tanggal_berangkat->format('d M Y') : '-' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">Tanggal Penerimaan</label>
                    <p class="text-base text-gray-900">{{ $perincian->penerimaan ? $perincian->penerimaan->format('d M Y') : '-' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <p class="text-base text-gray-900 font-semibold">{{ $perincian->pengirim ?? '-' }}</p>
                    @if($perincian->alamat_pengirim)
                        <p class="text-sm text-gray-600 mt-1 whitespace-pre-line">{{ $perincian->alamat_pengirim }}</p>
                    @endif
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">CONSIGNEE</label>
                    <p class="text-base text-gray-900 font-semibold">{{ $perincian->penerima ?? '-' }}</p>
                    @if($perincian->alamat_penerima)
                        <p class="text-sm text-gray-600 mt-1 whitespace-pre-line">{{ $perincian->alamat_penerima }}</p>
                    @endif
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">NOTIFY PARTY</label>
                    <p class="text-base text-gray-900 font-semibold">{{ $perincian->notify_party ?? '-' }}</p>
                    @if($perincian->alamat_notify_party)
                        <p class="text-sm text-gray-600 mt-1 whitespace-pre-line">{{ $perincian->alamat_notify_party }}</p>
                    @endif
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">Asal Kontainer</label>
                    <p class="text-base text-gray-900">{{ $perincian->asal_kontainer ?? '-' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">Ke</label>
                    <p class="text-base text-gray-900">{{ $perincian->ke ?? '-' }}</p>
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-500 mb-1">Alamat Pengiriman</label>
                    <p class="text-base text-gray-900 whitespace-pre-line">{{ $perincian->alamat_pengiriman ?? '-' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">Contact Person</label>
                    <p class="text-base text-gray-900">{{ $perincian->contact_person ?? '-' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <p class="text-base text-gray-900">{{ $perincian->createdBy->name ?? '-' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">Tanggal Dibuat</label>
                    <p class="text-base text-gray-900">{{ $perincian->created_at->format('d M Y H:i') }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">Diperbarui Oleh</label>
                    <p class="text-base text-gray-900">{{ $perincian->updatedBy->name ?? '-' }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">Tanggal Diperbarui</label>
                    <p class="text-base text-gray-900">{{ $perincian->updated_at->format('d M Y H:i') }}</p>
                </div>
            </div>
        </div>
    </div>
