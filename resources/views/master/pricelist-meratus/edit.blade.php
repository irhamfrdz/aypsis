            ...
            <form action="{{ route('master.pricelist-meratus.update', $pricelistMeratus->id) }}" method="POST">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="lokasi" id="lokasi" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('lokasi') border-red-500 @enderror" value="{{ old('lokasi', $pricelistMeratus->lokasi) }}" placeholder="Contoh: Jakarta, Surabaya, dll">
                        @error('lokasi')
                            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
