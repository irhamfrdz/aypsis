...
@section('title', 'Tambah Vendor/Bengkel')
    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h1 class="text-2xl font-bold text-gray-800">Tambah Vendor/Bengkel</h1>
                    <p class="text-gray-600 mt-1">Masukkan data vendor atau bengkel baru</p>
                </div>
            ...
            <!-- Form -->
            <form action="{{ route('master.vendor-bengkel.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea id="keterangan" name="keterangan" rows="3"
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                              placeholder="Deskripsi singkat vendor/bengkel...">{{ old('keterangan') }}</textarea>
                    @error('keterangan')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea id="catatan" name="catatan" rows="4"
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                              placeholder="Catatan tambahan, alamat, kontak, atau informasi lainnya...">{{ old('catatan') }}</textarea>
                    @error('catatan')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
