...
@section('title', 'Tambah Template WA')
@section('page_title', 'Tambah Template WhatsApp')

@section('content')
<div class="bg-white shadow-md rounded-lg p-6 font-sans max-w-4xl mx-auto">
    <div class="flex justify-between items-center mb-6 border-b pb-4">
        <h2 class="text-xl font-bold text-gray-800">Tambah Template Baru</h2>
    ...
    <form action="{{ route('master.wa-templates.store') }}" method="POST">
        @csrf
        <div class="space-y-6">
            <div>
                <label for="nama_template" class="block text-sm font-semibold text-gray-700 mb-1">Nama Template</label>
                <input type="text" name="nama_template" id="nama_template" value="{{ old('nama_template') }}" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
            </div>
