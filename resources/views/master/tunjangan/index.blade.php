...
@section('page_title', 'Master Tunjangan')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mb-8 transition-all duration-300 hover:shadow-md">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:y-0">
                <div>
                    <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Master Tunjangan</h1>
                    <p class="mt-2 text-sm text-gray-500 font-medium">Kelola daftar tunjangan</p>
                </div>
                <div class="flex flex-col sm:flex-row gap-3">
                    @can('master-tunjangan')
                    <button type="button" 
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mb-8 transition-all duration-300">
            <div class="relative max-w-2xl">
                ...
                <div class="group relative">
                    <input type="text"
                           id="searchInput"
                           placeholder="Cari berdasarkan nama tunjangan..."
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden transition-all duration-300 hover:shadow-lg">
            <div class="px-6 py-5 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
                <div>
                    <h3 class="text-lg font-bold text-gray-900">Daftar Tunjangan</h3>
                    <p class="text-xs text-gray-500 font-semibold uppercase tracking-wider mt-1">Total: <span id="itemCount">{{ count($tunjangans) }}</span> Tunjangan</p>
                </div>
                ...
                <table class="min-w-full divide-y divide-gray-100" id="itemTable">
                    <thead>
                        <tr class="bg-gray-50/50">
                            ...
                            <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest">Nama Tunjangan</th>
                        ...
                        @forelse ($tunjangans as $index => $item)
                            <tr class="item-row hover:bg-blue-50/30 transition-colors duration-150 group" 
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-bold text-gray-900 group-hover:text-blue-700 transition-colors duration-200">{{ $item->nama_tunjangan }}</div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex items-center justify-center space-x-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                        ...
                                        <form action="{{ route('master.tunjangan.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus tunjangan ini?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" 
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-20 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <h3 class="text-lg font-bold text-gray-900 mb-1">Belum ada data tunjangan</h3>
                                        <p class="text-sm text-gray-500 font-medium max-w-sm">Data tunjangan akan muncul di sini setelah Anda menambahkannya.</p>
                                        @can('master-tunjangan')
                                        <button type="button" 
                                                onclick="openCreateModal()"
...
<div id="createModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full animate-modal-up">
            <form action="{{ route('master.tunjangan.store') }}" method="POST">
                @csrf
                <div class="bg-white px-8 pt-8 pb-6">
                    <div class="flex items-center justify-between mb-6">
                        <div class="flex items-center">
                            ...
                            <h3 class="text-2xl font-extrabold text-gray-900" id="modal-title">Tambah Tunjangan Baru</h3>
                        </div>
                    ...
                    <div class="space-y-5">
                        <div>
                            <label for="nama_tunjangan" class="block text-sm font-bold text-gray-700 mb-1">Nama Tunjangan <span class="text-rose-500">*</span></label>
                            <input type="text" name="nama_tunjangan" id="nama_tunjangan" required
                                   ...
                                   placeholder="Contoh: Tunjangan Hari Raya">
                        </div>

                        <div>
                            <label for="status" class="block text-sm font-bold text-gray-700 mb-1">Status <span class="text-rose-500">*</span></label>
                ...
                <div class="bg-white px-8 pt-8 pb-6">
                    <div class="flex items-center justify-between mb-6">
                        <div class="flex items-center">
                            ...
                            <h3 class="text-2xl font-extrabold text-gray-900" id="modal-title">Edit Tunjangan</h3>
                        </div>
                    ...
                    <div class="space-y-5">
                        <div>
                            <label for="edit_nama_tunjangan" class="block text-sm font-bold text-gray-700 mb-1">Nama Tunjangan <span class="text-rose-500">*</span></label>
                            <input type="text" name="nama_tunjangan" id="edit_nama_tunjangan" required
                        ...
                        <div>
                            <label for="edit_status" class="block text-sm font-bold text-gray-700 mb-1">Status <span class="text-rose-500">*</span></label>
...
<script>
    ...
    function openEditModal(item) {
        ...
        form.action = `{{ url('master/tunjangan') }}/${item.id}`;
        
        document.getElementById('edit_nama_tunjangan').value = item.nama_tunjangan;
