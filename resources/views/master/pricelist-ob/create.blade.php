...
@section('title', 'Tambah Master Pricelist OB')
@section('page_title', 'Tambah Master Pricelist OB')
...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow-lg rounded-lg overflow-hidden mb-6">
            <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4">
                <div class="flex items-center">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-white">Tambah Master Pricelist OB</h1>
                        <p class="text-blue-100 text-sm">Buat pricelist OB baru berdasarkan size dan status kontainer</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white shadow-lg rounded-lg overflow-hidden">
            <form action="{{ route('master.pricelist-ob.store') }}" method="POST" class="p-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="size_kontainer" id="size_kontainer"
                            ...
                            @foreach($sizeOptions as $value => $label)
                                <option value="{{ $value }}" {{ old('size_kontainer') == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <!-- Status Service -->
                    <div>
                        <label for="status_service" class="block text-sm font-medium text-gray-700 mb-2">
                            Status Service <span class="text-red-500">*</span>
                        </label>
                        <select name="status_service" id="status_service"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('status_service') border-red-500 @enderror" required>
                            <option value="">Pilih Status Service</option>
                            @foreach($statusServiceOptions as $value => $label)
                                <option value="{{ $value }}" {{ old('status_service', 'non_service') == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="status_kontainer" id="status_kontainer"
                            ...
                            @foreach($statusOptions as $value => $label)
                                <option value="{{ $value }}" {{ old('status_kontainer') == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
        ...
        <div class="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div class="flex">
                ...
                <div class="ml-3">
                    <h3 class="text-sm font-medium text-blue-800">Informasi Pricelist OB</h3>
                    <div class="mt-2 text-sm text-blue-700">
                        <ul class="list-disc list-inside space-y-1">
                            <li>Setiap kombinasi size kontainer dan status kontainer harus unik</li>
                            <li>Size kontainer tersedia: 20ft dan 40ft</li>
                            <li>Status kontainer: Full (penuh) atau Empty (kosong)</li>
                            <li>Biaya akan otomatis diformat dalam tampilan sebagai mata uang Rupiah</li>
                        </ul>
                    </div>
                </div>
            </div>
