...
@section('title', 'Master Vendor Asuransi')
...
<div class="container mx-auto px-4 py-8">
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Master Vendor Asuransi</h1>
        ...
        <div class="bg-gray-50 rounded-lg p-4 mb-6">
            <form method="GET" action="{{ route('master.vendor-asuransi.index') }}" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Cari Vendor Asuransi</label>
                    <input type="text" name="search" value="{{ request('search') }}"
                           placeholder="Nama asuransi, alamat..."
                ...
                <div class="flex items-end space-x-2">
                    ...
                    <a href="{{ route('master.vendor-asuransi.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 text-sm rounded-lg font-medium transition duration-200">
                        Reset
                    </a>
                </div>
            </form>
            ...
            <table class="min-w-full bg-white border border-gray-200 rounded-lg">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Asuransi</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tarif (%)</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Telepon / Email</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Aksi</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($vendorAsuransi as $index => $vendor)
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                            {{ $vendorAsuransi->firstItem() + $index }}
                        </td>
                        <td class="px-4 py-4">
                            <div class="text-sm font-medium text-gray-900">
                                {{ $vendor->nama_asuransi }}
                            </div>
                            <div class="text-xs text-gray-500 truncate max-w-xs" title="{{ $vendor->alamat }}">
                                {{ $vendor->alamat ?: '-' }}
                            </div>
                        </td>
                        <td class="px-4 py-4">
                            <span class="text-sm font-medium text-gray-900">
                                {{ $vendor->tarif ? $vendor->tarif . '%' : '-' }}
                            </span>
                        </td>
                        <td class="px-4 py-4">
                    ...
                    <tr>
                        <td colspan="5" class="px-4 py-8 text-center text-gray-500">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-lg font-medium">Tidak ada data vendor asuransi</p>
                                <p class="text-sm">Belum ada vendor asuransi yang ditambahkan.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <!-- Pagination -->
        @if($vendorAsuransi->hasPages())
        <div class="mt-6">
            @include('components.modern-pagination', ['paginator' => $vendorAsuransi])
        </div>
        @endif
    </div>
</div>
    ...
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-medium text-gray-900">Import Vendor Asuransi</h3>
                <button onclick="document.getElementById('importModal').classList.add('hidden')" class="text-gray-400 hover:text-gray-600">
                    <span class="text-2xl">&times;</span>
                </button>
            </div>

            <form action="{{ route('master.vendor-asuransi.import') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-4">
                ...
                <div class="mb-4 p-4 bg-blue-50 rounded-lg">
                    ...
                    <div class="text-xs text-blue-700 font-mono bg-white p-2 rounded border truncate">
                        kode;nama_asuransi;alamat;telepon;email;keterangan;catatan
                    </div>
                </div>
