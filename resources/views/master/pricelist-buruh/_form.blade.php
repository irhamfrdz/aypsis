    ...
    <div>
        ...
        <div class="relative">
            ...
            <input type="text" 
                   name="full" 
                   id="full_input"
                   value="{{ old('full', isset($item) ? number_format($item->full, 0, ',', '.') : '') }}" 
                   class="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('full') border-red-500 @enderror"
                   placeholder="0">
        </div>
        @error('full')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
    ...
    <div>
        ...
        <div class="relative">
            ...
            <input type="text" 
                   name="empty" 
                   id="empty_input"
                   value="{{ old('empty', isset($item) ? number_format($item->empty, 0, ',', '.') : '') }}" 
                   class="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('empty') border-red-500 @enderror"
                   placeholder="0">
        </div>
        @error('empty')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
...
document.addEventListener('DOMContentLoaded', function() {
    const currencyInputs = ['tarif_input', 'full_input', 'empty_input'];
    ...
    currencyInputs.forEach(function(inputId) {
        ...
        if (input) {
            ...
            input.closest('form').addEventListener('submit', function(e) {
                const plainValue = input.value.replace(/\./g, '');
