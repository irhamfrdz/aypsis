    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 rounded-lg bg-white p-5 shadow">
            <form method="GET" class="grid grid-cols-1 gap-4 md:grid-cols-6">
                ...
                <select name="gudang_tujuan_id" class="rounded border-gray-300 text-sm"><option value="">Semua Gudang Tujuan</option><option value="0" @selected(request('gudang_tujuan_id') === '0')>Tarif Umum</option>@foreach($gudangs as $gudang)<option value="{{ $gudang->id }}" @selected(request('gudang_tujuan_id') == $gudang->id)>{{ $gudang->nama_gudang }}</option>@endforeach</select>
        ...
        <div class="overflow-hidden rounded-lg bg-white shadow">
            <div class="overflow-x-auto"><table class="min-w-full divide-y divide-gray-200"><thead class="bg-gray-50"><tr>
                <th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">Size</th><th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">Status Kontainer</th><th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">Service</th><th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">Gudang Tujuan</th><th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">Biaya</th><th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">Keterangan</th><th class="px-5 py-3 text-right text-xs font-medium uppercase text-gray-500">Aksi</th>
            </tr></thead><tbody class="divide-y divide-gray-200 bg-white">
            @forelse($pricelists as $pricelist)<tr><td class="px-5 py-4 text-sm">{{ $pricelist->size_kontainer_label }}</td><td class="px-5 py-4 text-sm">{{ $pricelist->status_kontainer_label ?: 'Tidak berlaku' }}</td><td class="px-5 py-4 text-sm">{{ $pricelist->status_service_label }}</td><td class="px-5 py-4 text-sm">{{ $pricelist->gudangTujuan?->nama_gudang ?? 'Semua Gudang (Umum)' }}</td><td class="px-5 py-4 text-sm font-medium">{{ $pricelist->formatted_biaya }}</td><td class="px-5 py-4 text-sm">{{ $pricelist->keterangan ?: '-' }}</td><td class="px-5 py-4 text-right text-sm">
                ...
                <tr><td colspan="7" class="px-5 py-8 text-center text-gray-500">Belum ada pricelist OB Antar Gudang.</td></tr>
            @endforelse
            </tbody></table></div>
