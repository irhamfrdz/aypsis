...
@section('page_title', 'Buat Uang Jalan Bongkaran')
...
<div class="container mx-auto px-3 py-2">
    <div class="max-w-4xl mx-auto bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 p-3 border-b border-gray-200">
            <div>
                <h1 class="text-lg font-semibold text-gray-900">Buat Uang Jalan Bongkaran</h1>
                <p class="text-xs text-gray-600 mt-0.5">Buat pembayaran uang jalan bongkaran berdasarkan surat jalan yang dipilih</p>
            </div>
                ...
                <div class="flex-1">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-5 gap-3 text-xs text-blue-700 min-w-0">
                        ...
                        <div>
                            ...
                            <div>{{ $suratJalanBongkaran->tanggal_surat_jalan ? $suratJalanBongkaran->tanggal_surat_jalan->format('d/m/Y') : '-' }}</div>
                        </div>
                        <div>
                            <span class="font-medium">Kegiatan:</span>
                            <div>{{ $suratJalanBongkaran->kegiatan }}</div>
                        </div>
                        <div>
                            <span class="font-medium">Supir:</span>
                            <div>{{ $suratJalanBongkaran->supir ?? '-' }}</div>
                        </div>
                        <div>
        ...
        <!-- Form -->
        <form action="{{ route('uang-jalan-bongkaran.store') }}" method="POST" class="p-3">
            @csrf
            @if(isset($suratJalanBongkaran))
            <input type="hidden" name="surat_jalan_bongkaran_id" value="{{ $suratJalanBongkaran->id }}">
            @endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 min-w-0">
                    ...
                    <div>
                        ...
                        <input type="text"
                               ...
                               value="{{ old('nomor_uang_jalan', $nomorUangJalan ?? null) }}"
                               readonly
                    ...
                    <div class="flex-1">
                        ...
                        <div class="text-xs text-yellow-700">
                            <div class="grid grid-cols-3 gap-3 mb-2 min-w-0">
                                ...
                                <div>
                                    <span class="font-medium">Kenek:</span> {{ $suratJalanBongkaran->kenek ?? '-' }}
                                </div>
                                <div>
                                    <span class="font-medium">Plat:</span> {{ $suratJalanBongkaran->no_plat ?? '-' }}
                                </div>
                            </div>
...
<script>
// Simple script to calculate totals - copied/adapted
function calculateTotal() {
    const jumlahUangJalan = Number(document.getElementById('jumlah_uang_jalan')?.value || 0);
    const mel = Number(document.getElementById('jumlah_mel')?.value || 0);
    const pelancar = Number(document.getElementById('jumlah_pelancar')?.value || 0);
    const kawalan = Number(document.getElementById('jumlah_kawalan')?.value || 0);
    const parkir = Number(document.getElementById('jumlah_parkir')?.value || 0);
    const penyesuaian = Number(document.getElementById('jumlah_penyesuaian')?.value || 0);

    const subtotal = jumlahUangJalan + mel + pelancar + kawalan + parkir;
    document.getElementById('subtotal').value = subtotal;
    document.getElementById('jumlah_total').value = subtotal + penyesuaian;
}
</script>
