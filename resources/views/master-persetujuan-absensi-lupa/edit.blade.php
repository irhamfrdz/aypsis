...
@section('title', 'Edit Pengajuan Lupa Absen')
@section('page_title', 'Edit Pengajuan Lupa Absen')
...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden max-w-3xl mx-auto">
        <div class="p-6 border-b border-gray-100">
            <h3 class="text-lg font-bold text-gray-800">Form Edit Pengajuan Lupa Absen</h3>
        </div>
        <div class="p-6">
            <form action="{{ route('master.persetujuan-absensi-lupa.update', $persetujuanAbsensiLupa->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <select name="karyawan_id" id="karyawan_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>
                            ...
                            @foreach($karyawans as $karyawan)
                                <option value="{{ $karyawan->id }}" {{ old('karyawan_id', $persetujuanAbsensiLupa->karyawan_id) == $karyawan->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', $persetujuanAbsensiLupa->tanggal->format('Y-m-d')) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>
                        @error('tanggal')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="time" name="waktu" id="waktu" value="{{ old('waktu', \Carbon\Carbon::parse($persetujuanAbsensiLupa->waktu)->format('H:i')) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>
                        @error('waktu')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <select name="tipe_absen" id="tipe_absen" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>
                            <option value="Masuk" {{ old('tipe_absen', $persetujuanAbsensiLupa->tipe_absen) == 'Masuk' ? 'selected' : '' }}>Masuk</option>
                            <option value="Pulang" {{ old('tipe_absen', $persetujuanAbsensiLupa->tipe_absen) == 'Pulang' ? 'selected' : '' }}>Pulang</option>
                        </select>
                        @error('tipe_absen')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <textarea name="alasan" id="alasan" rows="3" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>{{ old('alasan', $persetujuanAbsensiLupa->alasan) }}</textarea>
                        @error('alasan')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
