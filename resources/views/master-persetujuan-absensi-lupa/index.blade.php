...
@section('title', 'Persetujuan Absensi Lupa')
@section('page_title', 'Persetujuan Absensi Lupa')
...
<div class="bg-white rounded-lg border border-gray-200 p-6 mb-6 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <div>
        <h1 class="text-xl font-bold text-gray-900 leading-tight flex items-center gap-2">
            <i class="fa-solid fa-clock-rotate-left text-blue-500"></i> Daftar Pengajuan Lupa Absen
        </h1>
        <p class="text-xs text-gray-500 mt-1">Kelola dan tinjau permohonan karyawan yang lupa melakukan absensi masuk atau keluar.</p>
    </div>
    
    @if(auth()->user()->can('approval-absensi-lupa-create'))
    <a href="{{ route('master.persetujuan-absensi-lupa.create') }}" class="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold shadow-sm transition-colors flex items-center gap-2">
        ...
        <div class="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md w-full border border-gray-100">
            <form id="rejectForm" method="POST" action="">
                ...
                <div class="bg-white px-6 pt-6 pb-4">
                    ...
                    <div class="mt-2">
                        <label for="catatan_admin" class="block text-sm font-medium text-gray-700 mb-2">Alasan Penolakan <span class="text-gray-400 font-normal">(opsional)</span></label>
                        <textarea id="catatan_admin" name="catatan_admin" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-red-500 focus:border-red-500 text-sm transition-colors" rows="3" placeholder="Masukkan alasan kenapa pengajuan ditolak..."></textarea>
                    </div>
                </div>
    ...
    <script>
        $(document).ready(function() {
            var table = $('#dataTable').DataTable({
                processing: true,
                serverSide: false,
                ajax: "{{ route('master.persetujuan-absensi-lupa.index') }}",
                columns: [
                    ...
                    {data: 'waktu_format', name: 'waktu'},
                    {data: 'tipe_absen', name: 'tipe_absen', render: function(data) {
                        if(data === 'Masuk') return '<span class="px-2 py-1 bg-blue-50 text-blue-700 rounded-md text-xs font-semibold">Masuk</span>';
                        if(data === 'Pulang') return '<span class="px-2 py-1 bg-indigo-50 text-indigo-700 rounded-md text-xs font-semibold">Pulang</span>';
                        return data;
                    }},
                ...
                language: {
                    ...
                    infoEmpty: "",
                    emptyTable: "Belum ada pengajuan lupa absen.",
                    paginate: {
                        previous: "Sebelumnya",
                        next: "Selanjutnya"
                    }
