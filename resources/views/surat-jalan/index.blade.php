                                        ...
                                        <div class="py-1">
                                            ...
                                            @can('tagihan-supir-vendor-create')
                                                @if(!\App\Models\TagihanSupirVendor::where('surat_jalan_id', $suratJalan->id)->exists())
                                                    <form action="{{ route('tagihan-supir-vendor.store') }}" method="POST" class="inline w-full">
                                                        @csrf
                                                        <input type="hidden" name="surat_jalan_id" value="{{ $suratJalan->id }}">
                                                        <button type="submit"
                                                            onclick="event.stopPropagation(); return confirm('Buat Tagihan Vendor untuk Surat Jalan ini?')"
