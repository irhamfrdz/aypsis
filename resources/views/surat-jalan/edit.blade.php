        ...
        <form action="{{ route('surat-jalan.update', $suratJalan->id) }}" method="POST" enctype="multipart/form-data" class="p-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="date"
                           name="tanggal_muat"
                           value="{{ old('tanggal_muat', $suratJalan->tanggal_muat ? \Carbon\Carbon::parse($suratJalan->tanggal_muat)->format('Y-m-d') : '') }}"
                ...
                <div>
                        ...
                        @if(isset($keneks))
                            @foreach($keneks as $kenek)
                                <option value="{{ $kenek->nama_lengkap }}"
                                        {{ old('kenek', $suratJalan->kenek) == $kenek->nama_lengkap ? 'selected' : '' }}>
                                    {{ $kenek->nama_lengkap }}
                                </option>
                            @endforeach
                        @endif
                    ...
                    @error('kenek')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="text-xs text-gray-500 mt-1">Data kenek diambil dari master karyawan divisi kenek</p>
                </div>
