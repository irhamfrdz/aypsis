...
@section('title', 'Edit Pricelist CAT')
@section('page_title', 'Edit Pricelist CAT')

@section('content')
<div class="bg-white p-6 rounded-lg shadow-md">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Edit Pricelist CAT</h2>
    ...
    <form action="{{ route('master.pricelist-cat.update', $pricelistCat) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" id="vendor" name="vendor"
                       value="{{ old('vendor', $pricelistCat->vendor) }}"
            ...
            <div>
                ...
                <select id="jenis_cat" name="jenis_cat"
                    ...
                    <option value="cat_sebagian" {{ old('jenis_cat', $pricelistCat->jenis_cat) == 'cat_sebagian' ? 'selected' : '' }}>Cat Sebagian</option>
                    <option value="cat_full" {{ old('jenis_cat', $pricelistCat->jenis_cat) == 'cat_full' ? 'selected' : '' }}>Cat Full</option>
                </select>
                @error('jenis_cat')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select id="ukuran_kontainer" name="ukuran_kontainer"
                    ...
                    <option value="20ft" {{ old('ukuran_kontainer', $pricelistCat->ukuran_kontainer) == '20ft' ? 'selected' : '' }}>20ft</option>
                    <option value="40ft" {{ old('ukuran_kontainer', $pricelistCat->ukuran_kontainer) == '40ft' ? 'selected' : '' }}>40ft</option>
                    <option value="40ft HC" {{ old('ukuran_kontainer', $pricelistCat->ukuran_kontainer) == '40ft HC' ? 'selected' : '' }}>40ft HC</option>
                </select>
