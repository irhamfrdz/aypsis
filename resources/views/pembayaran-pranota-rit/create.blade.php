...
@section('content')
    <div class="bg-white shadow-lg rounded-lg p-4 max-w-6xl mx-auto">
        ...
        <div class="bg-gray-50 rounded-lg p-3 mb-3 border border-gray-200">
            <form action="{{ route('pembayaran-pranota-rit.create') }}" method="GET" class="flex flex-col sm:flex-row gap-2 items-end">
                ...
                <div class="flex gap-1">
                    ...
                    <a href="{{ route('pembayaran-pranota-rit.create') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded text-sm hover:bg-gray-300">Reset</a>
                </div>
            </form>
        </div>

        <form id="pembayaranForm" action="{{ route('pembayaran-pranota-rit.store') }}" method="POST" class="space-y-4">
            @csrf
            ...
            <div class="border rounded-lg overflow-hidden">
                <div class="bg-gray-100 px-4 py-2 border-b flex justify-between items-center">
                    <h4 class="text-sm font-semibold">Pilih Pranota Rit</h4>
                            ...
                            @forelse($pranotaRits as $pranota)
                                ...
                                <tr class="hover:bg-gray-50 cursor-pointer" onclick="toggleRow(this)">
                                    <td class="px-4 py-2" onclick="event.stopPropagation()">
                                        <input type="checkbox" name="pranota_uang_rit_ids[]" value="{{ $pranota->id }}" 
            ...
            <div class="flex justify-end gap-3 pt-4 border-t">
                <a href="{{ route('pembayaran-pranota-rit.index') }}" class="px-6 py-2 border rounded text-gray-600 hover:bg-gray-100 transition-colors">Batal</a>
    ...
    <script>
        ...
        document.getElementById('generateNomorBtn').addEventListener('click', function() {
            const btn = this;
            btn.disabled = true;
            fetch('{{ route("pembayaran-pranota-rit.generate-nomor") }}')
                .then(r => r.json())
                .then(data => {
                    if(data.success) {
