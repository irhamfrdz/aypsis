...
@section('title', 'Edit Tagihan Pelindo')
@section('page_title', 'Edit Tagihan Pelindo')

@section('content')
<div class="container mx-auto px-4 py-8 max-w-6xl">
    <div class="mb-6 flex justify-between items-center">
        <h1 class="text-xl font-bold text-gray-800">Form Edit Tagihan Pelindo</h1>
    ...
    <form action="{{ route('tagihan-pelindo.update', $tagihan->id) }}" method="POST" id="invoiceForm">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white p-6 rounded-xl border border-gray-100 shadow-sm mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <input type="text" name="nomor_tagihan" value="{{ old('nomor_tagihan', $tagihan->nomor_tagihan) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm" required>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1">Tanggal Tagihan <span class="text-red-500">*</span></label>
                    <input type="date" name="tanggal_tagihan" value="{{ old('tanggal_tagihan', $tagihan->tanggal_tagihan ? $tagihan->tanggal_tagihan->format('Y-m-d') : '') }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm" required>
                </div>
                <div>
                    ...
                    <select name="kapal" id="kapal_select" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm bg-white" onchange="onKapalChange()">
                        ...
                        @foreach($bls->pluck('nama_kapal')->unique() as $kapal)
                            <option value="{{ $kapal }}" {{ old('kapal', $tagihan->kapal) == $kapal ? 'selected' : '' }}>{{ $kapal }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="voyage" id="voyage_select" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm bg-white">
                        ...
                        @foreach($bls->unique('no_voyage') as $bl)
                            <option value="{{ $bl->no_voyage }}" data-kapal="{{ $bl->nama_kapal }}" {{ old('voyage', $tagihan->voyage) == $bl->no_voyage ? 'selected' : '' }}>{{ $bl->no_voyage }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div id="tanggal_bayar_container" class="{{ old('status_pembayaran', $tagihan->status_pembayaran) == 'Lunas' ? '' : 'hidden' }}">
                    <label class="block text-xs font-semibold text-gray-600 mb-1">Tanggal Bayar <span class="text-red-500">*</span></label>
                    <input type="date" name="tanggal_bayar" id="tanggal_bayar" value="{{ old('tanggal_bayar', $tagihan->tanggal_bayar ? $tagihan->tanggal_bayar->format('Y-m-d') : '') }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">
                </div>
            </div>
            <div class="mt-4">
                <label class="block text-xs font-semibold text-gray-600 mb-1">Keterangan Tambahan</label>
                <textarea name="keterangan" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm" placeholder="Catatan tambahan...">{{ old('keterangan', $tagihan->keterangan) }}</textarea>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden mb-6">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200" id="itemsTable">
                    ...
                    <tbody class="divide-y divide-gray-100" id="itemsTableBody">
                        ...
                        <tr id="row_{{ $idx }}" class="hover:bg-gray-50/50 transition-colors">
                            <td class="px-4 py-2 text-center text-sm font-medium text-gray-400 row-number">{{ $idx + 1 }}</td>
                            <td class="px-4 py-2">
                                <input type="text" name="items[{{ $idx }}][nomor_kontainer]" value="{{ $item->nomor_kontainer }}" class="w-full px-2 py-1.5 border border-gray-300 rounded text-xs focus:ring-1 focus:ring-blue-500 focus:border-blue-500" placeholder="Kontainer No.">
                            </td>
                            <td class="px-4 py-2">
                                <select name="items[{{ $idx }}][pricelist_pelindo_id]" class="w-full px-2 py-1.5 border border-gray-300 rounded text-xs focus:ring-1 focus:ring-blue-500 focus:border-blue-500 bg-white" onchange="onKegiatanChange(this, {{ $idx }})">
                                    <option value="">-- Pilih Kegiatan --</option>
                                    @foreach($pricelists as $pl)
                                        <option value="{{ $pl->id }}" data-tarif="{{ $pl->tarif }}" data-ukuran="{{ $pl->ukuran }}" data-kegiatan="{{ $pl->kegiatan }}" data-status_kontainer="{{ $pl->status_kontainer || '' }}" @if($item->pricelist_pelindo_id == $pl->id) selected @endif>
                                            {{ $pl->kegiatan }} ({{ $pl->ukuran }}ft) @if($pl->status_kontainer) [{{ $pl->status_kontainer }}] @endif - Rp {{ number_format($pl->tarif, 0, ',', '.') }}
                                        </option>
                                    @endforeach
                                </select>
                                <input type="hidden" name="items[{{ $idx }}][kegiatan]" value="{{ $item->kegiatan }}" id="kegiatan_{{ $idx }}">
                            </td>
                            <td class="px-4 py-2 text-center">
                                <input type="text" name="items[{{ $idx }}][ukuran]" value="{{ $item->ukuran }}" id="ukuran_{{ $idx }}" class="w-full px-2 py-1.5 border border-gray-300 rounded text-xs text-center bg-gray-50" readonly placeholder="-">
                            </td>
                            <td class="px-4 py-2">
                                <select name="items[{{ $idx }}][status_kontainer]" id="status_kontainer_{{ $idx }}" class="w-full px-2 py-1.5 border border-gray-300 rounded text-xs focus:ring-1 focus:ring-blue-500 focus:border-blue-500 bg-white">
                                    <option value="">-</option>
                                    <option value="Full" {{ $item->status_kontainer == 'Full' ? 'selected' : '' }}>Full</option>
                                    <option value="Empty" {{ $item->status_kontainer == 'Empty' ? 'selected' : '' }}>Empty</option>
                                </select>
                            </td>
                            <td class="px-4 py-2">
                                <input type="number" step="0.01" name="items[{{ $idx }}][tarif]" value="{{ $item->tarif }}" id="tarif_{{ $idx }}" class="w-full px-2 py-1.5 border border-gray-300 rounded text-xs text-right focus:ring-1 focus:ring-blue-500 focus:border-blue-500" oninput="calculateRowTotal({{ $idx }})">
                            </td>
                            <td class="px-4 py-2">
                                <input type="number" name="items[{{ $idx }}][jumlah]" value="{{ $item->jumlah }}" id="jumlah_{{ $idx }}" class="w-full px-2 py-1.5 border border-gray-300 rounded text-xs text-center focus:ring-1 focus:ring-blue-500 focus:border-blue-500" min="1" oninput="calculateRowTotal({{ $idx }})">
                            </td>
                            <td class="px-4 py-2">
                                <input type="text" id="total_display_{{ $idx }}" class="w-full px-2 py-1.5 border border-gray-200 rounded text-xs text-right bg-gray-50 font-semibold text-gray-700" value="Rp {{ number_format($item->total, 2, ',', '.') }}" readonly>
                                <input type="hidden" id="total_val_{{ $idx }}" class="row-total" value="{{ $item->total }}">
                            </td>
                            <td class="px-4 py-2">
                                <input type="text" name="items[{{ $idx }}][keterangan]" value="{{ $item->keterangan }}" class="w-full px-2 py-1.5 border border-gray-300 rounded text-xs focus:ring-1 focus:ring-blue-500 focus:border-blue-500" placeholder="Keterangan item...">
                            </td>
                            <td class="px-4 py-2 text-center">
                                <button type="button" onclick="removeRow({{ $idx }})" class="p-1.5 text-red-500 hover:text-red-700 hover:bg-red-50 rounded transition duration-150">
            ...
            <div class="p-6 bg-gray-50 border-t border-gray-100 flex flex-col md:flex-row justify-between items-end md:items-center gap-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-xl font-black text-indigo-700" id="grandTotalDisplay">Rp {{ number_format($tagihan->total_tagihan, 2, ',', '.') }}</span>
                </div>
            </div>
        </div>
...
@push('scripts')
<script>
    let rowIndex = {{ count($tagihan->items) }};
    ...
    // Kegiatan dropdown onchange handler
