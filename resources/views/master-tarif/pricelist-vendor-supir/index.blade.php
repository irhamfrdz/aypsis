...
@section('page_title', 'Master Pricelist Vendor Supir')
...
<div class="container mx-auto px-4 py-6">
    <div class="mb-6">
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Pricelist Vendor Supir</h1>
                <p class="text-gray-600 mt-1">Kelola data harga vendor supir berdasarkan tujuan dan jenis kontainer</p>
            </div>
            @can('master-pricelist-vendor-supir-create')
            <a href="{{ route('pricelist-vendor-supir.create') }}" 
    ...
    <div class="bg-white rounded-lg shadow p-4 mb-6">
        <form action="{{ route('pricelist-vendor-supir.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
                <label for="tujuan_id" class="block text-sm font-medium text-gray-700 mb-1">Tujuan</label>
                <select name="tujuan_id" id="tujuan_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200">
                    <option value="">Semua Tujuan</option>
                    @foreach($tujuans as $tujuan)
                        <option value="{{ $tujuan->id }}" {{ request('tujuan_id') == $tujuan->id ? 'selected' : '' }}>
                            {{ $tujuan->nama_tujuan }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="jenis_kontainer" id="jenis_kontainer" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200">
                    ...
                    <option value="40" {{ request('jenis_kontainer') == '40' ? 'selected' : '' }}>40 Feet</option>
                    <option value="45" {{ request('jenis_kontainer') == '45' ? 'selected' : '' }}>45 Feet</option>
                </select>
            </div>
            <div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">
                            {{ $pricelist->tujuan->nama_tujuan ?? '-' }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium text-gray-900">
                            {{ $pricelist->jenis_kontainer }} Feet
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-right font-medium text-gray-900">
                            Rp {{ number_format($pricelist->nominal, 0, ',', '.') }}
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                            <div class="flex justify-center gap-2">
                                @can('master-pricelist-vendor-supir-update')
                                <a href="{{ route('pricelist-vendor-supir.edit', $pricelist) }}" 
                                ...
                                @can('master-pricelist-vendor-supir-delete')
                                <form action="{{ route('pricelist-vendor-supir.destroy', $pricelist) }}" 
                                      method="POST" 
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-8 text-center text-gray-500">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-sm text-gray-500">Tambah data baru untuk memulai.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
