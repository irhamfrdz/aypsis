    ...
    <div class="max-w-7xl mx-auto bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 p-4 border-b border-gray-200">
            ...
            <div class="flex gap-4 text-sm">
                ...
                <div class="flex items-center gap-2">
                    @can('tanda-terima-tanpa-surat-jalan-batam-update')
                    ...
                    @endcan
                    @can('tanda-terima-tanpa-surat-jalan-batam-view')
                        <a href="{{ route('tanda-terima-tanpa-surat-jalan-batam.export', request()->query()) }}" class="inline-flex items-center px-3 py-1 bg-green-600 text-white rounded-lg hover:bg-green-700 transition shadow text-sm">
        ...
        <div class="p-4">
            ...
            <div class="mb-6 bg-gray-50 p-4 rounded-lg">
                <form method="GET" action="{{ route('tanda-terima-tanpa-surat-jalan-batam.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                    ...
                    <div class="md:col-span-4 flex gap-2">
                        ...
                        <a href="{{ route('tanda-terima-tanpa-surat-jalan-batam.index') }}" class="px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500 text-sm">
                            Reset
                        </a>
                        @can('tanda-terima-tanpa-surat-jalan-batam-create')
                            <a href="{{ route('tanda-terima-tanpa-surat-jalan-batam.pilih-tipe') }}" class="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 text-sm ml-auto">
                                <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div id="selectedActions" class="mb-4 p-4 bg-gradient-to-r from-blue-50 to-teal-50 border-2 border-blue-300 rounded-lg shadow-sm" style="display: none;">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                            ...
                            <div class="flex flex-wrap gap-2">
                                ...
                                @can('tanda-terima-tanpa-surat-jalan-batam-delete')
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($tandaTerimas as $tandaTerima)
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if(isset($isLclData) && $isLclData)
                                            <div class="text-sm text-gray-900 max-w-xs truncate" title="{{ $tandaTerima->nama_penerima ?? '-' }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if(isset($isLclData) && $isLclData)
                                            <div class="text-sm text-gray-900 max-w-xs truncate" title="{{ $tandaTerima->nama_pengirim ?? '-' }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if(isset($isLclData) && $isLclData)
                                            <div class="text-sm text-gray-900 max-w-xs truncate" title="{{ $tandaTerima->alamat_pengirim ?? '-' }}">
                                                {{ Str::limit($tandaTerima->alamat_pengirim ?? '-', 30) }}
                                            </div>
                                        @else
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <div class="flex justify-center gap-2">
                                            @if(isset($isLclData) && $isLclData)
                                                ...
                                                @endcan
                                            @else
                                                <a href="{{ route('tanda-terima-tanpa-surat-jalan-batam.show', $tandaTerima) }}"
                                                   class="text-indigo-600 hover:text-indigo-900" title="Lihat Detail">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                                ...
                                                @can('tanda-terima-tanpa-surat-jalan-batam-update')
                                                    <a href="{{ route('tanda-terima-tanpa-surat-jalan-batam.edit', $tandaTerima) }}"
                                                       class="text-yellow-600 hover:text-yellow-900" title="Edit">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                                        </svg>
                                                    ...
                                                    <form action="{{ route('tanda-terima-tanpa-surat-jalan-batam.add-to-prospek', $tandaTerima) }}" method="POST" class="inline" onsubmit="return confirm('Kirim data ini ke Prospek?')">
                                                        @csrf
                                                        <button type="submit" class="text-purple-600 hover:text-purple-900" title="Kirim ke Prospek">
                                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path>
                                                    ...
                                                    <form action="{{ route('tanda-terima-tanpa-surat-jalan-batam.sync-penerima-pengirim', $tandaTerima->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin melakukan sinkronisasi data penerima dan pengirim ke Prospek, Naik Kapal, Manifest, dan BL?')">
                                                        @csrf
                                                        <button type="submit" class="text-blue-600 hover:text-blue-900" title="Sync Penerima & Pengirim">
                                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
                                                ...
                                                @endcan
                                                @can('tanda-terima-tanpa-surat-jalan-batam-delete')
                                                    <form action="{{ route('tanda-terima-tanpa-surat-jalan-batam.destroy', $tandaTerima) }}" method="POST" class="inline"
            ...
            @else
                <div class="text-center py-8">
                    <div class="text-gray-400">
                        ...
                        @can('tanda-terima-tanpa-surat-jalan-batam-create')
                            <a href="{{ route('tanda-terima-tanpa-surat-jalan-batam.pilih-tipe') }}"
...
<div id="assignContainerModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-lg bg-white">
        <div class="mt-3">
            ...
            <form id="assignContainerForm" method="POST" action="{{ route('tanda-terima-tanpa-surat-jalan-batam.assign-container') }}">
                @csrf
                ...
                if (confirm(`Apakah Anda yakin ingin menghapus ${selectedIds.length} tanda terima yang dipilih?`)) {
                    ...
                    @else
                        ...
                        selectedIds.forEach(id => {
                            const form = document.createElement('form');
                            form.method = 'POST';
                            form.action = `{{ route('tanda-terima-tanpa-surat-jalan-batam.index', [], false) }}/${id}`;
