                    ...
                    <div class="space-y-6">
                        ...
                        <div class="bg-blue-50 p-4 rounded-lg border border-blue-200 mt-6 mb-6">
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                ...
                                <div class="md:col-span-2">
                                    <label class="block text-xs font-medium text-gray-500 mb-2">
                                        Gambar Bukti Timbangan Muat Saat Ini
                                    </label>
                                    @php
                                        // Check if it's a JSON array (multiple images) or single image path
                                        ...
                                        try {
                                            if (empty($buktiTimbanganMuat)) {
                                                $buktiTimbanganMuatPaths = [];
                                            } elseif (is_array($buktiTimbanganMuat)) {
                                                $buktiTimbanganMuatPaths = array_filter($buktiTimbanganMuat);
                                            } elseif (is_string($buktiTimbanganMuat)) {
                                                // Try to decode as JSON
                                                $decoded = json_decode($buktiTimbanganMuat, true);
                                                if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                                                    $buktiTimbanganMuatPaths = array_filter($decoded);
                                                } else {
                                                    // Single path
                                                    $buktiTimbanganMuatPaths = [$buktiTimbanganMuat];
                                                }
                                            }
                                        } catch (\Exception $e) {
                                            \Log::error('Error parsing bukti_timbangan_muat: ' . $e->getMessage());
                                            $buktiTimbanganMuatPaths = [];
                                        }
                                    @endphp
                                    @if(count($buktiTimbanganMuatPaths) > 0)
                                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                        @foreach($buktiTimbanganMuatPaths as $index => $imagePath)
                                        @if(!empty($imagePath) && is_string($imagePath))
                                        <div class="flex items-start gap-2 bg-gray-50 p-3 rounded-lg border border-gray-200">
                                            <a href="{{ asset('storage/' . $imagePath) }}" 
                                               target="_blank" 
                                               class="group relative block overflow-hidden rounded-lg border-2 border-gray-200 hover:border-blue-400 transition-all flex-shrink-0">
                                                ...
                                                @if($isImage)
                                                    <img src="{{ asset('storage/' . $imagePath) }}" 
                                                         alt="Gambar Bukti Timbangan Muat {{ $index + 1 }}" 
                                                         class="w-24 h-24 object-cover group-hover:scale-105 transition-transform">
                                                    <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-30 flex items-center justify-center transition-all">
                                                        <svg class="w-6 h-6 text-white opacity-0 group-hover:opacity-100 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7"></path>
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <p class="text-xs font-medium text-gray-700 mb-1">Bukti Timbangan Muat {{ $index + 1 }}</p>
                                                <div class="flex flex-col gap-1">
                                                    <a href="{{ asset('storage/' . $imagePath) }}" 
                                                       target="_blank" 
                                                       class="inline-flex items-center px-2 py-1 text-xs font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 rounded transition">
                                    ...
                                    <p class="text-xs text-gray-500 mt-2">
                                        <i class="fas fa-camera mr-1"></i>
                                        {{ count($buktiTimbanganMuatPaths) }} foto bukti timbangan muat diupload saat checkpoint
                                    </p>
                                    @endif
                                </div>
                                @endif
                                ...
                                <div class="md:col-span-2">
                                    <label class="block text-xs font-medium text-gray-500 mb-2">
                                        Gambar Bukti Timbang Kosong / Lainnya Saat Ini
                                    </label>
                                    @php
                                        // Check if it's a JSON array (multiple images) or single image path
                                        ...
                                        try {
                                            if (empty($buktiTimbangan)) {
                                                $buktiTimbanganPaths = [];
                                            } elseif (is_array($buktiTimbangan)) {
                                                $buktiTimbanganPaths = array_filter($buktiTimbangan);
                                            } elseif (is_string($buktiTimbangan)) {
                                                // Try to decode as JSON
                                                $decoded = json_decode($buktiTimbangan, true);
                                                if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                                                    $buktiTimbanganPaths = array_filter($decoded);
                                                } else {
                                                    // Single path
                                                    $buktiTimbanganPaths = [$buktiTimbangan];
                                                }
                                            }
                                        } catch (\Exception $e) {
                                            \Log::error('Error parsing bukti_timbangan: ' . $e->getMessage());
                                            $buktiTimbanganPaths = [];
                                        }
                                    @endphp
                                    @if(count($buktiTimbanganPaths) > 0)
                                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                        @foreach($buktiTimbanganPaths as $index => $imagePath)
                                        @if(!empty($imagePath) && is_string($imagePath))
                                        <div class="flex items-start gap-2 bg-gray-50 p-3 rounded-lg border border-gray-200">
                                            <a href="{{ asset('storage/' . $imagePath) }}" 
                                               target="_blank" 
                                               class="group relative block overflow-hidden rounded-lg border-2 border-gray-200 hover:border-blue-400 transition-all flex-shrink-0">
                                                ...
                                                @if($isImage)
                                                    <img src="{{ asset('storage/' . $imagePath) }}" 
                                                         alt="Gambar Bukti Timbang {{ $index + 1 }}" 
                                                         class="w-24 h-24 object-cover group-hover:scale-105 transition-transform">
                                                    <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-30 flex items-center justify-center transition-all">
                                                        <svg class="w-6 h-6 text-white opacity-0 group-hover:opacity-100 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7"></path>
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <p class="text-xs font-medium text-gray-700 mb-1">Bukti Timbang {{ $index + 1 }}</p>
                                                <div class="flex flex-col gap-1">
                                                    <a href="{{ asset('storage/' . $imagePath) }}" 
                                                       target="_blank" 
                                                       class="inline-flex items-center px-2 py-1 text-xs font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 rounded transition">
                                    ...
                                    <p class="text-xs text-gray-500 mt-2">
                                        <i class="fas fa-camera mr-1"></i>
                                        {{ count($buktiTimbanganPaths) }} foto bukti timbang kosong / lainnya diupload saat checkpoint
                                    </p>
                                    @endif
                                </div>
                                @endif
                                    ...
                                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md hover:border-blue-400 transition-colors upload-dropzone">
                                        <div class="space-y-1 text-center">
                                            ...
                                            <p class="text-xs text-gray-500">
                                                PNG, JPG, JPEG, GIF, WEBP, PDF sampai 10MB per file
                                            </p>
                                        </div>
                                    </div>
