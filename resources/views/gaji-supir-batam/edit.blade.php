...
@section('title', 'Ubah Gaji Supir Batam')
@section('page_title', 'Ubah Gaji Supir Batam')
...
<div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden max-w-4xl">
    ...
    <form method="POST" action="{{ route('gaji-supir-batam.update', $gaji->id) }}" class="p-6">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <select name="karyawan_id" id="karyawan_id" class="block w-full py-2.5 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 text-sm @error('karyawan_id') border-red-500 @enderror" required>
                    ...
                    @foreach($supirList as $s)
                        <option value="{{ $s->id }}" {{ old('karyawan_id', $gaji->karyawan_id) == $s->id ? 'selected' : '' }}>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                <select name="status_pembayaran" id="status_pembayaran" class="block w-full py-2.5 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 text-sm" required>
                    ...
                    <option value="PAID" {{ old('status_pembayaran', $gaji->status_pembayaran) == 'PAID' ? 'selected' : '' }}>PAID (SUDAH DIBAYAR)</option>
                    <option value="CANCELLED" {{ old('status_pembayaran', $gaji->status_pembayaran) == 'CANCELLED' ? 'selected' : '' }}>CANCELLED (BATAL)</option>
                </select>
            </div>
            ...
            <div id="payment_date_wrapper" class="hidden">
                ...
                <input type="date" name="tanggal_dibayar" id="tanggal_dibayar" value="{{ old('tanggal_dibayar', $gaji->tanggal_dibayar ? $gaji->tanggal_dibayar->format('Y-m-d') : date('Y-m-d')) }}" class="block w-full py-2 px-3 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500 text-sm">
            </div>
        </div>
        ...
        <div class="mb-6">
            ...
            <textarea name="keterangan" id="keterangan" rows="3" placeholder="Tambahkan catatan jika ada..." class="block w-full py-2 px-3 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500 text-sm">{{ old('keterangan', $gaji->keterangan) }}</textarea>
        </div>
