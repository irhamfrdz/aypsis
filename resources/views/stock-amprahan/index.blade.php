                                                    ...
                                                    @if($alatBerats->count() > 0)
                                                        ...
                                                        @foreach($alatBerats as $alat)
                                                            <div class="mobil-option cursor-pointer select-none relative py-2.5 pl-4 pr-9 hover:bg-blue-50 text-gray-900 transition-colors duration-150 border-b border-gray-50 last:border-0" 
                                                                 ...
                                                                 onclick="selectMobil('alat-{{ $alat->id }}', '{{ $alat->kode_alat }} - {{ $alat->nama }} ({{ $alat->jenis ?? 'Tidak ada jenis' }}, {{ $alat->merk ?? 'Tidak ada merk' }}, {{ $alat->warna ?? 'Tidak ada warna' }}, {{ $alat->lokasi ?? 'Tidak ada lokasi' }})')">
                                                                <span class="block truncate font-medium">{{ $alat->kode_alat }} - {{ $alat->nama }} <span class="text-sm text-gray-500">({{ $alat->jenis ?? 'Tidak ada jenis' }}, {{ $alat->merk ?? 'Tidak ada merk' }}, {{ $alat->warna ?? 'Tidak ada warna' }}, {{ $alat->lokasi ?? 'Tidak ada lokasi' }})</span></span>
                                                            </div>
                                                        @endforeach
                                                    @endif
