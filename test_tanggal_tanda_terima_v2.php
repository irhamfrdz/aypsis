...
echo "=== Test Tanggal Tanda Terima Pada Approval (Updated) ===\n\n";

// Test dengan approval yang baru
...
if (!$approval) {
    ...
    $suratJalan = \App\Models\SuratJalan::create([
        'order_id' => $order->id,
        'no_surat_jalan' => 'TEST-DATETIME-' . date('YmdHis'),
...
// Test proses approval dengan datetime yang tepat
echo "2. Melakukan approval dengan datetime yang tepat...\n";

$approvalTime = now();
echo "   Waktu approval: {$approvalTime}\n";
...
$approval->update([
    ...
    'approval_notes' => 'Test approval dengan datetime'
]);

// Update tanggal_tanda_terima dengan waktu yang sama
$suratJalan->update([
    'tanggal_tanda_terima' => $approvalTime
]);
...
$approvedAtCarbon = \Carbon\Carbon::parse($approval->approved_at);
$tandaTerimaCarbon = \Carbon\Carbon::parse($suratJalan->tanggal_tanda_terima);
$diffSeconds = abs($approvedAtCarbon->diffInSeconds($tandaTerimaCarbon));

echo "   - Selisih waktu: {$diffSeconds} detik\n";

if ($diffSeconds < 2) {
    echo "   ✅ SUCCESS: Tanggal tanda terima berhasil diisi otomatis saat approval!\n";
} else {
    echo "   ⚠️ Masih ada selisih waktu yang cukup besar\n";
}

echo "\n4. Test casting datetime...\n";
...
echo "   - Format ISO: " . $dbRecord->tanggal_tanda_terima->toISOString() . "\n";

echo "\n=== Test Selesai ===\n";
echo "💡 Implementasi berhasil! Saat approval surat jalan, tanggal_tanda_terima akan diisi otomatis.\n";
