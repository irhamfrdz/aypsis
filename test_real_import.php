...
while (($row = fgetcsv($handle, 1000, $delimiter)) !== false) {
    ...
    try {
        ...
        // Determine tarif per hari
        $tarifPerHari = ($vendor === 'DPE')
            ? (($size == '20') ? 25000 : 35000)
            : (($size == '20') ? 20000 : 30000);

        // Calculate financial data
        $dpp = $tarifPerHari * $jumlahHari;
        $ppn = $dpp * 0.11;
        $pph = $dpp * 0.02;
        $grand_total = $dpp + $ppn - $pph;

        echo "Baris $rowNumber: $vendor - $nomor_kontainer ($size ft) - $jumlahHari hari - Rp " . number_format($dpp, 0, ',', '.') . "\n";
...
foreach ($samples as $sample) {
    echo "- {$sample->vendor} - {$sample->nomor_kontainer} ({$sample->size}ft) - {$sample->periode} hari - Rp " . number_format($sample->dpp, 0, ',', '.') . "\n";
}

echo "\nTotal records in database: " . DaftarTagihanKontainerSewa::count() . "\n";
