...
function cost(price, type) {
    const fields = {};
    for (const key of ['price-input-temas', 'quantity-input-temas', 'temas-row-number', 'temas-row-nomor-bl', 'temas-row-bl', 'temas-row-size', 'type-select-temas']) {
        fields['.' + key] = input();
    }
...
console.log('PASS: totals, taxes, adjustment, BL association, duplicate and tariff-size validation');
