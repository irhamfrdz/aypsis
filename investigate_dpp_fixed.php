...
if ($masterPricelistHarian) {
    echo "Harian: Rp " . number_format((float)$masterPricelistHarian->harga_sewa, 0, ',', '.') . "\n";
} else {
    echo "❌ Master pricelist harian tidak ditemukan!\n";
}

if ($masterPricelistBulanan) {
    echo "Bulanan: Rp " . number_format((float)$masterPricelistBulanan->harga_sewa, 0, ',', '.') . "\n";
} else {
...
if ($tagihan->tarif === 'Harian' && $masterPricelistHarian) {
    $correctDPP = (float)$masterPricelistHarian->harga_sewa * $days;
    echo "Tarif Harian: Rp " . number_format((float)$masterPricelistHarian->harga_sewa, 0, ',', '.') . " × {$days} hari\n";
    echo "DPP Seharusnya: Rp " . number_format($correctDPP, 0, ',', '.') . "\n";
    echo "DPP Saat Ini: Rp " . number_format((float)$tagihan->dpp, 0, ',', '.') . "\n";
    echo "Selisih: Rp " . number_format($correctDPP - (float)$tagihan->dpp, 0, ',', '.') . "\n";

    if ($correctDPP != (float)$tagihan->dpp) {
        echo "❌ DPP TIDAK SESUAI!\n";
    } else {
        echo "✅ DPP SUDAH BENAR!\n";
    }
} elseif ($tagihan->tarif === 'Bulanan' && $masterPricelistBulanan) {
    $correctDPP = (float)$masterPricelistBulanan->harga_sewa * $tagihan->periode;
    echo "Tarif Bulanan: Rp " . number_format((float)$masterPricelistBulanan->harga_sewa, 0, ',', '.') . " × {$tagihan->periode} periode\n";
    echo "DPP Seharusnya: Rp " . number_format($correctDPP, 0, ',', '.') . "\n";
    echo "DPP Saat Ini: Rp " . number_format((float)$tagihan->dpp, 0, ',', '.') . "\n";
    echo "Selisih: Rp " . number_format($correctDPP - (float)$tagihan->dpp, 0, ',', '.') . "\n";
...
echo "Hasil Aktual: Rp " . number_format((float)$tagihan->dpp, 0, ',', '.') . "\n";
...
if ($days && $masterPricelistHarian) {
    ...
    $actualRate = (float)$tagihan->dpp / $days;
    echo "Tarif yang digunakan saat ini: Rp " . number_format($actualRate, 2, ',', '.') . " per hari\n";
}
