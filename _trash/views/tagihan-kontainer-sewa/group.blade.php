...
<div class="bg-white p-6 rounded-lg shadow-md">
    ...
    @if(isset($groupTotalHarga))
        <div class="mb-4">
            <div class="text-sm text-gray-500">Total Harga (sesuai Master Pricelist)</div>
            <div class="text-2xl font-bold">Rp {{ number_format($groupTotalHarga, 2, ',', '.') }}</div>
        </div>
    @endif
        ...
        <div class="flex items-center space-x-3">
            ...
            <form id="pranota-create-form" action="{{ route('pranota-tagihan-kontainer.store') }}" method="POST">
                ...
                <input type="hidden" name="tanggal" value="{{ $tanggal }}" />
                    ...
                    @php
                        // Prefer an explicit group_code passed from the controller (e.g. $group_code).
                        ...
                        $viewGroupCode = isset($group_code) && $group_code ? $group_code : ($sampleTagihan->group_code ?? null);
                        $viewPeriode = $periodeValue ?? ($sampleTagihan->periode ?? null);
                    @endphp
                    @if(!empty($viewGroupCode))
                        <input type="hidden" name="source_group_code" value="{{ $viewGroupCode }}" />
                    @endif
