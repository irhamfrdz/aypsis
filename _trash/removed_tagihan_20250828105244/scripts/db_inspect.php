...
foreach ($db->query("PRAGMA table_info('tagihan_kontainer_sewa')") as $c) {
    echo $c['name'].PHP_EOL;
}
...
foreach ($db->query("SELECT name FROM sqlite_master WHERE type='table' AND name='tagihan_kontainer_sewa_kontainers'") as $r) {
    foreach ($db->query("PRAGMA table_info('tagihan_kontainer_sewa_kontainers')") as $col) {
...
if ($pr) {
    ...
    foreach ($db->query('SELECT * FROM tagihan_kontainer_sewa_kontainers WHERE tagihan_id = '.intval($pid)) as $row) {
